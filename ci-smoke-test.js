...
async function runSmokeTest() {
  ...
  try {
    ...
    const healthOk = await makeRequest('/health', 200, 'Health endpoint always returns 200');
    
    // Test 2: Ready endpoint should reflect core service status
    const readyOk = await makeRequest('/ready', 200, 'Ready endpoint returns 200 when core services ready');
    
    // Test 3: Debug flags endpoint for monitoring
    const flagsOk = await makeRequest('/debug/flags', 200, 'Debug flags endpoint accessible');
