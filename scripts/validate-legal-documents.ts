...
} from '../lib/payroll/config/legal-document-tracker.js';
...
class LegalDocumentValidator {
  ...
  async validateAll(): Promise<ValidationResult> {
    ...
    try {
      ...
      console.log(
        `📚 Found ${registry.documentRegistry.documents.length} documents in registry`
      );
  ...
  private async validateDocument(
    ...
    if (!existsSync(filePath)) {
      ...
      this.errors.push(
        `File not found: ${doc.localPath} for document ${doc.documentId}`
      );
    } else {
      ...
      if (doc.checksumSHA256) {
        ...
        if (actualChecksum !== doc.checksumSHA256) {
          ...
          this.errors.push(
            `Checksum mismatch for ${doc.documentId}: expected ${doc.checksumSHA256}, got ${actualChecksum}`
          );
        }
      } else {
    ...
    if (doc.currentVersion && !/^\d+\.\d+(\.\d+)?$/.test(doc.currentVersion)) {
      ...
      this.errors.push(
        `Invalid version format for ${doc.documentId}: ${doc.currentVersion}`
      );
    }
    ...
    if (doc.officialUrl && !this.isValidUrl(doc.officialUrl)) {
      ...
      this.warnings.push(
        `Invalid URL for ${doc.documentId}: ${doc.officialUrl}`
      );
    }
   ...
   * Validate registry metadata
   */
  private async validateRegistryMetadata(
    ...
    const actualActive = registry.documentRegistry.documents.filter(
      (doc: LegalDocument) => doc.status === 'ACTIVE'
    ).length;
    const actualArchived = registry.documentRegistry.documents.filter(
      (doc: LegalDocument) => doc.status === 'ARCHIVED'
    ).length;

    if (metadata.totalDocuments !== actualTotal) {
      this.warnings.push(
        `Metadata total count mismatch: reported ${metadata.totalDocuments}, actual ${actualTotal}`
      );
    }

    if (metadata.activeDocuments !== actualActive) {
      this.warnings.push(
        `Metadata active count mismatch: reported ${metadata.activeDocuments}, actual ${actualActive}`
      );
    }

    if (metadata.archivedDocuments !== actualArchived) {
      this.warnings.push(
        `Metadata archived count mismatch: reported ${metadata.archivedDocuments}, actual ${actualArchived}`
      );
    }
    ...
    if (!validStatuses.includes(metadata.complianceStatus)) {
      this.errors.push(
        `Invalid compliance status: ${metadata.complianceStatus}`
      );
    }
  }
...
async function main() {
  ...
  if (result.isValid) {
    ...
    console.log('\n💡 Recommendations:');
    console.log(
      '  1. Ensure all referenced files exist in the legal-docs directory'
    );
