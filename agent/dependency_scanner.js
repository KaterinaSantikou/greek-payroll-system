...
class DependencyScanner {
    ...
    scanProject() {
        ...
        console.log(`✅ Dependency scan complete: ${this.dependencyGraph.size} files analyzed`);
        return {
    ...
    scanFile(sourceFile) {
        ...
        for (const importDecl of importDeclarations) {
            ...
            } else if (!moduleSpecifier.startsWith('node:') && !this.isNodeModule(moduleSpecifier)) {
    ...
    analyzeChangeImpact(changedFiles) {
        console.log(`📊 Analyzing impact of changes to ${changedFiles.length} files...`);
        ...
        for (const changedFile of changedFiles) {
            this.findAffectedFiles(changedFile, impact.directlyAffected, impact.indirectlyAffected);
        }
        
        // Assess risk level
        const totalAffected = impact.directlyAffected.size + impact.indirectlyAffected.size;
        if (totalAffected > 20) {
        ...
        impact.potentialBreakingChanges = this.identifyBreakingChanges(changedFiles, impact.directlyAffected);
     ...
     */
    findAffectedFiles(filePath, directlyAffected, indirectlyAffected, visited = new Set(), depth = 0) {
        if (visited.has(filePath) || depth > 10) { // Prevent infinite loops and limit depth
            return;
        }
        ...
        for (const dependent of dependents) {
            ...
            this.findAffectedFiles(dependent, directlyAffected, indirectlyAffected, visited, depth + 1);
        }
    }
    ...
    identifyBreakingChanges(changedFiles, affectedFiles) {
        ...
        for (const changedFile of changedFiles) {
            ...
            if (changedFile.includes('shared/') || changedFile.includes('types') || changedFile.includes('schema')) {
            ...
            if (changedFile.includes('api/') || changedFile.includes('service') || changedFile.includes('routes')) {
                breakingChanges.push({
                    file: changedFile,
        ...
        .then(result => {
            if (result.success) {
                ...
                if (result.impactAnalysis) {
                    ...
                    console.log(`   Directly affected: ${result.impactAnalysis.directlyAffected.length}`);
                    console.log(`   Indirectly affected: ${result.impactAnalysis.indirectlyAffected.length}`);
