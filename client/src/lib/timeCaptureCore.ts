...
export class TimeCaptureValidator {
  static validatePunch(
    event: TimePunchEvent,
  ...
  private static validateScheduleMatch(
    event: TimePunchEvent,
  ...
  private static validateDoublePunch(
    event: TimePunchEvent,
    recentEvents: TimePunchEvent[]
  ): ValidationResult {
    const sameTypeEvents = recentEvents.filter(
      e => e.employeeId === event.employeeId && e.punchType === event.punchType
    );
    ...
    if (sameTypeEvents.length > 0) {
      ...
      const timeDiff =
        ...
        new Date(lastSameType.timestamp).getTime();
  ...
  private static validateBreakRules(
    event: TimePunchEvent,
    context: ValidationContext
  ): ValidationResult {
    if (event.punchType === 'break-start') {
      // Check if already on break
      const onBreak = context.recentEvents.some(
        e =>
          e.employeeId === event.employeeId &&
          e.punchType === 'break-start' &&
          !context.recentEvents.some(
            be =>
  ...
  private static validateMaxHours(
    event: TimePunchEvent,
    context: ValidationContext
  ): ValidationResult {
  ...
  private static validateCBARules(
    event: TimePunchEvent,
    cbaRules: CBARule[]
  ): ValidationResult[] {
  ...
  private static calculateWorkedHours(events: TimePunchEvent[]): number {
    ...
    events.forEach(event => {
      if (event.punchType === 'in') {
        ...
        const hoursWorked =
          (new Date(event.timestamp).getTime() -
            new Date(lastIn.timestamp).getTime()) /
          (1000 * 60 * 60);
...
export class ErganiIIIntegration {
  private static apiEndpoint =
    import.meta.env.VITE_ERGANI_API_ENDPOINT || '/api/ergani';
    ...
    for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
      try {
        ...
        if (response.ok) {
          ...
          throw new Error(
            `ERGANI API error: ${response.status} - ${response.statusText}`
          );
        }
      } catch (error) {
  ...
  private static transformToErganiFormat(
    event: TimePunchEvent
  ): ErganiEventPayload {
    return {
