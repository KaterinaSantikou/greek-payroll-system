...
export const REGULAR_ALLOWANCES = {
  TRANSPORT: {
    ...
    types: {
      ...
      COMPANY_CAR: {
        name: 'Εταιρικό Αυτοκίνητο',
        calculation: 'benefit_in_kind',
      },
      MILEAGE: {
        name: 'Χιλιομετρικό Επίδομα',
        ratePerKm: 0.2,
        maxMonthly: 300,
      },
      FUEL_ALLOWANCE: {
        name: 'Επίδομα Καυσίμων',
        maxMonthly: 200,
        taxExempt: 150,
      },
    },
  ...
  FOOD: {
    ...
    types: {
      ...
      COMPANY_CANTEEN: {
        name: 'Εταιρικό Εστιατόριο',
        calculation: 'subsidized_rate',
      },
    },
    mandatory: false,
    taxable: true,
  ...
  POSITION: {
    ...
    types: {
      MANAGEMENT: {
        name: 'Διοικητικό',
        percentage: 0.15,
        description: 'Επίδομα διοικητικής θέσης',
      },
      SUPERVISOR: {
        name: 'Επιβλεπτικό',
        percentage: 0.1,
        description: 'Επίδομα επίβλεψης',
      },
      SPECIALIST: {
        name: 'Ειδικότητας',
        percentage: 0.08,
        description: 'Επίδομα ειδικής ειδικότητας',
      },
      RESPONSIBILITY: {
        name: 'Ευθύνης',
        percentage: 0.12,
        description: 'Επίδομα ειδικής ευθύνης',
      },
      BILINGUAL: {
        name: 'Δίγλωσσο',
        fixedAmount: 150,
        description: 'Επίδομα ξένης γλώσσας',
      },
    },
    mandatory: false,
    taxable: true,
...
export const INDUSTRY_ALLOWANCES = {
  BANKING: {
    ...
    allowances: {
      CASH_HANDLING: {
        name: 'Χειρισμός Μετρητών',
        fixedAmount: 200,
        description: 'Επίδομα χειρισμού χρημάτων',
      },
      BRANCH_MANAGER: {
        name: 'Διευθυντής Καταστήματος',
        percentage: 0.2,
        description: 'Επίδομα διεύθυνσης',
      },
      SALES_TARGET: {
        name: 'Στόχων Πωλήσεων',
        calculation: 'performance_based',
        description: 'Επίδομα επίτευξης στόχων',
      },
    },
  },
  ...
  TOURISM: {
    ...
    allowances: {
      SEASONAL: {
        name: 'Εποχιακό',
        percentage: 0.15,
        description: 'Επίδομα εποχιακής εργασίας',
      },
      LANGUAGE: {
        name: 'Ξένων Γλωσσών',
        fixedAmount: 100,
      ...
      CUSTOMER_SERVICE: {
        name: 'Εξυπηρέτησης',
        fixedAmount: 80,
        description: 'Επίδομα εξυπηρέτησης πελατών',
      },
    },
  },
  ...
  CONSTRUCTION: {
    ...
    allowances: {
      DANGEROUS_WORK: {
        name: 'Επικίνδυνης Εργασίας',
        percentage: 0.2,
        description: 'Επίδομα επικινδυνότητας',
      },
      HEIGHT_WORK: {
        name: 'Εργασίας σε Ύψος',
        percentage: 0.25,
        description: 'Επίδομα εργασίας σε ύψος',
      },
      HEAVY_MACHINERY: {
        name: 'Βαρέων Μηχανημάτων',
        fixedAmount: 150,
        description: 'Επίδομα χειρισμού μηχανημάτων',
      },
    },
  },
  ...
  HEALTHCARE: {
    ...
    allowances: {
      NIGHT_SHIFT: {
        name: 'Νυχτερινής Βάρδιας',
        percentage: 0.25,
        description: 'Επίδομα νυχτερινής εργασίας',
      },
      INFECTION_RISK: {
        name: 'Κινδύνου Μόλυνσης',
        percentage: 0.15,
        description: 'Επίδομα κινδύνου μόλυνσης',
      },
      SPECIALIST_MEDICAL: {
        name: 'Ιατρικής Ειδικότητας',
        fixedAmount: 300,
        description: 'Επίδομα ιατρικής ειδικότητας',
      },
    },
  },
  ...
  COMMERCE: {
    ...
    allowances: {
      SALES_COMMISSION: {
        name: 'Προμήθεια Πωλήσεων',
        calculation: 'commission_based',
        description: 'Προμήθεια επί των πωλήσεων',
      },
      CASHIER: {
        name: 'Ταμείου',
        fixedAmount: 100,
        description: 'Επίδομα ταμείου',
      },
      INVENTORY: {
        name: 'Αποθήκης',
        fixedAmount: 80,
        description: 'Επίδομα διαχείρισης αποθήκης',
      },
    },
  },
};
...
export function calculateRegularAllowance(
  ...
  const subTypeConfig = allowance.types[
    subType as keyof typeof allowance.types
  ] as any;

  if (!subTypeConfig) {
    throw new Error(
      `Invalid subtype ${subType} for allowance ${allowanceType}`
    );
  }
...
export function calculateIndustryAllowance(
  ...
  const allowanceConfig = industryConfig.allowances[
    allowanceCode as keyof typeof industryConfig.allowances
  ] as any;

  if (!allowanceConfig) {
    throw new Error(
      `Invalid allowance ${allowanceCode} for industry ${industry}`
    );
  }
  ...
  if (allowanceConfig.percentage) {
    ...
    allowanceConfig.calculation === 'performance_based' &&
    performanceData
  ) {
    ...
    allowanceConfig.calculation === 'commission_based' &&
    performanceData
  ) {
    // Commission-based calculation
    monthlyAmount =
      performanceData.salesAmount * (performanceData.commissionRate || 0.02);
  }
...
export function calculateFamilyAllowances(
  ...
  // Children allowances
  const eligibleChildren = childrenAges.filter(
    age =>
...
export function calculateTotalAllowances(
  ...
  selectedAllowances.forEach(allowance => {
    ...
    if (GREEK_HOLIDAY_BONUSES[mainType as keyof typeof GREEK_HOLIDAY_BONUSES]) {
      ...
      REGULAR_ALLOWANCES[mainType as keyof typeof REGULAR_ALLOWANCES]
    ) {
      // Regular allowance
