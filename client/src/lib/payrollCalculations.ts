...
export const MINIMUM_WAGE_2025 = {
  ...
  DAILY: {
    BLUE_COLLAR: {
      amount: 39.3, // €39.30 per day for blue-collar workers (effective April 1, 2025)
      description: 'Κατώτατος ημερομίσθιο εργατών 2025',
      effectiveDate: '2025-04-01',
      previousAmount: 34.1, // Previous daily amount
...
export const OVERTIME_RATES = {
  ...
  dailyExtended: 0.5, // 50% after 2 hours
...
export function calculateSolidarityTax(annualIncome: number): number {
  ...
  return (
    Math.round(
      (annualIncome - SOLIDARITY_TAX_THRESHOLD) * SOLIDARITY_TAX_RATE * 100
    ) / 100
  );
}
...
export function calculateEfkaContributions(
  ...
  if (specialCategory && SPECIAL_INSURANCE_RATES[specialCategory]) {
    const additionalRate =
      SPECIAL_INSURANCE_RATES[specialCategory].additionalRate;
  ...
  const unemploymentEmployee =
    Math.round(grossSalary * EFKA_RATES.employee.unemployment * 100) / 100;
  const unemploymentEmployer =
    Math.round(grossSalary * EFKA_RATES.employer.unemployment * 100) / 100;
  ...
  return {
    ...
    total:
      employeeMain + employerMain + unemploymentEmployee + unemploymentEmployer,
  };
}
    ...
    case 'daily':
      ...
      if (overtimeHours <= 2) {
        ...
        const remainingHours =
          (overtimeHours - 2) * hourlyRate * OVERTIME_RATES.dailyExtended;
...
export const COLLECTIVE_AGREEMENTS = {
  ...
  CONSTRUCTION: {
    ...
    minimumWage: 800,
    hazardBonus: 0.2, // 20% hazard bonus
    experienceBonus: [
      { years: 0, bonus: 0 },
      { years: 5, bonus: 80 },
      { years: 10, bonus: 160 },
...
export function calculateCompletePayroll(params: {
  ...
  const grossTotal =
    grossSalary + overtimePay + sundayPay + allowances + bonuses;

  // Calculate tax allowances
  const taxAllowances = calculateTaxFreeAllowances(
    maritalStatus,
    children,
    hasDisability
  );
  ...
  const totalDeductions =
    incomeTax + solidarityTax + efka.employee + efka.unemploymentEmployee;
  const totalEmployerCosts =
    efka.employer +
    efka.unemploymentEmployer +
    grossTotal * EFKA_RATES.employer.family;
...
export function getIndustrySectors() {
  return Object.keys(COLLECTIVE_AGREEMENTS).map(key => ({
    value: key,
    label:
      COLLECTIVE_AGREEMENTS[key as keyof typeof COLLECTIVE_AGREEMENTS].name,
  }));
}
...
export function validateMinimumWage(
  ...
  const minimumWage =
    agreement &&
    COLLECTIVE_AGREEMENTS[agreement as keyof typeof COLLECTIVE_AGREEMENTS]
      ? COLLECTIVE_AGREEMENTS[agreement as keyof typeof COLLECTIVE_AGREEMENTS]
          .minimumWage
      : MINIMUM_WAGE_2025;
