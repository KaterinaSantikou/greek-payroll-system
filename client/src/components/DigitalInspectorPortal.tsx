...
const INSPECTIONS: Inspection[] = [
  {
    ...
    categoriesEl: ['Ωράριο Εργασίας', 'Πρότυπα Ασφαλείας', 'Αρχεία Εργαζομένων'],
    findings: [],
    overallScore: 0,
    violationsFound: 0,
    ...
    findings: [
      {
        ...
        severity: 'minor',
        description: 'Minor discrepancies in overtime calculations for 3 employees',
        descriptionEl: 'Μικρές αποκλίσεις στους υπολογισμούς υπερωριών για 3 εργαζομένους',
        violation: true,
  ...
  },
  {
    ...
    categories: ['Safety Compliance', 'Working Conditions', 'Equipment Standards'],
    categoriesEl: ['Συμμόρφωση Ασφαλείας', 'Συνθήκες Εργασίας', 'Πρότυπα Εξοπλισμού'],
    findings: [
      {
        id: 'find-002',
...
const INSPECTORS: Inspector[] = [
  {
    ...
    specializationsEl: ['Εργατικό Δίκαιο', 'Πρότυπα Ασφαλείας', 'Συμμόρφωση Μισθοδοσίας'],
  ...
  },
  {
    ...
    specializations: ['Insurance Compliance', 'Working Hours', 'Employee Rights'],
    specializationsEl: ['Συμμόρφωση Ασφάλισης', 'Ώρες Εργασίας', 'Δικαιώματα Εργαζομένων'],
...
export default function DigitalInspectorPortal({ locale = 'en' }: DigitalInspectorPortalProps) {
  ...
  const [inspectorCredentials, setInspectorCredentials] = useState({ badgeNumber: '', password: '' });
  ...
  const translations = {
    ...
    el: {
      ...
      auth: {
        ...
        logout: 'Αποσύνδεση',
        unauthorized: 'Άρνηση πρόσβασης. Παρακαλώ επαληθεύστε τα διαπιστευτήριά σας.'
      },
      metrics: {
  ...
  const filteredCompanies = companies.filter(company => {
    const sectorMatch = filterSector === 'all' || company.sector.toLowerCase() === filterSector;
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50 flex items-center justify-center p-6">
        <Card className="w-full max-w-md">
          ...
          <CardContent className="space-y-4">
            <div>
              <label className="block text-sm font-medium mb-2">{t.auth.badgeNumber}</label>
              <Input
                type="text"
                value={inspectorCredentials.badgeNumber}
                onChange={(e) => setInspectorCredentials(prev => ({...prev, badgeNumber: e.target.value}))}
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => setInspectorCredentials(prev => ({...prev, password: e.target.value}))}
                placeholder="••••••••"
              />
            </div>
            <Button 
              ...
              onClick={handleLogin}
              disabled={!inspectorCredentials.badgeNumber || !inspectorCredentials.password}
            >
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 xl:grid-cols-8 gap-4 mb-6">
            <Card className="p-4 bg-gradient-to-r from-blue-500 to-blue-600 text-white">
              <div className="text-center">
                <div className="text-2xl font-bold">{INSPECTION_METRICS.totalInspections}</div>
                <div className="text-xs text-blue-100">{t.metrics.totalInspections}</div>
              </div>
            </Card>

            <Card className="p-4 bg-gradient-to-r from-green-500 to-green-600 text-white">
              <div className="text-center">
                <div className="text-2xl font-bold">{INSPECTION_METRICS.activeInspections}</div>
                <div className="text-xs text-green-100">{t.metrics.activeInspections}</div>
              </div>
            </Card>

            <Card className="p-4 bg-gradient-to-r from-purple-500 to-purple-600 text-white">
              <div className="text-center">
                <div className="text-2xl font-bold">{INSPECTION_METRICS.completedThisMonth}</div>
                <div className="text-xs text-purple-100">{t.metrics.completedThisMonth}</div>
              </div>
            </Card>

            <Card className="p-4 bg-gradient-to-r from-orange-500 to-orange-600 text-white">
              <div className="text-center">
                <div className="text-2xl font-bold">{INSPECTION_METRICS.violationsFound}</div>
                <div className="text-xs text-orange-100">{t.metrics.violationsFound}</div>
              </div>
            </Card>

            <Card className="p-4 bg-gradient-to-r from-teal-500 to-teal-600 text-white">
              <div className="text-center">
                <div className="text-2xl font-bold">{INSPECTION_METRICS.complianceRate}%</div>
                <div className="text-xs text-teal-100">{t.metrics.complianceRate}</div>
              </div>
            </Card>

            <Card className="p-4 bg-gradient-to-r from-indigo-500 to-indigo-600 text-white">
              <div className="text-center">
                <div className="text-2xl font-bold">{INSPECTION_METRICS.averageInspectionTime}m</div>
                <div className="text-xs text-indigo-100">{t.metrics.averageInspectionTime}</div>
              </div>
            </Card>

            <Card className="p-4 bg-gradient-to-r from-red-500 to-red-600 text-white">
              <div className="text-center">
                <div className="text-2xl font-bold">{INSPECTION_METRICS.finesIssued}</div>
                <div className="text-xs text-red-100">{t.metrics.finesIssued}</div>
              </div>
            </Card>

            <Card className="p-4 bg-gradient-to-r from-yellow-500 to-yellow-600 text-white">
              <div className="text-center">
                <div className="text-2xl font-bold">€{(INSPECTION_METRICS.totalFineAmount / 1000).toFixed(0)}K</div>
                <div className="text-xs text-yellow-100">{t.metrics.totalFineAmount}</div>
              </div>
            </Card>
          </div>
        </div>
            ...
            <div className="grid lg:grid-cols-3 gap-6">
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      {companies.filter(company => company.riskLevel === 'high' || company.riskLevel === 'critical').map((company) => {
                        const SectorIcon = getSectorIcon(company.sector);
                        
                        return (
                          <div key={company.id} className="flex items-center justify-between p-4 border border-red-200 rounded-lg bg-red-50">
                            <div className="flex items-center gap-4">
                              ...
                              <div>
                                ...
                                <div className="text-sm text-gray-600">
                                  AFM: {company.afm} • {company.employees} employees • {company.activeViolations} violations
                                </div>
                                <Badge className={`${getRiskColor(company.riskLevel)} text-xs mt-1`}>
              ...
              <Card>
                ...
                <CardContent className="space-y-3">
                  {inspections.filter(inspection => inspection.status === 'in_progress' || inspection.status === 'scheduled').map((inspection) => {
                    const company = companies.find(c => c.id === inspection.companyId);
                    
                    return (
                      <div key={inspection.id} className="flex items-start gap-3 text-sm p-3 border rounded">
                        <div className={`w-2 h-2 rounded-full mt-2 ${
                          inspection.status === 'in_progress' ? 'bg-green-500' : 'bg-yellow-500'
                        }`}></div>
                        <div className="flex-1">
                          ...
                          <div className="text-gray-600">
                            {t.inspectionType[inspection.type as keyof typeof t.inspectionType]} • {inspection.scheduledDate}
                          </div>
                          <Badge className={getStatusColor(inspection.status)} variant="outline">
                            {t.status[inspection.status as keyof typeof t.status]}
                          </Badge>
                        </div>
                ...
                <div className="space-y-4">
                  <div className="flex items-start gap-4 p-4 border rounded-lg">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-sm text-gray-600">Athens Retail Solutions - Follow-up inspection completed successfully</div>
                      <div className="text-xs text-gray-500 mt-1">Today, 2:30 PM by Sofia Kalogerou</div>
                    </div>
                    <Badge className="bg-green-100 text-green-800">Score: 92%</Badge>
                  </div>
                  ...
                  <div className="flex items-start gap-4 p-4 border rounded-lg">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-sm text-gray-600">Hellenic Manufacturing Co. - Safety compliance inspection ongoing</div>
                      <div className="text-xs text-gray-500 mt-1">Started today, 10:00 AM by Kostas Dimitriou</div>
                    </div>
                    <Badge className="bg-orange-100 text-orange-800">1 Violation</Badge>
                  </div>
                  ...
                  <div className="flex items-start gap-4 p-4 border rounded-lg">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-sm text-gray-600">Aegean Hotels Group - Routine inspection scheduled</div>
                      <div className="text-xs text-gray-500 mt-1">December 20, 2024 at 9:00 AM</div>
                    </div>
                    ...
                    <CardContent className="p-6">
                      <div className="flex items-start justify-between mb-4">
                        <div className="flex items-start gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-gray-600 text-sm mb-2">
                              AFM: {company.afm} • {company.sector} • {company.employees} employees
                            </p>
                            <div className="flex items-center gap-4 text-sm text-gray-500">
                              <span>{t.company.address}: {locale === 'en' ? company.address : company.addressEl}</span>
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <Badge className={`${getRiskColor(company.riskLevel)} border-current`} variant="outline">
                            {t.risk[company.riskLevel as keyof typeof t.risk]}
                          </Badge>
                        </div>
                      </div>
                      ...
                      <div className="grid md:grid-cols-4 gap-4 mb-4">
                        <div>
                          <div className="text-sm font-medium text-gray-700 mb-1">{t.company.complianceScore}</div>
                          <div className="flex items-center gap-2">
                            <Progress value={company.complianceScore} className="flex-1" />
                            <span className="text-sm font-medium">{company.complianceScore}%</span>
                          </div>
                        </div>

                        <div>
                          <div className="text-sm font-medium text-gray-700 mb-1">{t.company.activeViolations}</div>
                          <div className="text-lg font-bold text-red-600">{company.activeViolations}</div>
                        </div>

                        <div>
                          <div className="text-sm font-medium text-gray-700 mb-1">{t.company.lastInspection}</div>
                        ...
                        <div>
                          <div className="text-sm font-medium text-gray-700 mb-1">{t.company.legalRepresentative}</div>
                          <div className="text-sm font-medium">
                            {locale === 'en' ? company.legalRepresentative : company.legalRepresentativeEl}
                          </div>
                        </div>
                      </div>
                        ...
                        <div className="flex items-start justify-between mb-3">
                          <div>
                            ...
                            <p className="text-sm text-gray-600 mt-1">
                              {t.inspectionType[inspection.type as keyof typeof t.inspectionType]} • 
                              Inspector: {locale === 'en' ? inspection.inspectorName : inspection.inspectorNameEl}
                            </p>
                            <div className="flex items-center gap-4 text-xs text-gray-500 mt-2">
                              <span>Scheduled: {inspection.scheduledDate}</span>
                              {inspection.completedDate && <span>Completed: {inspection.completedDate}</span>}
                              {inspection.duration && <span>Duration: {inspection.duration}m</span>}
                            </div>
                          </div>
                          <div className="flex items-center gap-2">
                        ...
                        <div className="grid md:grid-cols-4 gap-4 mb-3">
                          <div>
                            ...
                            <div className="text-sm">
                              {(locale === 'en' ? inspection.categories : inspection.categoriesEl).join(', ')}
                            </div>
                          </div>
                          ...
                          {inspection.status === 'completed' && (
                            <>
                              <div>
                                ...
                                <div className="text-sm font-bold text-blue-600">{inspection.overallScore}%</div>
                              </div>

                              <div>
                                <div className="text-xs font-medium text-gray-700">Violations</div>
                                <div className="text-sm font-bold text-red-600">{inspection.violationsFound}</div>
                              </div>

                              <div>
                                <div className="text-xs font-medium text-gray-700">Fines</div>
                                <div className="text-sm font-bold text-orange-600">€{inspection.totalFineAmount}</div>
                              </div>
                            </>
                          )}
                        </div>
                          ...
                          <div className="mb-3">
                            ...
                            <div className="space-y-2">
                              {inspection.findings.map((finding) => (
                                <div key={finding.id} className="flex items-start gap-3 p-2 bg-gray-50 rounded">
                                  <div className={`p-1 rounded ${
                                    finding.violation ? 'bg-red-100' : 'bg-green-100'
                                  }`}>
                                    {finding.violation ? (
                                  ...
                                  <div className="flex-1">
                                    <div className="text-sm font-medium">
                                      {locale === 'en' ? finding.category : finding.categoryEl}
                                    </div>
                                    <div className="text-xs text-gray-600">
                                      {locale === 'en' ? finding.description : finding.descriptionEl}
                                    </div>
                                    <div className="flex items-center gap-4 text-xs text-gray-500 mt-1">
                                      <span>Law: {finding.lawReference}</span>
                                      {finding.fineAmount && <span>Fine: €{finding.fineAmount}</span>}
                                      <Badge className={getStatusColor(finding.status)} variant="outline">
                                        {t.status[finding.status as keyof typeof t.status]}
                                      </Badge>
                                    </div>
                                  </div>
                                </div>
                          ...
                          <div className="flex items-center gap-4 text-sm text-gray-600">
                            {inspection.followUpRequired && (
                              <Badge className="bg-yellow-100 text-yellow-800">
                                Follow-up Required
                                {inspection.followUpDate && ` - ${inspection.followUpDate}`}
                              </Badge>
                            )}
                          </div>
                          <div className="flex items-center gap-2">
          ...
          <TabsContent value="reports" className="space-y-6">
            <div className="grid lg:grid-cols-2 gap-6">
              <Card>
                ...
                <CardContent>
                  <div className="space-y-4">
                    {['Tourism', 'Retail', 'Manufacturing', 'Healthcare'].map((sector, index) => {
                      const compliance = [87, 92, 78, 95][index];
                      return (
                        <div key={sector} className="flex items-center gap-3">
                          <div className="w-20 text-sm font-medium">{sector}</div>
                          <Progress value={compliance} className="flex-1" />
                          <div className="w-12 text-sm font-medium text-right">{compliance}%</div>
                        </div>
                      );
                    })}
                  </div>
                ...
                <CardContent>
                  <div className="space-y-4">
                    <div className="flex justify-between">
                      <span className="text-sm text-gray-600">Inspections conducted</span>
                      <span className="text-sm font-medium">{INSPECTION_METRICS.completedThisMonth}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-sm text-gray-600">Violations found</span>
                      <span className="text-sm font-medium">{INSPECTION_METRICS.violationsFound}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-sm text-gray-600">Fines issued</span>
                      <span className="text-sm font-medium">{INSPECTION_METRICS.finesIssued}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-sm text-gray-600">Total fine amount</span>
                      <span className="text-sm font-medium">€{INSPECTION_METRICS.totalFineAmount.toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-sm text-gray-600">Average inspection time</span>
                      <span className="text-sm font-medium">{INSPECTION_METRICS.averageInspectionTime} minutes</span>
                    </div>
                  </div>
                </CardContent>
              </Card>
            ...
            <Card>
              ...
              <CardContent>
                <div className="grid md:grid-cols-3 gap-6">
                  <div className="text-center">
                    <div className="text-2xl font-bold text-green-600 mb-2">{INSPECTION_METRICS.complianceRate}%</div>
                    <div className="text-sm text-gray-600">Overall Compliance Rate</div>
                  </div>
                  
                  <div className="text-center">
                    <div className="text-2xl font-bold text-blue-600 mb-2">{INSPECTION_METRICS.averageInspectionTime}m</div>
                    <div className="text-sm text-gray-600">Average Inspection Duration</div>
                  </div>
                  
                  <div className="text-center">
                    <div className="text-2xl font-bold text-orange-600 mb-2">
                      {(INSPECTION_METRICS.violationsFound / INSPECTION_METRICS.totalInspections * 100).toFixed(1)}%
                    </div>
                    <div className="text-sm text-gray-600">Violation Detection Rate</div>
                  </div>
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent className="space-y-6">
                <div>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      <div>
                        <label className="font-medium">High-risk company alerts</label>
                        <p className="text-sm text-gray-600">Receive notifications for high-risk companies</p>
                      </div>
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        <label className="font-medium">Inspection deadline reminders</label>
                        <p className="text-sm text-gray-600">Automatic reminders for scheduled inspections</p>
                      </div>
                      <Button variant="default" size="sm">Enabled</Button>
                    </div>
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        <label className="font-medium">Violation correction updates</label>
                        <p className="text-sm text-gray-600">Updates when companies correct violations</p>
                      </div>
                      <Button variant="default" size="sm">Enabled</Button>
                    </div>
                  </div>
