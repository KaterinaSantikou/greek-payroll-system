...
export function JobSalaryRangeGenerator({ propertyId = "demo-property" }: { propertyId?: string }) {
  const [generatedRange, setGeneratedRange] = useState<GeneratedRange | null>(null);
  const [isGenerated, setIsGenerated] = useState(false);
  const { toast } = useToast();
  const queryClient = useQueryClient();

  const { register, handleSubmit, formState: { errors }, reset } = useForm<SalaryRangeForm>();

  const generateRangeMutation = useMutation({
    mutationFn: async (data: SalaryRangeForm & { propertyId: string; createdBy: string }) => {
    ...
    onSuccess: (result: { data: any }) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['/api/pay-equity/salary-ranges', propertyId] });
    },
    onError: (error) => {
      toast({
  ...
  return (
    <Card className="w-full">
      <CardHeader>
        ...
        <CardDescription>
          Generate EU-compliant salary ranges for job advertisements (Article 5, EU 2023/970)
        </CardDescription>
      </CardHeader>
      ...
      <CardContent className="space-y-6">
        {!isGenerated ? (
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="benefitsDescription">Benefits & Additional Compensation</Label>
              <Textarea
            ...
            <Alert>
              ...
              <AlertDescription>
                <strong>EU 2023/970 Compliance:</strong> Salary ranges must be included in job advertisements. 
                The range will be calculated based on internal equity data and market benchmarks.
              </AlertDescription>
            </Alert>
          ...
          <div className="space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="text-center">
                    <div className="text-3xl font-bold text-green-700 dark:text-green-300 mb-2">
                      {formatCurrency(generatedRange.suggestedRange.min)} - {formatCurrency(generatedRange.suggestedRange.max)}
                    </div>
                    <p className="text-sm text-green-600 dark:text-green-400">Monthly Salary Range</p>
                  </div>

                  {/* Market Data */}
                  {generatedRange.marketData && generatedRange.marketData.sampleSize && (
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-4 border-t border-green-200 dark:border-green-700">
                      <div className="text-center">
                        ...
                        <p className="text-xs text-green-600 dark:text-green-400">Sample Size</p>
                      </div>
                      <div className="text-center">
                        <div className="text-lg font-medium text-green-700 dark:text-green-300">
                          {formatCurrency(generatedRange.marketData.median)}
                        </div>
                        <p className="text-xs text-green-600 dark:text-green-400">Median</p>
                      </div>
                      <div className="text-center">
                        <div className="text-lg font-medium text-green-700 dark:text-green-300">
                          {formatCurrency(generatedRange.marketData.p25)}
                        </div>
                        <p className="text-xs text-green-600 dark:text-green-400">25th Percentile</p>
                      </div>
                      <div className="text-center">
                        <div className="text-lg font-medium text-green-700 dark:text-green-300">
                          {formatCurrency(generatedRange.marketData.p75)}
                        </div>
                        <p className="text-xs text-green-600 dark:text-green-400">75th Percentile</p>
                      </div>
                    </div>
                  )}
                </div>
            ...
            {generatedRange?.factors && (
              <div className="space-y-3">
                ...
                <p className="text-xs text-gray-600 dark:text-gray-400">
                  These factors must be disclosed to candidates as per EU transparency requirements.
                </p>
              </div>
            )}
            ...
            <Alert>
              ...
              <AlertDescription>
                <strong>✅ EU Compliant:</strong> This salary range meets Article 5 requirements and can be used in job advertisements. 
                Range is based on internal equity analysis and includes required pay factors disclosure.
              </AlertDescription>
            </Alert>
          </div>
        )}
