...
export function BillingDashboard({ organizationId }: Props) {
  ...
  const { data: subscriptionData, isLoading: subscriptionLoading } = useQuery({
    ...
    queryFn: () => apiRequest('GET', `/api/billing/subscription/${organizationId}`).then(res => res.json())
  });

  // Fetch invoices
  const { data: invoices = [], isLoading: invoicesLoading } = useQuery({
    queryKey: ['/api/billing/invoices', organizationId],
    queryFn: () => apiRequest('GET', `/api/billing/invoices/${organizationId}`).then(res => res.json())
  });

  // Fetch payment methods
  const { data: paymentMethods = [], isLoading: paymentMethodsLoading } = useQuery({
    queryKey: ['/api/billing/payment-methods', organizationId],
    queryFn: () => apiRequest('GET', `/api/billing/payment-methods/${organizationId}`).then(res => res.json())
  });

  // Fetch metering data
  const { data: meteringData = [] } = useQuery({
    queryKey: ['/api/billing/metering', organizationId],
    queryFn: () => apiRequest('GET', `/api/billing/metering/${organizationId}`).then(res => res.json())
  });

  // Payment processing mutation
  const processPaymentMutation = useMutation({
    mutationFn: ({ invoiceId, paymentMethodId }: { invoiceId: string; paymentMethodId?: string }) =>
      apiRequest('POST', `/api/billing/payment/${invoiceId}`, { paymentMethodId }),
    onSuccess: () => {
      toast({ title: "Payment processed successfully" });
      queryClient.invalidateQueries({ queryKey: ['/api/billing/invoices', organizationId] });
    },
    onError: (error: any) => {
      toast({
  ...
  const totalOutstanding = overdueInvoices.reduce((sum: number, inv: Invoice) => sum + inv.totalCents, 0);
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <p className="text-sm font-medium text-gray-600">Billable Employees</p>
                <p className="text-2xl font-bold">{latestMetering?.billableEmployees || 0}</p>
              </div>
            </div>
          </CardContent>
        </Card>
                      ...
                      <div className="flex items-center space-x-4">
                        ...
                        <div className="flex space-x-2">
                          <Button
                            ...
                            size="sm"
                            onClick={() => {
                              window.open(`/api/billing/invoice/${invoice.id}/pdf`, '_blank');
                            }}
                          >
                          ...
                          {invoice.status === 'sent' && (
                            <Button
                              size="sm"
                              onClick={() => processPaymentMutation.mutate({ invoiceId: invoice.id })}
                          ...
                          {!invoice.mydataTransmitted && invoice.status !== 'draft' && (
                            <Button
                              variant="outline"
                              size="sm"
                              onClick={() => transmitToMyDataMutation.mutate(invoice.id)}
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="text-lg">{formatCurrency(Math.round(parseFloat(plan.baseFeeEur) * 100))}/month</p>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-600">Per Employee</label>
                  <p className="text-lg">{formatCurrency(Math.round(parseFloat(plan.perEmployeeFeeEur) * 100))}/employee/month</p>
                </div>
                <div>
          ...
          <Card>
            ...
            <CardContent>
              {paymentMethodsLoading ? (
                ...
                <p className="text-gray-500 text-center py-8">No payment methods configured</p>
              ) : (
                <div className="space-y-4">
                        ...
                        <div>
                          {method.type === 'card' ? (
                            <div>
                              ...
                              <div className="text-sm text-gray-500">{method.cardBrand?.toUpperCase()}</div>
                            </div>
                          ) : (
                            <div>
                              <div className="font-medium">SEPA Direct Debit</div>
                              <div className="text-sm text-gray-500">***{method.sepaIban?.slice(-4)}</div>
                            </div>
                          )}
                        </div>
                      </div>
                  ...
                  {latestMetering && (
                    <div className="p-4 bg-blue-50 rounded-lg">
                      <h3 className="font-medium mb-2">
                        Current Month ({latestMetering.month}/{latestMetering.year})
                      </h3>
                      <div className="grid grid-cols-3 gap-4">
                        <div>
                          <div className="text-2xl font-bold">{latestMetering.billableEmployees}</div>
                          <div className="text-sm text-gray-600">Billable Employees</div>
                        </div>
                        <div>
                          <div className="text-2xl font-bold">{latestMetering.uniqueActiveEmployees}</div>
                          <div className="text-sm text-gray-600">Active Headcount</div>
                        </div>
                        <div>
                          <div className="text-2xl font-bold">{latestMetering.employeesWithPayActivity}</div>
                          <div className="text-sm text-gray-600">With Pay Activity</div>
                        </div>
                      </div>
                      <div className="mt-2 text-sm text-gray-600">
                        Method: {latestMetering.meteringMethod === 'active_headcount' ? 'Active Headcount' : 'Pay Activity'}
                  ...
                  <div>
                    ...
                    <div className="space-y-4">
                      {meteringData.slice(0, 6).map((data: any) => (
                        <div key={`${data.year}-${data.month}`} className="flex items-center justify-between p-3 border rounded">
                          <div>
                          ...
                          <div className="text-right">
                            <div className="font-medium">{data.billableEmployees} employees</div>
                            <div className="text-sm text-gray-500">
                              {data.meteringMethod === 'active_headcount' ? 'Headcount' : 'Pay Activity'}
                            </div>
                          </div>
                        </div>
                      ))}
