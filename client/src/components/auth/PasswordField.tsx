...
function calculatePasswordStrength(
  password: string,
  options?: PasswordFieldProps['strengthOptions']
): PasswordStrength {
  const {
    minLength = 8,
...
const PasswordFieldStandalone = forwardRef<
  HTMLInputElement,
  PasswordFieldProps & {
    value?: string;
...
>(
  (
    ...
    const strength = showStrengthMeter
      ? calculatePasswordStrength(value, strengthOptions)
      : null;
  ...
  return (
    <FormField
      ...
      render={({ field, fieldState }) => {
        const strength = showStrengthMeter
          ? calculatePasswordStrength(field.value || '', strengthOptions)
          : null;
