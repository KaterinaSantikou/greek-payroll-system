...
const EmailFieldStandalone = forwardRef<HTMLInputElement, EmailFieldProps & {
  value?: string;
  onChange?: (value: string) => void;
  error?: string;
}>(({ label = "Email", placeholder = "Enter your email", className, disabled, required, value, onChange, error, ...props }, ref) => {
  return (
    <div className="space-y-2">
      <Label htmlFor="email" className={cn(required && "after:content-['*'] after:ml-0.5 after:text-red-500")}>
        {label}
      </Label>
...
function EmailFieldFormComponent<
  TFieldValues extends FieldValues = FieldValues,
  TName extends FieldPath<TFieldValues> = FieldPath<TFieldValues>
>({ control, name, label = "Email", placeholder = "Enter your email", ...props }: EmailFieldFormProps<TFieldValues, TName>) {
  return (
    <FormField
      control={control}
      name={name}
