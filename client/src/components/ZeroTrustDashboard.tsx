...
const SECURITY_METRICS: SecurityMetric[] = [
  {
    ...
    trend: 'up',
    description:
      'Multi-factor authentication and identity verification coverage',
    descriptionEl:
      'Κάλυψη πολυπαραγοντικής αυθεντικοποίησης και επαλήθευσης ταυτότητας',
    category: 'identity',
  },
  {
    ...
    trend: 'stable',
    description:
      'Percentage of devices meeting security compliance requirements',
    descriptionEl:
      'Ποσοστό συσκευών που πληρούν τις απαιτήσεις συμμόρφωσης ασφαλείας',
    category: 'devices',
  },
  {
    ...
    descriptionEl:
      'Αποτελεσματικότητα μικρο-κατάτμησης δικτύου και ελέγχου πρόσβασης',
    category: 'network',
  },
  {
    id: 'data-encryption',
    ...
    descriptionEl:
      'Κάλυψη κρυπτογράφησης από άκρο σε άκρο για ευαίσθητα δεδομένα μισθοδοσίας',
    category: 'data',
  },
  {
    ...
    trend: 'up',
    description:
      'Application-level security controls and vulnerability management',
    descriptionEl:
      'Έλεγχοι ασφαλείας σε επίπεδο εφαρμογής και διαχείριση ευπάθειας',
...
const ACCESS_REQUESTS: AccessRequest[] = [
  {
    ...
    justification:
      'Need access to submit new employee data to ERGANI II system',
  },
  {
    id: 'req-002',
    ...
    justification:
      'Regular monthly payroll run requires banking integration access',
  },
];
...
export default function ZeroTrustDashboard({
  locale = 'en',
}: ZeroTrustDashboardProps) {
  const [activeTab, setActiveTab] = useState('overview');
  const [selectedMetric, setSelectedMetric] = useState<SecurityMetric | null>(
    null
  );
  ...
  const translations = {
    en: {
      ...
      subtitle:
        'Never trust, always verify - Comprehensive security posture monitoring',
    ...
    el: {
      ...
      subtitle:
        'Ποτέ μην εμπιστεύεσαι, πάντα επαληθεύεις - Ολοκληρωμένη παρακολούθηση στάσης ασφαλείας',
  ...
  const overallSecurityScore = Math.round(
    SECURITY_METRICS.reduce((acc, metric) => acc + metric.value, 0) /
      SECURITY_METRICS.length
  );

  const activeThreatsCount = RECENT_THREATS.filter(
          ...
          <Card>
            <CardContent className="p-6">
              ...
              <p className="text-sm text-gray-500 mt-2">
                {RECENT_THREATS.length} total detected
              </p>
            </CardContent>
          </Card>
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      ...
                      <p className="text-sm text-gray-600">
                        {locale === 'en'
                          ? metric.description
                          : metric.descriptionEl}
                      </p>
                      <div className="text-xs text-gray-500">
                        Category:{' '}
                        {(t.categories as any)[metric.category] ||
                          metric.category}
                      </div>
                    </div>
                  </CardContent>
                    ...
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-2">
                          ...
                          <span
                            className={`font-medium ${getSeverityColor(threat.severity)}`}
                          >
                ...
                <CardHeader>
                  <div className="flex items-center justify-between">
                    ...
                    <Badge className={getStatusColor(control.status)}>
                      {(t as any)[control.status.replace('-', '')] ||
                        control.status}
                    </Badge>
                  </div>
