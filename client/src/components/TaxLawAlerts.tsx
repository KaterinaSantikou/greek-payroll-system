...
interface TaxLawChange {
  ...
  title: string;
  titleEl: string;
  category: 'income_tax' | 'social_security' | 'vat' | 'corporate_tax' | 'payroll_tax' | 'withholding_tax' | 'other';
  categoryEl: string;
  description: string;
...
const TAX_LAW_CHANGES: TaxLawChange[] = [
  {
    ...
    description: 'Adjustment of income tax rates and tax brackets effective January 1, 2025. The first tax bracket remains at 9%, while higher brackets see reductions.',
    descriptionEl: 'Προσαρμογή των συντελεστών φόρου εισοδήματος και των φορολογικών κλιμακίων με ισχύ από 1η Ιανουαρίου 2025. Η πρώτη κλίμακα παραμένει στο 9%, ενώ οι υψηλότερες κλίμακες μειώνονται.',
  ...
  },
  {
    ...
    description: 'Modification of social security contribution rates for employees and employers in the private sector, effective February 1, 2025.',
    descriptionEl: 'Τροποποίηση των συντελεστών εισφορών κοινωνικής ασφάλισης για εργαζομένους και εργοδότες στον ιδιωτικό τομέα, με ισχύ από 1η Φεβρουαρίου 2025.',
  ...
  },
  {
    ...
    category: 'vat',
    categoryEl: 'ΦΠΑ',
    description: 'Temporary reduction of VAT rates for hospitality services to support tourism recovery, effective March 1, 2025.',
    descriptionEl: 'Προσωρινή μείωση των συντελεστών ΦΠΑ για υπηρεσίες φιλοξενίας για την υποστήριξη της ανάκαμψης του τουρισμού, με ισχύ από 1η Μαρτίου 2025.',
  ...
  },
  {
    ...
    category: 'income_tax',
    categoryEl: 'Φόρος Εισοδήματος',
    description: 'Introduction of special tax regime for digital nomads and remote workers relocating to Greece, effective April 1, 2025.',
    descriptionEl: 'Εισαγωγή ειδικού φορολογικού καθεστώτος για ψηφιακούς νομάδες και εξ αποστάσεως εργαζομένους που μετεγκαθίστανται στην Ελλάδα, με ισχύ από 1η Απριλίου 2025.',
...
const TAX_ALERTS: TaxAlert[] = [
  {
    ...
    alertType: 'immediate',
    priority: 'urgent',
    message: 'URGENT: Income tax rates changing January 1, 2025. Immediate action required.',
    messageEl: 'ΕΠΕΙΓΟΝ: Αλλαγή συντελεστών φόρου εισοδήματος 1η Ιανουαρίου 2025. Απαιτείται άμεση ενέργεια.',
  ...
  },
  {
    ...
    alertType: 'reminder',
    priority: 'medium',
    message: 'Reminder: Social security rates changing February 1, 2025. Update your systems.',
    messageEl: 'Υπενθύμιση: Αλλαγή συντελεστών κοινωνικής ασφάλισης 1η Φεβρουαρίου 2025. Ενημερώστε τα συστήματά σας.',
...
const COMPLIANCE_TASKS: ComplianceTask[] = [
  {
    ...
    description: 'Configure new income tax rates in payroll system for January 2025',
    descriptionEl: 'Διαμόρφωση νέων συντελεστών φόρου εισοδήματος στο σύστημα μισθοδοσίας για Ιανουάριο 2025',
  ...
  },
  {
    ...
    descriptionEl: 'Αποστολή ειδοποίησης σε όλους τους εργαζομένους για τις αλλαγές φορολογικών συντελεστών',
    assignedTo: 'HR Team',
    dueDate: '2024-12-30',
    priority: 'high',
    status: 'pending',
...
export default function TaxLawAlerts({ locale = 'en' }: TaxLawAlertsProps) {
  ...
  const getCategoryIcon = (category: string) => {
    switch (category) {
      case 'income_tax': return Calculator;
      case 'social_security': return Users;
      case 'vat': return Receipt;
      case 'corporate_tax': return Building;
      case 'payroll_tax': return Euro;
      case 'withholding_tax': return Percent;
  ...
  const filteredChanges = changes.filter(change => {
    ...
    const impactMatch = selectedImpact === 'all' || change.impact === selectedImpact;
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 xl:grid-cols-8 gap-4 mb-6">
            <Card className="p-4 bg-gradient-to-r from-orange-500 to-orange-600 text-white">
              <div className="text-center">
                <div className="text-2xl font-bold">{TAX_METRICS.totalChanges}</div>
                <div className="text-xs text-orange-100">{t.metrics.totalChanges}</div>
              </div>
            </Card>

            <Card className="p-4 bg-gradient-to-r from-yellow-500 to-yellow-600 text-white">
              <div className="text-center">
                <div className="text-2xl font-bold">{TAX_METRICS.pendingChanges}</div>
                <div className="text-xs text-yellow-100">{t.metrics.pendingChanges}</div>
              </div>
            </Card>

            <Card className="p-4 bg-gradient-to-r from-red-500 to-red-600 text-white">
              <div className="text-center">
                <div className="text-2xl font-bold">{TAX_METRICS.criticalChanges}</div>
                <div className="text-xs text-red-100">{t.metrics.criticalChanges}</div>
              </div>
            </Card>

            <Card className="p-4 bg-gradient-to-r from-green-500 to-green-600 text-white">
              <div className="text-center">
                <div className="text-2xl font-bold">{TAX_METRICS.complianceRate}%</div>
                <div className="text-xs text-green-100">{t.metrics.complianceRate}</div>
              </div>
            </Card>

            <Card className="p-4 bg-gradient-to-r from-purple-500 to-purple-600 text-white">
              <div className="text-center">
                <div className="text-2xl font-bold">{TAX_METRICS.alertsSent}</div>
                <div className="text-xs text-purple-100">{t.metrics.alertsSent}</div>
              </div>
            </Card>

            <Card className="p-4 bg-gradient-to-r from-blue-500 to-blue-600 text-white">
              <div className="text-center">
                <div className="text-2xl font-bold">{TAX_METRICS.tasksCompleted}</div>
                <div className="text-xs text-blue-100">{t.metrics.tasksCompleted}</div>
              </div>
            </Card>

            <Card className="p-4 bg-gradient-to-r from-indigo-500 to-indigo-600 text-white">
              <div className="text-center">
                <div className="text-2xl font-bold">{TAX_METRICS.averageComplianceTime}d</div>
                <div className="text-xs text-indigo-100">{t.metrics.averageComplianceTime}</div>
              </div>
            </Card>

            <Card className="p-4 bg-gradient-to-r from-teal-500 to-teal-600 text-white">
              <div className="text-center">
                <div className="text-2xl font-bold">{TAX_METRICS.sectorsAffected}</div>
                <div className="text-xs text-teal-100">{t.metrics.sectorsAffected}</div>
              </div>
            </Card>
          </div>
        </div>
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      {changes.filter(change => change.impact === 'critical').map((change) => {
                        const CategoryIcon = getCategoryIcon(change.category);
                        
                        return (
                          <div key={change.id} className="flex items-center justify-between p-4 border border-red-200 rounded-lg bg-red-50">
                            <div className="flex items-center gap-4">
                              ...
                              <div>
                                ...
                                <div className="text-sm text-gray-600">
                                  Effective: {change.effectiveDate} • Affects {change.affectedEmployees.toLocaleString()} employees
                                </div>
                                {change.isUrgent && (
                                  <Badge className="bg-red-600 text-white text-xs mt-1">URGENT</Badge>
                                )}
                              </div>
                            </div>
              ...
              <Card>
                ...
                <CardContent className="space-y-3">
                  <div className="flex items-start gap-3 text-sm">
                    ...
                    <div>
                      <div className="font-medium">Income tax rates updated</div>
                      <div className="text-gray-600">Effective January 1, 2025</div>
                  ...
                  <div className="flex items-start gap-3 text-sm">
                    ...
                    <div>
                      ...
                      <div className="text-gray-600">e-EFKA contribution update</div>
                  ...
                  <div className="flex items-start gap-3 text-sm">
                    ...
                    <div>
                      ...
                      <div className="text-gray-600">Hospitality sector 13% VAT</div>
                  ...
                  <div className="flex items-start gap-3 text-sm">
                    ...
                    <div>
                      <div className="font-medium">Digital nomad incentives</div>
                      <div className="text-gray-600">New tax regime published</div>
              ...
              <CardContent>
                <div className="grid md:grid-cols-3 lg:grid-cols-6 gap-6">
                  {Object.entries(t.categories).map(([key, category]) => {
                    ...
                    const progress = [95, 87, 92, 89, 91, 94][Object.keys(t.categories).indexOf(key)] || 90;
                ...
                <div className="grid md:grid-cols-4 gap-4">
                  <Select value={selectedCategory} onValueChange={setSelectedCategory}>
                    ...
                    <SelectContent>
                      ...
                      {Object.entries(t.categories).map(([key, category]) => (
                        <SelectItem key={key} value={key}>{category}</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                  ...
                  <Select value={selectedImpact} onValueChange={setSelectedImpact}>
                    ...
                    <SelectContent>
                      ...
                      {Object.entries(t.impact).map(([key, impact]) => (
                        <SelectItem key={key} value={key}>{impact}</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                  ...
                  <Card key={change.id} className="hover:shadow-md transition-shadow">
                    <CardContent className="p-6">
                        ...
                        <div className="flex items-start gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-gray-600 text-sm mb-2">
                              {locale === 'en' ? change.description : change.descriptionEl}
                            </p>
                            <div className="flex items-center gap-4 text-sm text-gray-500">
                              <span>Effective: {change.effectiveDate}</span>
                              <span>Published: {change.publishedDate}</span>
                              <span>Source: {locale === 'en' ? change.source : change.sourceEl}</span>
                            </div>
                          </div>
                        </div>
                        <div className="flex items-center gap-3">
                          ...
                          <Badge className={`${getImpactColor(change.impact)} border-current`} variant="outline">
                      ...
                      <div className="grid md:grid-cols-2 gap-4 mb-4">
                        <div>
                          <div className="text-sm font-medium text-gray-700 mb-1">Affected Employees</div>
                          <div className="text-lg font-bold text-blue-600">{change.affectedEmployees.toLocaleString()}</div>
                        </div>

                        <div>
                          <div className="text-sm font-medium text-gray-700 mb-1">Category</div>
                          <div className="text-sm font-medium">
                            {locale === 'en' ? t.categories[change.category as keyof typeof t.categories] : change.categoryEl}
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="mb-4">
                        ...
                        <div className="space-y-1">
                          {(locale === 'en' ? change.keyChanges : change.keyChangesEl).slice(0, 3).map((keyChange, index) => (
                      ...
                      <div className="mb-4">
                        <div className="text-sm font-medium text-gray-700 mb-2">Compliance Actions Required</div>
                        <div className="flex flex-wrap gap-2">
                          {(locale === 'en' ? change.complianceActions : change.complianceActionsEl).slice(0, 2).map((action, index) => (
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4 text-sm text-gray-600">
                          <span>Sectors: {change.affectedSectors.join(', ')}</span>
                          {change.deadline && (
                      ...
                      <div key={alert.id} className="p-4 border rounded-lg">
                        <div className="flex items-start justify-between mb-3">
                          <div className="flex items-start gap-3">
                            <div className={`p-2 rounded-lg ${getPriorityColor(alert.priority)} bg-opacity-10`}>
                              <Bell className={`h-5 w-5 ${getPriorityColor(alert.priority)}`} />
                            </div>
                            <div>
                              ...
                              <p className="text-sm text-gray-600 mt-1">
                                {locale === 'en' ? alert.message : alert.messageEl}
                              </p>
                              <div className="flex items-center gap-4 text-xs text-gray-500 mt-2">
                                <span>Type: {t.alertTypes[alert.alertType as keyof typeof t.alertTypes]}</span>
                                <span>Scheduled: {alert.scheduledDate}</span>
                                {alert.sentDate && <span>Sent: {alert.sentDate}</span>}
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <Badge className={`${getPriorityColor(alert.priority)} border-current`} variant="outline">
                              {t.priority[alert.priority as keyof typeof t.priority]}
                            </Badge>
                          </div>
                        </div>
                        ...
                        {alert.actionRequired && (
                          <div className="mb-3 p-2 bg-yellow-50 rounded text-sm text-yellow-800 border border-yellow-200">
                            ...
                            Action required
                            {alert.actionDeadline && ` by ${alert.actionDeadline}`}
                          </div>
                        )}
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-4 text-sm text-gray-600">
                            {alert.readBy.length > 0 && (
                              <span>Read by: {alert.readBy.length} of {alert.recipients.length}</span>
                            )}
                          </div>
                          <div className="flex items-center gap-2">
                            <Button size="sm" variant="outline">
                      ...
                      <div key={task.id} className="p-4 border rounded-lg">
                        <div className="flex items-start justify-between mb-3">
                          <div>
                            ...
                            <p className="text-sm text-gray-600 mt-1">
                              {locale === 'en' ? task.description : task.descriptionEl}
                            </p>
                            <div className="flex items-center gap-4 text-xs text-gray-500 mt-2">
                              <span>Related: {locale === 'en' ? change?.title : change?.titleEl}</span>
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <Badge className={`${getPriorityColor(task.priority)} border-current`} variant="outline">
                    ...
                    {Object.entries(t.categories).map(([key, category]) => {
                      ...
                      return (
                        <div key={key} className="flex items-center gap-3">
                          ...
                          <div className="w-12 text-sm font-medium text-right">{progress}%</div>
                        </div>
                      );
                    })}
                  </div>
                ...
                <CardContent>
                  <div className="space-y-4">
                    <div className="flex justify-between">
                      <span className="text-sm text-gray-600">Tax law changes tracked</span>
                    ...
                    <div className="flex justify-between">
                      <span className="text-sm text-gray-600">Average compliance time</span>
                      <span className="text-sm font-medium">4.2 days</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-sm text-gray-600">Sectors affected</span>
              ...
              <CardContent className="space-y-6">
                <div>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      <div>
                        <label className="font-medium">Email alerts for critical changes</label>
                        <p className="text-sm text-gray-600">Immediate notification for urgent tax law changes</p>
                      </div>
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        <label className="font-medium">Weekly digest reports</label>
                        <p className="text-sm text-gray-600">Summary of all tax law changes and compliance status</p>
                      </div>
                      <Button variant="default" size="sm">Enabled</Button>
                    </div>
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-sm text-gray-600">Automatic reminders for compliance deadlines</p>
                      </div>
                      <Button variant="default" size="sm">Enabled</Button>
                    </div>
                  </div>
                ...
                <div>
                  <h3 className="text-lg font-medium mb-4">Monitoring Categories</h3>
