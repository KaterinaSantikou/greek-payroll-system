  ...
  const kioskActionMutation = useMutation({
    mutationFn: async (data: {
      ...
      const response = await fetch(
        `/api/hotel/kiosk/${data.departmentId}/action`,
        {
          method: 'POST',
    ...
    <div className="space-y-8">
      ...
      <Tabs defaultValue="multi-property" className="w-full">
        <TabsList className="grid w-full grid-cols-4">
          ...
          <TabsTrigger value="seasonal-onboarding">
            Seasonal Onboarding
          </TabsTrigger>
        ...
        <TabsContent value="multi-property" className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Manage employees across multiple hotel properties with rotation
                schedules
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
          ...
          <Card>
            ...
            <CardContent>
              {multiPropertyAssignments &&
              multiPropertyAssignments.length > 0 ? (
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="font-medium">
                            Employee {assignment.employeeId.slice(-6)}
                          </span>
                          <Badge variant="outline">
                            Primary: {assignment.primaryPropertyId}
                          </Badge>
                        </div>
                      ...
                      <div className="mt-2 text-sm text-gray-600">
                        ...
                        {assignment.secondaryProperties
                          .map((sp: any) => sp.propertyId)
                          .join(', ')}
                      </div>
                    </div>
                  ))}
                ...
                <Alert>
                  ...
                  <AlertDescription>
                    No multi-property assignments found. Create assignments for
                    employees working across multiple hotels.
                  </AlertDescription>
                </Alert>
              )}
            </CardContent>
          ...
          {kioskConfig && (
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  {selectedLanguage === 'GR'
                    ? 'Ρύθμιση Kiosk Τμήματος'
                    : 'Department Kiosk Setup'}
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                      <Badge
                        variant={
                          kioskConfig.deviceBinding ? 'default' : 'secondary'
                        }
                      >
                        Device Binding:{' '}
                        {kioskConfig.deviceBinding ? 'ON' : 'OFF'}
                      </Badge>
                      <Badge
                        variant={
                          kioskConfig.biometricRequired
                            ? 'default'
                            : 'secondary'
                        }
                      >
                        Biometric:{' '}
                        {kioskConfig.biometricRequired ? 'ON' : 'OFF'}
                      </Badge>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-2">
                      <div
                        ...
                        style={{
                          backgroundColor: kioskConfig.theme.primaryColor,
                        }}
                      />
                      <span className="text-sm">
                        {kioskConfig.theme.primaryColor}
                      </span>
                    </div>
                  </div>
                </div>
          ...
          {kioskConfig && (
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  {selectedLanguage === 'GR'
                    ? 'Γρήγορες Ενέργειες Kiosk'
                    : 'Icon-first kiosk interface actions'}
                </CardDescription>
              </CardHeader>
              <CardContent>
        ...
        <TabsContent value="seasonal-onboarding" className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Batch import employees, device provisioning, and geofence setup
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  <div>
                    <Label htmlFor="batch-data">
                      Batch Employee Data (JSON)
                    </Label>
                    <Textarea
                      id="batch-data"
                      placeholder={`[
    ...
    "Email": "john@example.com"
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    {properties
                      ?.find(p => p.propertyId === selectedProperty)
                      ?.seasonalPeriods.map(period => (
                        ...
                        >
                          <div className="flex items-center justify-between">
                            ...
                            <Badge
                              variant={
                                period.type === 'HIGH' ? 'default' : 'secondary'
                              }
                            >
                              {period.type}
                            </Badge>
              ...
              {seasonalOnboardingMutation.data && (
                <Alert className="border-green-200 bg-green-50">
                  <CheckCircle className="h-4 w-4 text-green-600" />
                  <AlertDescription className="text-green-800">
                    Seasonal onboarding started! Session ID:{' '}
                    {seasonalOnboardingMutation.data.onboardingSessionId?.slice(
                      -8
                    )}
                    <br />
                    ...
                    {
                      seasonalOnboardingMutation.data.estimatedCompletionMinutes
                    }{' '}
                    minutes
                  </AlertDescription>
                </Alert>
        ...
        <TabsContent value="split-shifts" className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Manage employees working across multiple departments and cost
                centers
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
                    ...
                    <div key={shift.shiftId} className="border rounded-lg p-4">
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="font-medium">
                            Employee {shift.employeeId.slice(-6)}
                          </span>
