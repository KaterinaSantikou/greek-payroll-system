  ...
  const bankProfiles = [
    {
      key: 'alpha',
      name: 'Alpha Bank',
      features: ['Dual PAIN format', 'Enhanced processing'],
    },
    {
      key: 'piraeus',
      name: 'Piraeus Bank',
      features: ['e-PPS Mass Payments', 'Encryption'],
    },
    {
      key: 'eurobank',
      name: 'Eurobank',
      features: ['Corporate XML Guide', 'Extended cut-off'],
    },
    {
      key: 'nbg',
      name: 'NBG',
      features: ['Bulk file management', 'SEPA Instant'],
    },
  ];
    ...
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-muted-foreground">
            Comprehensive pain.001 schema validation, cut-off enforcement, and
            SCT Instant switching
          </p>
        </div>
      </div>
            ...
            {mockPayments.map((payment, index) => (
              ...
              >
                <div className="flex items-center gap-4">
                  <Badge
                    variant={
                      payment.paymentType === 'URGENT'
                        ? 'destructive'
                        : payment.paymentType === 'CORRECTION'
                          ? 'default'
                          : 'secondary'
                    }
                  >
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Validates pain.001 schema, bank-specific rules, cut-off windows, and
            SCT Instant switching
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <Button
            onClick={() =>
              validateEngine.mutate({
                bankProfile: selectedBank,
                ...
                <div className="space-y-2">
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm">
                        Processing Mode:{' '}
                        {validationResult.validation.processingMode}
                      </span>
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm">
                        Correlation ID:{' '}
                        {validationResult.validation.correlationId}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="space-y-1">
                    <p className="text-sm">
                      ...
                      {new Date(
                        validationResult.validation.executionDate
                      ).toLocaleString()}
                    </p>
                    <p className="text-sm">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Process status reports and surface payment rejects with actionable
            insights
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <Button
            onClick={() =>
              processStatus.mutate({
                ...
                correlationId:
                  validationResult?.validation.correlationId || 'DEMO-12345',
              })
            }
            ...
            variant="outline"
          >
            {processStatus.isPending
              ? 'Processing...'
              : 'Process Mock pain.002 Status'}
            <ArrowRight className="w-4 h-4 ml-2" />
          </Button>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  {statusResult.rejectAnalysis.acceptedPayments.map(
                    (paymentId: string) => (
                      <div
                        key={paymentId}
                ...
                <div className="space-y-2">
                  ...
                  {statusResult.rejectAnalysis.rejectedPayments.map(
                    (reject: any) => (
                      <div
                        key={reject.paymentId}
                        className="p-3 bg-red-50 dark:bg-red-900/20 rounded space-y-1"
                      >
                        <div className="flex items-center gap-2">
                          <AlertTriangle className="w-4 h-4 text-red-600" />
