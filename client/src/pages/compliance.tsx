...
export default function Compliance() {
  ...
  const { data: dwcDashboard, isLoading: dashboardLoading } = useQuery({
    queryKey: [
      '/api/digital-work-card/dashboard',
      selectedProperty,
      selectedPeriod,
    ],
  });
  ...
  const generateAPDMutation = useMutation({
    mutationFn: async ({
      ...
      return apiRequest('POST', '/api/filings/apd/generate', {
        propertyId,
        period,
      });
    },
  ...
  const generateFMYMutation = useMutation({
    mutationFn: async ({
      ...
      return apiRequest('POST', '/api/filings/fmy/generate', {
        propertyId,
        period,
      });
    },
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-300">
            APD (e-EFKA) generator, ΦΜΥ (AADE) filing, ERGANI II forms, Digital
            Work Card dashboards
          </p>
        </div>
        <div className="flex items-center space-x-4">
          <Select value={selectedProperty} onValueChange={setSelectedProperty}>
            ...
            <SelectContent>
              ...
              <SelectItem value="PROP_003">
                Olympus Hotel Thessaloniki
              </SelectItem>
            </SelectContent>
          </Select>
          <Input
        ...
        <TabsContent value="dashboard" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Coverage, submission success, and inspector readiness for period{' '}
                {selectedPeriod}
              </CardDescription>
            </CardHeader>
            <CardContent>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <Card>
                    ...
                    <CardContent>
                      <div className="text-2xl font-bold">
                        {dwcDashboard.coverageMetrics.coveragePercentage.toFixed(
                          1
                        )}
                        %
                      </div>
                      <p className="text-xs text-muted-foreground">
                        {dwcDashboard.coverageMetrics.employeesWithCards} of{' '}
                        {dwcDashboard.totalEmployees} employees
                      </p>
                      <div className="mt-2 space-y-1">
                        <div className="text-xs">
                          <span className="text-yellow-600">
                            Pending:{' '}
                            {dwcDashboard.coverageMetrics.pendingActivations}
                          </span>
                        </div>
                        <div className="text-xs">
                          <span className="text-red-600">
                            Expired: {dwcDashboard.coverageMetrics.expiredCards}
                          </span>
                        </div>
                      </div>
                    </CardContent>
                  ...
                  <Card>
                    ...
                    <CardContent>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {dwcDashboard.submissionMetrics.successfulSubmissions}{' '}
                        successful submissions
                      </p>
                      <div className="mt-2">
                        <div className="text-xs text-red-600">
                          Failed:{' '}
                          {dwcDashboard.submissionMetrics.failedSubmissions}
                        </div>
                      </div>
                    </CardContent>
                  </Card>
                  ...
                  <Card>
                    ...
                    <CardContent>
                      <div className="text-2xl font-bold mb-2">
                        {getInspectorReadinessBadge(
                          dwcDashboard.complianceStatus.inspectorReadiness
                        )}
                      </div>
                      <p className="text-xs text-muted-foreground">
                        {dwcDashboard.complianceStatus.compliantEmployees}{' '}
                        compliant employees
                      </p>
                      <div className="mt-2">
                        <div className="text-xs text-red-600">
                          Violations:{' '}
                          {dwcDashboard.complianceStatus.violationCount}
                        </div>
                      </div>
                    </CardContent>
                  </Card>
        ...
        <TabsContent value="apd-efka" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Generate and submit monthly APD filings for employee and
        ...
        <TabsContent value="fmy-aade" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Generate monthly ΦΜΥ filings for employee tax withholding and
                submission to AADE
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
        ...
        <TabsContent value="ergani" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Create and submit ERGANI II forms for hire, schedule, overtime,
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={value =>
                      setErganiFormData(prev => ({
                        ...prev,
                        formType: value as any,
                      }))
                    }
                  >
                    <SelectTrigger>
              ...
              <Button
                ...
                disabled={
                  createERGANIFormMutation.isPending ||
                  !erganiFormData.employeeId ||
                  !erganiFormData.afm
                }
                className="flex items-center space-x-2"
              >
                  ...
                  <TableBody>
                    {filingHistory.map((filing: any) => (
                      <TableRow key={filing.filingId}>
                        ...
                        <TableCell>
                          {format(new Date(filing.createdAt), 'PPp')}
                        </TableCell>
                        <TableCell>
