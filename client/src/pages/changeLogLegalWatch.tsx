...
export default function ChangeLogLegalWatch() {
  const [selectedVersion, setSelectedVersion] = useState<RulePackVersion | null>(null);
  const [selectedAlert, setSelectedAlert] = useState<LegalAlert | null>(null);
  const [diffViewOpen, setDiffViewOpen] = useState(false);
  const [deployModalOpen, setDeployModalOpen] = useState(false);
  const [compareVersions, setCompareVersions] = useState<{ source: string; target: string }>({ source: '', target: '' });
  ...
  const { data: comparison } = useQuery({
    queryKey: ['/api/changelog/compare', compareVersions.source, compareVersions.target],
    enabled: !!compareVersions.source && !!compareVersions.target && compareVersions.source !== compareVersions.target
  });

  // Mutations
  const updateRulePackStatusMutation = useMutation({
    mutationFn: async (data: { id: string; status: string; legalApproval?: any }) =>
      apiRequest('/api/changelog/rule-packs/status', 'PUT', data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/changelog/rule-packs'] });
      queryClient.invalidateQueries({ queryKey: ['/api/changelog/dashboard'] });
      toast({ title: "Success", description: "Rule pack status updated successfully" });
    }
  });

  const deployToTenantMutation = useMutation({
    mutationFn: async (data: { tenantId: string; rulePackVersionId: string; deployedBy: string; notes: string }) =>
      apiRequest('/api/changelog/deploy', 'POST', data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/changelog/tenants'] });
      setDeployModalOpen(false);
      toast({ title: "Success", description: "Deployment initiated successfully" });
    }
  });
  ...
  const updateLegalAlertMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/changelog/legal-alerts'] });
      toast({ title: "Success", description: "Legal alert updated successfully" });
    }
  });

  const simulateLegalScanMutation = useMutation({
    mutationFn: async () => apiRequest('/api/changelog/simulate-legal-scan', 'POST', {}),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/changelog/legal-alerts'] });
      toast({ title: "Success", description: "Legal monitoring scan completed" });
    }
  });
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
      <Card>
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle className="text-sm font-medium">Rule Pack Versions</CardTitle>
          <FileText className="h-4 w-4 text-muted-foreground" />
        </CardHeader>
        <CardContent>
          <div className="text-2xl font-bold">{dashboard?.data?.rulePackSummary?.total || 0}</div>
          <p className="text-xs text-muted-foreground">
            {dashboard?.data?.rulePackSummary?.pendingApproval || 0} pending approval
          </p>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent>
          <div className="text-2xl font-bold">{dashboard?.data?.deploymentSummary?.totalDeployments || 0}</div>
          <p className="text-xs text-muted-foreground">
            {dashboard?.data?.deploymentSummary?.tenantsCovered || 0} tenants covered
          </p>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent>
          <div className="text-2xl font-bold">{dashboard?.data?.alertsSummary?.totalAlerts || 0}</div>
          <p className="text-xs text-muted-foreground">
      ...
      <Card>
        ...
        <CardContent>
          <div className="text-2xl font-bold">{dashboard?.data?.complianceStatus?.coverageScore || 0}%</div>
          <Progress 
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
                <div>
                  ...
                  <p className="text-sm">
                    {new Date(rulePack.effectiveDate).toLocaleDateString('en-GB')}
                  </p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="text-sm">
                    {(rulePack.diffSummary?.added || 0) + (rulePack.diffSummary?.modified || 0) + (rulePack.diffSummary?.removed || 0)} total
                  </p>
                </div>
              </div>
              ...
              {rulePack.legalApproval && (
                <div className="bg-green-50 p-4 rounded-lg mb-4">
                  <h4 className="font-medium text-green-900 mb-2">Legal Approval</h4>
                  <div className="space-y-2 text-sm text-green-800">
                    <p><strong>Approved by:</strong> {rulePack.legalApproval.approvedBy}</p>
                    <p><strong>Date:</strong> {new Date(rulePack.legalApproval.approvedDate).toLocaleDateString('en-GB')}</p>
                    <p><strong>Notes:</strong> {rulePack.legalApproval.approvalNotes}</p>
                  </div>
                </div>
              )}
              ...
              <div className="flex gap-2">
                ...
                {rulePack.status === 'review' && (
                  <Button size="sm" onClick={() => handleApproveRulePack(rulePack.id)}>
                    <CheckCircle className="mr-2 h-4 w-4" />
                    Approve
                  </Button>
                )}
                {rulePack.status === 'approved' && (
                  <Button size="sm" onClick={() => handlePublishRulePack(rulePack.id)}>
                    <Send className="mr-2 h-4 w-4" />
                    Publish
                  </Button>
                )}
                {(rulePack.status === 'approved' || rulePack.status === 'published') && (
                  <Button 
                    variant="outline" 
                    size="sm"
                    onClick={() => {
                ...
                <Button variant="ghost" size="sm" onClick={() => setSelectedVersion(rulePack)}>
                  <Eye className="mr-2 h-4 w-4" />
                  View Details
                </Button>
              </div>
  ...
  const LegalAlerts = () => (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        >
          <RefreshCcw className={`mr-2 h-4 w-4 ${simulateLegalScanMutation.isPending ? 'animate-spin' : ''}`} />
          Run Legal Scan
        </Button>
      </div>
        ...
        {legalAlerts?.data?.map((alert: LegalAlert) => (
          <Card key={alert.id}>
              ...
              <div className="flex justify-between items-start">
                <div className="flex-1">
                  <CardTitle className="text-lg flex items-center gap-2">
                    {alert.urgency === 'urgent' && <AlertTriangle className="h-5 w-5 text-red-500" />}
                    {alert.urgency === 'high' && <AlertCircle className="h-5 w-5 text-orange-500" />}
                    {alert.title}
                  </CardTitle>
            ...
            <CardContent>
              ...
              <div className="mb-4">
                <Label className="text-sm text-muted-foreground">Recommended Actions</Label>
              ...
              <div className="flex gap-2">
                {alert.status === 'new' && (
                  <>
                    <Button size="sm" onClick={() => handleUpdateAlert(alert.id, 'reviewed')}>
                      <Eye className="mr-2 h-4 w-4" />
                      Mark Reviewed
                    </Button>
                    <Button size="sm" onClick={() => handleUpdateAlert(alert.id, 'actioned')}>
                      <CheckCircle className="mr-2 h-4 w-4" />
                      Take Action
                    </Button>
                  </>
        ...
        {(!legalAlerts?.data || legalAlerts.data.length === 0) && (
          <Card>
            <CardContent className="p-8 text-center">
              ...
              <p className="text-muted-foreground">No legal alerts at this time</p>
        ...
        {legalWatches?.data?.map((watch: any) => (
          <Card key={watch.id}>
            <CardHeader>
              ...
              <CardDescription>{watch.category.replace('_', ' ')}</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1 mt-1">
                    {watch.keywords.slice(0, 3).map((keyword: string, index: number) => (
                ...
                {watch.lastAlert && (
                  <div className="bg-blue-50 p-3 rounded-lg">
                    ...
                    <p className="text-xs text-blue-600 mt-1">
                      {new Date(watch.lastAlert.date).toLocaleDateString('en-GB')}
                    </p>
                  </div>
                )}
              </div>
    ...
    <Dialog open={deployModalOpen} onOpenChange={setDeployModalOpen}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Deploy {selectedVersion?.packName} v{selectedVersion?.version} to selected tenants
          </DialogDescription>
        </DialogHeader>
              ...
              <Card key={tenant.id}>
                <CardContent className="p-4">
                  <div className="flex justify-between items-start">
                    ...
                    <Button
                      size="sm"
                      onClick={() => deployToTenantMutation.mutate({
                        tenantId: tenant.id,
          ...
          {comparison?.data && (
            <div className="space-y-4">
                ...
                <CardContent>
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm">
                      ...
                      <Badge className={
                        comparison.data.summary.riskLevel === 'critical' ? 'bg-red-100 text-red-800' :
                        comparison.data.summary.riskLevel === 'high' ? 'bg-orange-100 text-orange-800' :
                        comparison.data.summary.riskLevel === 'medium' ? 'bg-yellow-100 text-yellow-800' :
              ...
              <ScrollArea className="h-64">
                <div className="space-y-2">
                  {comparison.data.differences.added.map((change: any, index: number) => (
                    <div key={index} className="p-3 bg-green-50 rounded border-l-4 border-green-400">
                      <p className="text-sm font-medium text-green-800">
                        + Added: {change.data.name}
                      </p>
                      <p className="text-xs text-green-600">{change.data.description}</p>
                    </div>
                  ))}
                  
                  {comparison.data.differences.modified.map((change: any, index: number) => (
                    <div key={index} className="p-3 bg-blue-50 rounded border-l-4 border-blue-400">
                      <p className="text-sm font-medium text-blue-800">
                        ~ Modified: {change.after.name}
                      </p>
                      <p className="text-xs text-blue-600">{change.after.description}</p>
                    </div>
                  ))}
                  
                  {comparison.data.differences.removed.map((change: any, index: number) => (
                    <div key={index} className="p-3 bg-red-50 rounded border-l-4 border-red-400">
                      <p className="text-sm font-medium text-red-800">
                        - Removed: {change.data.name}
                      </p>
                      <p className="text-xs text-red-600">{change.data.description}</p>
                    </div>
                  ))}
                </div>
              </ScrollArea>
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-8">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Change Log & Legal Watch</h1>
          <p className="text-lg text-gray-600 mt-2">
            Versioned rule packs with legal compliance monitoring and tenant deployment
          </p>
        </div>
        <div className="flex gap-2">
