...
export default function ABTestingDashboard() {
  ...
  React.useEffect(() => {
    if (tests.length === 0 && !isLoading) {
      ...
      const exitIntentTest = createTest({
        ...
        description:
          'Test different button colors for exit intent popup conversion',
        status: 'running',
  ...
  const totalImpressions = tests.reduce(
    (sum, test) =>
      sum +
      test.variants.reduce(
        (varSum, variant) => varSum + variant.metrics.impressions,
        0
      ),
    0
  );
  const totalConversions = tests.reduce(
    (sum, test) =>
      sum +
      test.variants.reduce(
        (varSum, variant) => varSum + variant.metrics.conversions,
        0
      ),
    0
  );
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-lg text-gray-600 dark:text-gray-400">
              Optimize conversion rates systematically with data-driven
              experiments.
            </p>
          </div>
            ...
            <DialogContent className="max-w-2xl">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    >
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="email_signup">
                          Email Signup
                        </SelectItem>
                        <SelectItem value="trial_signup">
                          Trial Signup
                        </SelectItem>
                  ...
                  <div>
                    <Label htmlFor="trafficAllocation">
                      Traffic Allocation (%)
                    </Label>
                    <Input
                        ...
                        {results && (
                          <div className="border-t pt-4">
                            ...
                            <div className="flex justify-between text-xs text-gray-500 mt-1">
                              ...
                              <span>
                                Target:{' '}
                                {(test.confidenceLevel * 100).toFixed(0)}%
                              </span>
                            </div>
                          </div>
                        )}
                        ...
                        {results.isStatisticallySignificant ? (
                          <Alert>
                            ...
                            <AlertDescription>
                              ...
                              {results.winner &&
                                test.variants.find(v => v.id === results.winner)
                                  ?.name}{' '}
                              performed{' '}
                              {Math.abs(results.liftPercentage).toFixed(1)}%
                              better ({results.confidence.toFixed(1)}%
                              confidence).
                            </AlertDescription>
                          </Alert>
                        ) : (
                          <Alert>
                            ...
                            <AlertDescription>
                              ...
                              Consider running longer or testing a bigger
                              change.
                            </AlertDescription>
                          </Alert>
                        )}
                          ...
                          {results.results.map(result => (
                            ...
                            >
                              ...
                              <div className="grid grid-cols-2 gap-4 text-sm">
                                ...
                                <div>
                                  <div className="font-medium">
                                    {result.confidenceInterval.lower.toFixed(1)}
                                    % -{' '}
                                    {result.confidenceInterval.upper.toFixed(1)}
                                    %
                                  </div>
                                  <div className="text-xs text-gray-500">
        ...
        <TabsContent value="results" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Deep dive into your test results and statistical significance
              </CardDescription>
            </CardHeader>
            <CardContent>
              {Object.values(testResults).length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-sm text-gray-500">
                    Results will appear here once tests start collecting data
                  </p>
                </div>
              ) : (
                      ...
                      <div key={testId} className="border rounded-lg p-4">
                        ...
                        <div className="bg-gray-50 dark:bg-gray-800 rounded p-3 text-sm">
                          ...
                          {results.recommendedAction === 'stop_winner' &&
                            'Stop the test and implement the winning variant.'}
                          {results.recommendedAction === 'stop_no_winner' &&
                            'Stop the test - no significant difference found.'}
