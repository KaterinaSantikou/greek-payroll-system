      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">
            Cost & Overtime Analytics
          </h1>
          <p className="text-gray-600 dark:text-gray-400">
            Trends, heatmaps, department drill-downs
          </p>
        </div>
            ...
            {departmentCosts.map((dept, index) => (
              <div key={index} className="p-4 border rounded-lg">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <Badge
                    variant={dept.variance > 0 ? 'destructive' : 'default'}
                  >
                    {dept.variance > 0 ? '+' : ''}
                    {dept.variance}%
                  </Badge>
                ...
                <div className="grid grid-cols-5 gap-4 text-sm">
                  <div>
                    ...
                    <p className="font-medium">
                      €{dept.budgetEur.toLocaleString()}
                    </p>
                  </div>
                  <div>
                    <p className="text-gray-600 dark:text-gray-400">Actual</p>
                    <p className="font-medium">
                      €{dept.actualEur.toLocaleString()}
                    </p>
                  </div>
                  <div>
                    <p className="text-gray-600 dark:text-gray-400">Variance</p>
                    <p
                      className={`font-medium ${getVarianceColor(dept.variance)}`}
                    >
                      €
                      {Math.abs(
                        dept.actualEur - dept.budgetEur
                      ).toLocaleString()}
                    </p>
                  </div>
                  <div>
            ...
            {heatmapData.map((day, index) => (
              <div key={index} className="grid grid-cols-4 gap-2">
                ...
                <div className="text-center py-2">
                  <div
                    className={`w-full h-8 rounded flex items-center justify-center text-white text-sm ${getEfficiencyColor(day.shift1)}`}
                  >
                    {day.shift1}%
                  </div>
                </div>
                <div className="text-center py-2">
                  <div
                    className={`w-full h-8 rounded flex items-center justify-center text-white text-sm ${getEfficiencyColor(day.shift2)}`}
                  >
                    {day.shift2}%
                  </div>
                </div>
                <div className="text-center py-2">
                  <div
                    className={`w-full h-8 rounded flex items-center justify-center text-white text-sm ${getEfficiencyColor(day.shift3)}`}
                  >
                    {day.shift3}%
                  </div>
                </div>
          ...
          <div className="space-y-3">
            <div className="flex items-start gap-3 p-3 bg-orange-50 dark:bg-orange-950/20 rounded-lg">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600 dark:text-gray-400">
                  78 hours overtime (7% over budget). Consider adding 1
            ...
            <div className="flex items-start gap-3 p-3 bg-green-50 dark:bg-green-950/20 rounded-lg">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600 dark:text-gray-400">
                  3.4% under budget with minimal overtime. Efficient scheduling
                  maintained.
                </p>
              </div>
            </div>
            ...
            <div className="flex items-start gap-3 p-3 bg-blue-50 dark:bg-blue-950/20 rounded-lg">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600 dark:text-gray-400">
                  Friday-Saturday show 95%+ efficiency. Consider similar
                  patterns for other days.
                </p>
              </div>
            </div>
          </div>
