      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-2">
            Human-readable YAML/JSON DSL for Greek payroll compliance rules with
        ...
        <TabsContent value="overview" className="space-y-6">
            ...
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground">
                  Across {rulesData?.categories?.length || 0} categories
                </p>
              </CardContent>
            </Card>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Comprehensive DSL for Greek payroll compliance with versioning
                and effective date management
              </CardDescription>
            </CardHeader>
        ...
        <TabsContent value="create" className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Write rules in human-readable YAML format with versioning and
                effective dates
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="flex items-center gap-2 mb-4">
                ...
                <Select onValueChange={loadTemplate}>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="time_bands">
                      Time Bands & Night Hours
                    </SelectItem>
                    <SelectItem value="ergani_routing">
                      ERGANI Routing
                    </SelectItem>
              ...
              {parseRuleMutation.data && (
                <Alert>
                  ...
                  <AlertDescription>
                    Rule "{parseRuleMutation.data.rule}" v
                    {parseRuleMutation.data.version} created successfully
                  </AlertDescription>
                </Alert>
              )}
            </CardContent>
                ...
                <Card className="mt-4">
                  ...
                  <CardContent className="space-y-3">
                    ...
                    <div className="text-sm text-muted-foreground">
                      <p>
                        Conditions Met:{' '}
                        {testRuleMutation.data.summary.conditionsMet
                          ? 'Yes'
                          : 'No'}
                      </p>
                      <p>
                        Actions Triggered:{' '}
                        {testRuleMutation.data.summary.actionsTriggered}
                      </p>
                      <p>
                        Violations: {testRuleMutation.data.summary.violations}
                      </p>
                      <p>
                        Calculations:{' '}
                        {testRuleMutation.data.summary.calculations}
                      </p>
                    </div>
                    ...
                    {testRuleMutation.data.results?.length > 0 && (
                      <div className="space-y-2">
                        ...
                        {testRuleMutation.data.results.map(
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Validate employee salary against Greek minimum wage
                  requirements
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                ...
                >
                  <Calculator className="h-4 w-4" />
                  {validateWageMutation.isPending
                    ? 'Validating...'
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Input type="time" defaultValue="22:00" id="shift-start" />
                  </div>

                  <div className="space-y-2">
                    <Label>Shift End</Label>
                    <Input type="time" defaultValue="06:00" id="shift-end" />
                  </div>
                </div>

                <div className="space-y-2">
                  <Label>Hourly Rate (€)</Label>
                  <Input type="number" placeholder="5.50" id="hourly-rate" />
                </div>
                ...
                <Button
                  ...
                  onClick={() => {
                    const shiftStart = (
                      document.getElementById('shift-start') as HTMLInputElement
                    )?.value;
                    const shiftEnd = (
                      document.getElementById('shift-end') as HTMLInputElement
                    )?.value;
                    ...
                    if (!shiftStart || !shiftEnd || !hourlyRate) {
                      toast({
                        title: 'Error',
                        description:
                          'Please provide shift times and hourly rate',
                        variant: 'destructive',
                      });
                      return;
                    }
              ...
              <CardContent className="space-y-3">
                <div className="text-sm">
                  <p>
                    ...
                    {validateWageMutation.data.employeeId}
                  </p>
                  <p>
                    <strong>Base Salary:</strong> €
                    {validateWageMutation.data.baseSalary}
                  </p>
                  <p>
                    <strong>Valid:</strong>{' '}
                    {validateWageMutation.data.isValid ? 'Yes' : 'No'}
                  </p>
                </div>
                ...
                {validateWageMutation.data.violations?.length > 0 && (
                  <div className="space-y-2">
                    ...
                    {validateWageMutation.data.violations.map(
                      (violation: ValidationResult, index: number) => (
                        <Alert key={index} variant="destructive">
                          <AlertCircle className="h-4 w-4" />
                          <AlertDescription>
                            <strong>
                              {violation.rule} v{violation.version}:
                            </strong>{' '}
                            {violation.message}
                          </AlertDescription>
                        </Alert>
                ...
                {validateWageMutation.data.alerts?.length > 0 && (
                  <div className="space-y-2">
                    ...
                    {validateWageMutation.data.alerts.map(
                      (alert: ValidationResult, index: number) => (
                        <Alert key={index}>
                          <AlertCircle className="h-4 w-4" />
                          <AlertDescription>
                            <strong>
                              {alert.rule} v{alert.version}:
                            </strong>{' '}
                            {alert.message}
                          </AlertDescription>
                        </Alert>
        ...
        <TabsContent value="browse" className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                View all currently active payroll compliance rules with their
                definitions
              </CardDescription>
            </CardHeader>
            <CardContent>
                    ...
                    >
                      ...
                      <CardContent className="space-y-3">
                        <div className="text-sm space-y-1">
                          ...
                          <p>
                            <strong>Effective:</strong> {rule.effective_from}{' '}
                          ...
                          {rule.rate && (
                            <p>
                              <strong>Rate:</strong> {rule.rate}x
                            </p>
                          )}
                          {rule.formula && (
                            <p>
