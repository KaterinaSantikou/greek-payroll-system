...
export default function EmployeeSelfServicePage() {
  ...
  const { data: correctionRequests = [], isLoading: correctionsLoading } =
    useQuery({
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="space-y-4">
          {((paycheckTimeline as any[]) || []).map(
            (paycheck: PaycheckTimelineItem, index: number) => (
              <Card key={paycheck.paycheckId} className="relative">
                <CardHeader className="pb-3">
                  <div className="flex justify-between items-start">
                    <div>
                      <CardTitle className="text-base">
                        {format(parseISO(paycheck.payPeriodStart), 'MMM dd')} -{' '}
                        {format(
                          parseISO(paycheck.payPeriodEnd),
                          'MMM dd, yyyy'
                        )}
                      </CardTitle>
                      <CardDescription>
                        Paid on{' '}
                        {format(parseISO(paycheck.payDate), 'MMM dd, yyyy')}
                      </CardDescription>
                    </div>
                    <Badge
                      variant={
                        paycheck.status === 'paid' ? 'default' : 'secondary'
                      }
                    >
                      {paycheck.status}
                    </Badge>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-lg font-medium text-red-600">
                        -€{paycheck.taxWithheld.toFixed(2)}
                      </p>
                    </div>
                    <div>
                      ...
                      <p className="text-lg font-medium text-red-600">
                        -€{paycheck.efkaContributions.toFixed(2)}
                      </p>
                    </div>
                    <div>
            ...
            <TableBody>
              {(workCardLogs as any[]).map((log: WorkCardLog) => (
                <TableRow key={log.logId}>
                  <TableCell>
                    {format(parseISO(log.workDate), 'MMM dd, yyyy')}
                  </TableCell>
                  <TableCell>
                    {log.clockInTime
                      ? format(parseISO(log.clockInTime), 'HH:mm')
                      : '-'}
                  </TableCell>
                  <TableCell>
                    {log.clockOutTime
                      ? format(parseISO(log.clockOutTime), 'HH:mm')
                      : '-'}
                  </TableCell>
                  ...
                  <TableCell>
                    <Badge
                      variant={
                        log.erganiSyncStatus === 'synced'
                          ? 'default'
                          : log.erganiSyncStatus === 'failed'
                            ? 'destructive'
                            : 'secondary'
                      }
                    >
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="mt-2 text-gray-600">
            Manage your paycheck timeline, work history, and time corrections
          </p>
        </div>
                ...
                <div className="flex items-center">
                  ...
                  <div className="ml-4">
                    ...
                    <p className="text-2xl font-bold text-gray-900">
                      €{(dashboard as any)?.currentMonthGross || '0.00'}
                    </p>
                  </div>
                </div>
              </CardContent>
              ...
              <CardContent className="p-6">
                <div className="flex items-center">
                  ...
                  <div className="ml-4">
                    ...
                    <p className="text-2xl font-bold text-gray-900">
                      {(dashboard as any)?.hoursThisMonth || '0'}h
                    </p>
                  </div>
                </div>
              </CardContent>
              ...
              <CardContent className="p-6">
                <div className="flex items-center">
                  ...
                  <div className="ml-4">
                    ...
                    <p className="text-2xl font-bold text-gray-900">
                      {(dashboard as any)?.overtimeHours || '0'}h
                    </p>
                  </div>
                </div>
              </CardContent>
              ...
              <CardContent className="p-6">
                <div className="flex items-center">
                  ...
                  <div className="ml-4">
                    ...
                    <p className="text-2xl font-bold text-gray-900">
                      {(dashboard as any)?.pendingRequests || '0'}
                    </p>
                  </div>
                </div>
              </CardContent>
          ...
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              ...
              <DialogDescription>
                ...
                {format(
                  parseISO(
                    correctionDialog.workDate || new Date().toISOString()
                  ),
                  'MMM dd, yyyy'
                )}
              </DialogDescription>
            ...
            <div className="space-y-4">
              <div>
                ...
                <Select
                  ...
                  onValueChange={value =>
                    setCorrectionDialog(prev => ({ ...prev, type: value }))
                  }
                >
                  <SelectTrigger>
            ...
            <div className="flex justify-end space-x-2 mt-6">
              <Button
                variant="outline"
                onClick={() =>
                  setCorrectionDialog(prev => ({ ...prev, open: false }))
                }
              >
                Cancel
              </Button>
