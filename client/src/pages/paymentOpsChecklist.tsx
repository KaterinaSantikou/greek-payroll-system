...
export default function PaymentOpsChecklist() {
  ...
  const [steps, setSteps] = useState<PaymentOpsStep[]>([
    {
      ...
      description:
        'Choose entity and configure debtor IBAN with preferred bank',
      status: 'pending',
  ...
  const mockPayrollRuns = [
    ...
    {
      id: 'PR-2025-01-BONUS',
      name: 'January Bonus Run',
      employees: 12,
      amount: 18500.0,
    },
    {
      id: 'PR-2025-01-CORRECTION',
      name: 'Overtime Corrections',
      employees: 8,
      amount: 2150.0,
    },
  ];
  ...
  const bankChannels = {
    alpha: {
      name: 'Alpha Bank',
      channels: ['host-to-host', 'portal'],
      cutoff: '14:00',
    },
    piraeus: {
      name: 'Piraeus Bank',
      channels: ['bib', 'host-to-host'],
      cutoff: '13:30',
    },
    eurobank: {
      name: 'Eurobank',
  ...
  const uploadBank = useMutation({
    mutationFn: async () => {
      const entity = mockEntities.find(e => e.id === selectedEntity);
      const bank = entity
        ? bankChannels[entity.preferredBank as keyof typeof bankChannels]
        : null;
    ...
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-muted-foreground">
            Complete SEPA payment workflow from bank profile selection to
            reconciliation
          </p>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {selectedEntity && (
                <div className="text-xs space-y-1">
                  {(() => {
                    ...
                    const bank = entity
                      ? bankChannels[
                          entity.preferredBank as keyof typeof bankChannels
                        ]
                      : null;
                    return entity ? (
                      <>
                        <p>
                          <strong>Debtor IBAN:</strong> {entity.debtorIban}
                        </p>
                        <p>
                          <strong>Preferred Bank:</strong> {bank?.name}
                        </p>
                        <p>
                          <strong>Upload Channel:</strong>{' '}
                          {entity.uploadChannel}
                        </p>
                        <p>
                          <strong>Cut-off:</strong> {bank?.cutoff}
                        </p>
                      </>
                    ) : null;
                  })()}
            ...
            <div className="space-y-2">
              ...
              {selectedPayrollRun && (
                <div className="text-xs space-y-1">
                  {(() => {
                    const run = mockPayrollRuns.find(
                      r => r.id === selectedPayrollRun
                    );
                    return run ? (
                      <>
                        <p>
                          <strong>Employees:</strong> {run.employees}
                        </p>
                        <p>
                ...
                {step.details && (
                  <div className="bg-gray-50 dark:bg-gray-800 p-3 rounded text-xs space-y-1">
                    {step.id === 'select-profile' && step.details.entity && (
                      <>
                        <p>
                          <strong>Selected:</strong> {step.details.entity.name}
                        </p>
                        <p>
                          <strong>Bank:</strong>{' '}
                          {
                            bankChannels[
                              step.details.entity
                                .preferredBank as keyof typeof bankChannels
                            ]?.name
                          }
                        </p>
                      </>
                    ...
                    {step.id === 'generate-pain001' && (
                      <>
                        <p>
                          <strong>Category Purpose:</strong> SALA
                        </p>
                        <p>
                          <strong>Batch Booking:</strong> false
                        </p>
                        <p>
                          <strong>Format:</strong> pain.001.001.03
                        </p>
                      </>
                    )}
                    {step.id === 'upload-bank' && (
                      <>
                        <p>
                          <strong>Channel:</strong> {step.details.channel}
                        </p>
                        <p>
                          <strong>Bank:</strong> {step.details.bank}
                        </p>
                        <p>
                          <strong>Submission ID:</strong>{' '}
                          {step.details.submissionId}
                        </p>
                      </>
                    )}
                    {step.id === 'poll-status' && (
                      <>
                        <p>
                          ...
                          {step.details.acceptedPayments} payments
                        </p>
                        <p>
                          <strong>Rejected:</strong>{' '}
                          {step.details.rejectedPayments} payments
                        </p>
                        <p>
                          <strong>Correlation:</strong>{' '}
                          {step.details.correlationId}
                        </p>
                      </>
                    )}
                    {step.id === 'download-reconcile' && (
                      <>
                        <p>
                          <strong>camt.054:</strong> Downloaded
                        </p>
                        <p>
                          <strong>Reconciliation:</strong> Complete
                        </p>
                        <p>
      ...
      {completedSteps === steps.length && (
        <Alert>
          ...
          <AlertDescription>
            Payment operations workflow completed successfully! All payments
            have been processed and reconciled through the selected bank
            channel.
          </AlertDescription>
        </Alert>
      )}
