...
export default function KPIDashboardPage() {
  ...
  // KPI Metrics Query
  const { data: kpiData, isLoading: kpiLoading, error } = useQuery<KPIMetrics[]>({
    queryKey: ["/api/success-metrics", selectedProperty === "all" ? undefined : selectedProperty],
  ...
  const getKPIStatus = (value: number, target: number, isInverse = false) => {
    ...
    return {
      ...
      bgColor: isGood ? 'border-green-200 bg-green-50/50 dark:border-green-800 dark:bg-green-950/20' : 'border-red-200 bg-red-50/50 dark:border-red-800 dark:bg-red-950/20',
      icon: isGood ? CheckCircle : AlertTriangle,
    };
  };

  const unresolved = getKPIStatus(latestMetrics.unresolved_exception_rate, KPI_TARGETS.UNRESOLVED_EXCEPTIONS_MAX, true);
  const submissions = getKPIStatus(latestMetrics.government_submission_rate, KPI_TARGETS.GOVERNMENT_SUBMISSION_MIN);
  const runtime = getKPIStatus(latestMetrics.payroll_runtime_minutes, KPI_TARGETS.PAYROLL_RUNTIME_MAX, true);
  const automation = getKPIStatus(latestMetrics.automation_rate, KPI_TARGETS.AUTOMATION_TARGET);
    ...
    <div className="min-h-screen bg-background p-4 space-y-6">
      <div className="max-w-7xl mx-auto">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-semibold text-red-900 dark:text-red-100">
                      {criticalAlerts.length} Critical KPI Alert{criticalAlerts.length !== 1 ? 's' : ''}
                    </h3>
                    <p className="text-sm text-red-700 dark:text-red-200">
                      Immediate attention required to maintain operational targets
                    </p>
                  </div>
                </div>
              </CardContent>
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4 mb-8">
          ...
          <Card className={cn("transition-all duration-200", unresolved.bgColor)}>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground mb-2">
                <span className="font-medium">Target:</span> &lt; {KPI_TARGETS.UNRESOLVED_EXCEPTIONS_MAX}% unresolved per pay period
              </p>
              <Progress 
                value={Math.max(0, Math.min(100, (KPI_TARGETS.UNRESOLVED_EXCEPTIONS_MAX - latestMetrics.unresolved_exception_rate) / KPI_TARGETS.UNRESOLVED_EXCEPTIONS_MAX * 100))}
                className="h-2"
              />
              <div className="flex items-center mt-2">
                <Badge variant={unresolved.status === 'success' ? 'default' : 'destructive'} className="text-xs">
                  {unresolved.status === 'success' ? 'On Target' : 'Action Needed'}
                </Badge>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className={cn("transition-all duration-200", submissions.bgColor)}>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Government Submission Success</CardTitle>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground mb-2">
                <span className="font-medium">Target:</span> ≥ {KPI_TARGETS.GOVERNMENT_SUBMISSION_MIN}% ERGANI/APD/ΦΜΥ success
              </p>
              <Progress 
              ...
              />
              <div className="flex items-center mt-2">
                <Badge variant={submissions.status === 'success' ? 'default' : 'destructive'} className="text-xs">
                  {submissions.status === 'success' ? 'Compliant' : 'Below Target'}
                </Badge>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className={cn("transition-all duration-200", runtime.bgColor)}>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Payroll Processing Speed</CardTitle>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground mb-2">
                <span className="font-medium">Target:</span> ≤ {KPI_TARGETS.PAYROLL_RUNTIME_MAX} min for {latestMetrics.employee_count} employees
              </p>
              <Progress 
                value={Math.max(0, Math.min(100, (KPI_TARGETS.PAYROLL_RUNTIME_MAX - latestMetrics.payroll_runtime_minutes) / KPI_TARGETS.PAYROLL_RUNTIME_MAX * 100))}
                className="h-2"
              />
              <div className="flex items-center mt-2">
                <Badge variant={runtime.status === 'success' ? 'default' : 'destructive'} className="text-xs">
                  {runtime.status === 'success' ? 'Fast' : 'Optimization Needed'}
                </Badge>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className={cn("transition-all duration-200", automation.bgColor)}>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Zero Manual Re-key Rate</CardTitle>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground mb-2">
                <span className="font-medium">Target:</span> {KPI_TARGETS.AUTOMATION_TARGET}% ERP/SEPA automation
              </p>
              <Progress 
                value={latestMetrics.automation_rate} 
                className="h-2"
              />
              <div className="flex items-center mt-2">
                <Badge variant={automation.status === 'success' ? 'default' : 'destructive'} className="text-xs">
                  {automation.status === 'success' ? 'Fully Automated' : 'Manual Work Detected'}
                </Badge>
              </div>
            </CardContent>
          </Card>
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
              <div className="text-center">
                <div className="text-2xl font-bold text-primary mb-1">
                  {[unresolved, submissions, runtime, automation].filter(kpi => kpi.status === 'success').length}/4
                </div>
              ...
              <div className="text-center">
                ...
                <p className="text-sm text-muted-foreground">Employees managed</p>
              </div>
              <div className="text-center">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">
                          Target: {alert.threshold} | Current: {alert.actual_value}
                        </p>
                      </div>
                    </div>
