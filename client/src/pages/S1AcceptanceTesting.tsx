...
export default function S1AcceptanceTesting() {
  ...
  const [testResults, setTestResults] = useState<
    Record<string, AcceptanceCriteriaResult>
  >({
    ac1: {
      status: 'pending',
      message: 'Click "Test AC1" to run S1-16 calculations',
    },
    ac2: {
      status: 'pending',
      message: 'Click "Test AC2" to validate XBRL export',
    },
    ac3: {
      status: 'pending',
      message: 'Click "Test AC3" to test ruleset switching',
    },
    ac4: {
      status: 'pending',
      message: 'Click "Test AC4" to generate evidence pack',
    },
  });
  ...
  const testAC1 = useMutation({
    mutationFn: async () => {
      const response = await fetch(
        `/api/csrd/s1-16-metrics/${fiscalYear}?entity=${selectedEntity}&country=${selectedCountry}`
      );
      if (!response.ok) throw new Error('AC1 test failed');
      return response.json();
    },
    onSuccess: data => {
      const hasGenderPayGap =
        data.genderPayGap && typeof data.genderPayGap.value === 'number';
      const hasRatio =
        data.highestToMedianRatio &&
        typeof data.highestToMedianRatio.value === 'number';
      const hasMethodNotes =
        data.genderPayGap?.methodNote && data.highestToMedianRatio?.methodNote;
      const hasPopulationCounts =
        data.genderPayGap?.populationCounts &&
        data.highestToMedianRatio?.populationCounts;

      const allPassed =
        hasGenderPayGap && hasRatio && hasMethodNotes && hasPopulationCounts;
    ...
    onError: error => {
      setTestResults(prev => ({
        ...
        ac1: {
          status: 'fail',
          message: `AC1 Failed: ${error.message}`,
          error: error.message,
        },
      }));
    },
  ...
  const testAC2 = useMutation({
    mutationFn: async () => {
      const response = await fetch(
        `/api/csrd/xbrl-export/${fiscalYear}?entity=${selectedEntity}&taxonomyVersion=ESRS_Set1_2023`
      );
    ...
    onSuccess: data => {
      const hasXBRL =
        data.xbrlSnippet &&
        data.xbrlSnippet.includes('esrs-s1:GenderPayGapPercentage');
      const isValidTaxonomy = data.validation?.isValid === true;
      const correctVersion =
        data.validation?.taxonomyVersion === 'ESRS_Set1_2023';
    ...
    onError: error => {
      setTestResults(prev => ({
        ...
        ac2: {
          status: 'fail',
          message: `AC2 Failed: ${error.message}`,
          error: error.message,
        },
      }));
    },
  ...
  const testAC3 = useMutation({
    ...
    onSuccess: data => {
      const enableWorked =
        data.enableResponse?.activeRuleset === 'esrs_s1.v2025_quickfix';
      const disableWorked =
        data.disableResponse?.activeRuleset === 'esrs_s1.v2023';
      ...
      setTestResults(prev => ({
        ...
        ac3: {
          status: allPassed ? 'pass' : 'fail',
          message: allPassed
            ? '✓ Ruleset switching works. Currently using: ' +
              data.disableResponse?.activeRuleset
    ...
    onError: error => {
      setTestResults(prev => ({
        ...
        ac3: {
          status: 'fail',
          message: `AC3 Failed: ${error.message}`,
          error: error.message,
        },
      }));
    },
  ...
  const testAC4 = useMutation({
    mutationFn: async () => {
      const response = await fetch(
        `/api/csrd/evidence-pack/${fiscalYear}?entity=${selectedEntity}&country=${selectedCountry}`
      );
    ...
    onSuccess: data => {
      ...
      const hasAssurance =
        data.assuranceMetadata && data.assuranceMetadata.dataLineageComplete;
    ...
    onError: error => {
      setTestResults(prev => ({
        ...
        ac4: {
          status: 'fail',
          message: `AC4 Failed: ${error.message}`,
          error: error.message,
        },
      }));
    },
  ...
  const overallStatus = Object.values(testResults).every(
    r => r.status === 'pass'
  )
    ? 'pass'
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">
            S1 Acceptance Criteria Testing
          </h1>
        ...
        <TabsContent value="ac1">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Calculate S1-16 metrics by entity and country for closed fiscal
                years with method notes and population counts (EFRAG
                requirement)
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                <div
                  className={`p-3 rounded border ${getStatusColor(testResults.ac1.status)} border`}
                >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    <div>
                      ...
                      <ul className="space-y-1 text-muted-foreground">
                        <li>
                          Value:{' '}
                          {testResults.ac1.data.genderPayGap.value.toFixed(1)}%
                        </li>
                        <li>
                          Population:{' '}
                          {
                            testResults.ac1.data.genderPayGap.populationCounts
                              .totalEmployees
                          }{' '}
                          employees
                        </li>
                    ...
                    <div>
                      <h4 className="font-medium mb-2">
                        Highest-to-Median Ratio
                      </h4>
                      <ul className="space-y-1 text-muted-foreground">
                        <li>
                          Ratio:{' '}
                          {testResults.ac1.data.highestToMedianRatio.value.toFixed(
                            1
                          )}
                          :1
                        </li>
                        <li>
                          Population:{' '}
                          {
                            testResults.ac1.data.highestToMedianRatio
                              .populationCounts.totalEmployees
                          }{' '}
                          employees
                        </li>
                        <li>
                          Highest paid: €
                          {testResults.ac1.data.highestToMedianRatio.populationCounts.highestPaidValue.toLocaleString()}
                        </li>
                      </ul>
                    </div>
                  </div>
        ...
        <TabsContent value="ac2">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Export XBRL snippet that validates against ESRS Set-1 (2023)
                taxonomy (EFRAG XBRL requirement)
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                <div
                  className={`p-3 rounded border ${getStatusColor(testResults.ac2.status)} border`}
                >
                  <div className="flex items-center gap-2 mb-2">
                    {getStatusIcon(testResults.ac2.status)}
        ...
        <TabsContent value="ac3">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Support ruleset switch when Commission's 2025 quick-fix is
                finalized with feature flag (Finance requirement)
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                <div
                  className={`p-3 rounded border ${getStatusColor(testResults.ac3.status)} border`}
                >
                  <div className="flex items-center gap-2 mb-2">
                    {getStatusIcon(testResults.ac3.status)}
                ...
                {testResults.ac3.data && (
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div className="space-y-2">
                      ...
                      <p className="text-muted-foreground">
                        Ruleset:{' '}
                        {testResults.ac3.data.enableResponse.activeRuleset}
                      </p>
                      <p className="text-muted-foreground">
                        Flag:{' '}
                        {testResults.ac3.data.enableResponse.featureFlag
                          ? 'Enabled'
                          : 'Disabled'}
                      </p>
                    </div>
                    <div className="space-y-2">
                      ...
                      <p className="text-muted-foreground">
                        Ruleset:{' '}
                        {testResults.ac3.data.disableResponse.activeRuleset}
                      </p>
                      <p className="text-muted-foreground">
                        Flag:{' '}
                        {testResults.ac3.data.disableResponse.featureFlag
                          ? 'Enabled'
                          : 'Disabled'}
                      </p>
                    </div>
        ...
        <TabsContent value="ac4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Generate Evidence Pack with CSV extract and JSON calculation log
                suitable for assurance
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                <div
                  className={`p-3 rounded border ${getStatusColor(testResults.ac4.status)} border`}
                >
                  <div className="flex items-center gap-2 mb-2">
                    {getStatusIcon(testResults.ac4.status)}
                ...
                {testResults.ac4.data && (
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                    <div className="space-y-2">
                      ...
                      <p className="text-muted-foreground">
                        {testResults.ac4.data.csvExtract.split('\n').length}{' '}
                        lines
                      </p>
                    ...
                    <div className="space-y-2">
                      ...
                      <p className="text-muted-foreground">
                        Data lineage:{' '}
                        {testResults.ac4.data.assuranceMetadata
                          .dataLineageComplete
                          ? 'Complete'
                          : 'Incomplete'}
                      </p>
