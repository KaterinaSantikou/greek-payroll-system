...
export default function DocumentAI() {
  ...
  const [signatureForm, setSignatureForm] = useState({
    ...
    expirationDays: 30,
    signers: [
      { name: '', email: '', role: 'employee', sequence: 1, authenticationMethod: 'email' }
    ]
  });
  ...
  }) as { data?: { data?: { requests?: SignatureRequest[]; summary?: any } }; isLoading: boolean };

  // Process documents mutation
  const processDocumentsMutation = useMutation({
    mutationFn: async ({ files, type, documentType }: { 
      files: FileList; 
      type: 'single' | 'batch'; 
      documentType: string;
    }) => {
  ...
  const createSignatureRequestMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['/api/documents/signature-requests'] });
    },
    onError: (error: any) => {
      toast({
  ...
  const sendSignatureRequestMutation = useMutation({
    mutationFn: async (requestId: string) => {
      const response = await fetch(`/api/documents/send-signature-request/${requestId}`, {
        method: 'POST',
      });
      return await response.json();
    },
  ...
  const handleCreateSignatureRequest = useCallback(async () => {
    if (!signatureForm.documentName || !signatureForm.documentUrl || signatureForm.signers.length === 0) {
      toast({
        title: "Missing Required Fields",
        description: "Please fill in document name, URL, and at least one signer",
        variant: "destructive",
      });
      return;
    }
      ...
      <div className="flex items-center justify-between">
        <div className="space-y-1">
          ...
          <p className="text-muted-foreground">
            OCR processing for Greek employment contracts and IDs, plus e-signature workflows
          </p>
        </div>
      ...
      {analytics && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{analytics.data?.documentsProcessed?.total || 0}</div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{analytics.data?.signatureRequests?.pending || 0}</div>
        ...
        <TabsContent value="upload" className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Upload Greek employment contracts, ID cards, or other documents for AI-powered OCR processing
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                />
                <p className="text-xs text-muted-foreground">
                  Supported formats: PDF, JPG, PNG, TIFF, BMP, DOC, DOCX (Max: 50MB)
                </p>
              </div>
          ...
          {processedResults.length > 0 && (
            <Card>
              <CardHeader>
                ...
                <CardDescription>OCR extraction results with confidence scores</CardDescription>
              </CardHeader>
              <CardContent>
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            ...
                            <CardTitle className="text-base">
                              {result.originalFilename || `Document ${result.id.slice(0, 8)}`}
                            </CardTitle>
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <Badge variant={getStatusBadgeVariant(result.status)}>
                              {result.confidence > 0 ? `${(result.confidence * 100).toFixed(1)}% confidence` : result.status}
                            </Badge>
                          </div>
                        </div>
                      </CardHeader>
                      <CardContent className="space-y-3">
                        {result.validation && !result.validation.valid && (
                          <Alert>
                            ...
                            <AlertDescription>
                              Validation Issues: {result.validation.errors.join(', ')}
                            </AlertDescription>
                          </Alert>
                        )}
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 bg-muted rounded-lg">
                            ...
                            <div className="space-y-2">
                              ...
                              <div className="text-xs space-y-1">
                                <div>Size: {result.fileSize ? `${(result.fileSize / 1024).toFixed(1)} KB` : 'Unknown'}</div>
                                <div>Processed: {new Date(result.processedAt).toLocaleString()}</div>
                              </div>
                            </div>
                          </div>
                        )}
                          ...
                          {result.documentType === 'contract' && (
                            <Button 
                              ...
                              onClick={() => {
                                setSignatureForm(prev => ({
                                  ...prev,
                                  documentName: result.originalFilename || `Contract ${result.id.slice(0, 8)}`,
        ...
        <TabsContent value="signature" className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Set up e-signature workflow for Greek employment contracts with legal compliance
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setSignatureForm(prev => ({ ...prev, documentName: e.target.value }))}
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setSignatureForm(prev => ({ ...prev, documentUrl: e.target.value }))}
                    placeholder="https://..."
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setSignatureForm(prev => ({ ...prev, expirationDays: parseInt(e.target.value) || 30 }))}
                    min={1}
                    max={365}
                  />
                </div>
              ...
              <Alert>
                ...
                <AlertDescription>
                  <strong>Greek Legal Compliance:</strong> This signature request will comply with Greek Law 4070/2012 
                  for electronic signatures and include ERGANI notification for employment contracts.
                </AlertDescription>
              </Alert>
        ...
        <TabsContent value="requests" className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Manage and monitor e-signature requests with real-time status updates
              </CardDescription>
            </CardHeader>
            <CardContent>
              {loadingRequests ? (
                ...
                <div className="space-y-4">
                  {signatureRequests.data.summary && (
                    <div className="grid grid-cols-2 md:grid-cols-5 gap-4 p-4 bg-muted rounded-lg">
                      <div className="text-center">
                        <div className="text-2xl font-bold">{signatureRequests.data.summary.total}</div>
                        <div className="text-xs text-muted-foreground">Total</div>
                      </div>
                      <div className="text-center">
                        <div className="text-2xl font-bold text-blue-600">{signatureRequests.data.summary.in_progress}</div>
                        <div className="text-xs text-muted-foreground">In Progress</div>
                      </div>
                      <div className="text-center">
                        <div className="text-2xl font-bold text-green-600">{signatureRequests.data.summary.completed}</div>
                        <div className="text-xs text-muted-foreground">Completed</div>
                      </div>
                      <div className="text-center">
                        <div className="text-2xl font-bold text-red-600">{signatureRequests.data.summary.declined}</div>
                        <div className="text-xs text-muted-foreground">Declined</div>
                      </div>
                      <div className="text-center">
                        <div className="text-2xl font-bold text-yellow-600">{signatureRequests.data.summary.expired}</div>
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {signatureRequests.data.requests.map((request: SignatureRequest) => (
                        <TableRow key={request.id}>
                          ...
                          <TableCell>
                            <div className="flex items-center gap-1">
                              <span>{request.signers.filter(s => s.status === 'signed').length}</span>
                              <span className="text-muted-foreground">/{request.signers.length}</span>
                          ...
                          <TableCell>
                            <div className="flex items-center gap-2">
                              {request.status === 'draft' && (
                                <Button
                                  onClick={() => sendSignatureRequestMutation.mutate(request.id)}
                              ...
                              {request.status === 'in_progress' && (
                                <Button
                                  onClick={() => {
                                    ...
                                    toast({
                                      title: "Reminder Sent",
                                      description: "Reminder emails sent to pending signers",
                                    });
                                  }}
                                  size="sm"
                                  variant="outline"
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-muted-foreground">Create your first signature request to get started</p>
                </div>
              )}
            </CardContent>
          </Card>
