...
export default function EmbedPage() {
  useEffect(() => {
    ...
    const validSurfaces = ['payroll_run', 'exceptions_review', 'filings_panel', 'payments_cockpit'];
  ...
  // Parse URL parameters
  const params = new URLSearchParams(window.location.search);
  const surface = params.get('surface') as 'payroll_run' | 'exceptions_review' | 'filings_panel' | 'payments_cockpit';
  const token = params.get('token');
  const tenantId = params.get('tenantId');
  const locale = params.get('locale') || 'en';
  const theme = params.get('theme') as 'light' | 'dark' || 'light';
    ...
    return (
      <div className="flex items-center justify-center h-screen bg-red-50">
        <div className="text-center">
          ...
          <p className="text-gray-600">Missing required parameters: surface, token, or tenantId</p>
        </div>
      </div>
    );
  }
