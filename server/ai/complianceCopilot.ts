...
export class ComplianceCopilot {
    ...
    try {
      const query = db
        ...
        .where(
          and(
            ...
            propertyIds
              ? inArray(employees.defaultPropertyId, propertyIds)
              : undefined
          )
        )
        .groupBy(
      ...
      for (const result of results) {
        if (
          result.totalPay &&
          result.hoursWorked &&
          Number(result.totalPay) > 0 &&
          Number(result.hoursWorked) > 0
        ) {
          const hourlyRate =
            Number(result.totalPay) / Number(result.hoursWorked);
          const minHourlyRate = this.MIN_WAGE_2025 / 173.33; // Monthly to hourly

          if (hourlyRate < minHourlyRate) {
            const shortfall =
              (minHourlyRate - hourlyRate) * Number(result.hoursWorked);
            ...
            anomalies.push({
              ...
              suggestedFix: {
                ...
                preventionStrategy:
                  'Implement automated minimum wage validation in payroll calculation',
              },
              autoFixable: false,
              confidence: 95,
              legalRisk: 'high',
  ...
  private async detectExcessiveOvertime(
    ...
    try {
      ...
      for (const result of results) {
        ...
        let overtimeHours = 0;
        if (
          result.overtimeByTier &&
          typeof result.overtimeByTier === 'object'
        ) {
        ...
        if (totalWeeklyHours > this.MAX_WEEKLY_HOURS) {
          ...
          anomalies.push({
            ...
            suggestedFix: {
              ...
              preventionStrategy:
                'Implement real-time overtime alerts at 40h/week threshold',
            },
            autoFixable: false,
            confidence: 90,
            legalRisk: 'high',
            ...
            if (restHours < this.MIN_REST_HOURS) {
              anomalies.push({
                ...
                suggestedFix: {
                  ...
                  preventionStrategy:
                    'Add automated rest period validation to scheduling system',
                },
                autoFixable: true,
                confidence: 85,
                legalRisk: 'medium',
          ...
          if (affectsRequestedProperty) {
            anomalies.push({
              ...
              suggestedFix: {
                ...
                preventionStrategy:
                  'Implement automated ERGANI submission with 24h advance alerts',
              },
              autoFixable: true,
              confidence: 95,
              legalRisk: 'high',
  ...
  private async detectWorkingTimeViolations(
    ...
    try {
      ...
      const employeeWorkDays = new Map<
        string,
        {
          employee: any;
          dates: Date[];
            ...
            if (consecutiveDays > this.MAX_CONSECUTIVE_DAYS) {
              anomalies.push({
                ...
                suggestedFix: {
                  ...
                  preventionStrategy:
                    'Implement 6-day maximum rule in scheduling algorithm',
                },
                autoFixable: true,
                confidence: 88,
                legalRisk: 'medium',
        ...
        if (consecutiveDays > this.MAX_CONSECUTIVE_DAYS) {
          anomalies.push({
            ...
            suggestedFix: {
              ...
              estimatedTime: '30 minutes',
              preventionStrategy:
                'Add real-time consecutive day counter to scheduling system',
            },
            autoFixable: true,
            confidence: 92,
            legalRisk: 'medium',
  ...
  async generateComplianceReport(
    ...
    const summary = {
      ...
      totalFinancialImpact: anomalies.reduce(
  ...
  private generateRecommendations(
    ...
    if (summary.bySeverity.critical > 0) {
      recommendations.push(
        `Address ${summary.bySeverity.critical} critical issues immediately to avoid legal penalties`
      );
    }

    if (summary.byType.under_min_wage > 0) {
      recommendations.push(
        'Review and update wage calculation system to prevent minimum wage violations'
      );
    }

    if (summary.byType.excessive_overtime > 0) {
      recommendations.push(
        'Implement stricter overtime controls and consider additional hiring'
      );
    }

    if (summary.byType.ergani_delay > 0) {
      recommendations.push(
        'Set up automated ERGANI submissions with advance notifications'
      );
    }

    if (summary.autoFixableCount > 0) {
      recommendations.push(
        `${summary.autoFixableCount} issues can be auto-fixed - consider enabling automation`
      );
    }

    if (summary.totalFinancialImpact > 5000) {
      recommendations.push(
        'High financial risk detected - prioritize compliance improvements'
      );
    }
