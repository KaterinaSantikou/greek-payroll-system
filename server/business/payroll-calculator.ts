...
export class PayrollCalculator {
  ...
  calculateGreekBonuses(
    ...
    const contractFactor = contractType === 'seasonal' 
      ? CONTRACT_TYPE_RULES.seasonal.bonusReduction 
      : 1;
    
    return {
      christmas: baseSalary * GREEK_BONUSES.christmas.fullTimeMonthly * proRationFactor * contractFactor,
      easter: baseSalary * GREEK_BONUSES.easter.fullTimeMonthly * proRationFactor * contractFactor,
      vacation: baseSalary * GREEK_BONUSES.vacation.fullTimeMonthly * proRationFactor * contractFactor
    };
  }
  ...
  calculateLeavePay(
    ...
    return {
      ...
      sickPay: (leaveHours.sick || 0) * hourlyRate * LEAVE_ENTITLEMENTS.sick.payPercentage,
  ...
  calculateBenefitsInKind(
    ...
    const taxableMeals = Math.max(0, mealVouchers - TAX_FREE_LIMITS.mealVouchers);

    // Company car: percentage of car value as imputed income
    const companyCarBenefit = (benefitsInKind.companyCar || 0) * 
      BENEFITS_IN_KIND.companyCar.annualTaxablePercentage / 12;
  ...
  calculatePayroll(input: PayrollCalculationInput): PayrollCalculationResult {
    ...
    const effectiveHourlyRate = hourlyRate || (baseSalary / WORKING_TIME_LIMITS.standardMonthlyHours);
    ...
    const totalAllowances = Object.values(allowancesBreakdown).reduce((sum, val) => sum + val, 0);

    // Calculate bonuses
    const bonuses = this.calculateGreekBonuses(
      baseSalary, employmentStartDate, periodEndDate, isFullTime, contractType
    );
    ...
    const totalLeavePay = leavePay.paidLeave + leavePay.sickPay + leavePay.maternityPay + leavePay.paternityPay;
