...
import {
  seedCbaPacks,
  demoCbaPackFunctionality,
} from '../seeders/cbaPackSeeder';
...
router.post('/demo/install', async (req: Request, res: Response) => {
  try {
    ...
    res.json({
      ...
      data: {
        installed_packs: [result.tourismPack.name, result.fnbPack.name],
        next_steps: [
...
router.get('/dashboard', async (req: Request, res: Response) => {
  try {
    ...
    const dashboard = {
      ...
      by_sector: {
        tourism_hotels: allPacks.filter(p => p.sector === 'tourism_hotels')
          .length,
        fnb_restaurants: allPacks.filter(p => p.sector === 'fnb_restaurants')
          .length,
      },
      rule_counts: {
        total_wage_tables: allPacks.reduce(
          (sum, p) => sum + p.wageTables.length,
          0
        ),
        total_premium_rules: allPacks.reduce(
          (sum, p) => sum + p.premiumRules.length,
          0
        ),
        total_allowance_rules: allPacks.reduce(
          (sum, p) => sum + p.allowanceRules.length,
          0
        ),
        total_tip_policies: allPacks.reduce(
          (sum, p) => sum + p.tipPolicies.length,
          0
        ),
      },
...
router.post('/validate-compliance', async (req: Request, res: Response) => {
  try {
    ...
    const [wageResult, premiums, allowances, constraints] = await Promise.all([
      CbaPackService.calculateEffectiveWage(
        propertyId,
        employeeCategory,
        employeeGrade,
        seniorityStep || 0
    ...
    if (scheduledHours) {
      for (const constraint of constraints) {
        if (scheduledHours > constraint.maxHoursDay) {
          violations.push(
            `Scheduled hours (${scheduledHours}) exceed daily max (${constraint.maxHoursDay})`
          );
        }
      }
    }
    ...
    if (wageResult.baseWage < minWage) {
      violations.push(
        `Base wage (€${wageResult.baseWage}) below statutory minimum (€${minWage})`
      );
    }
    ...
    const complianceReport = {
      ...
      compliance_status:
        violations.length === 0 ? 'compliant' : 'violations_found',
      violations,
