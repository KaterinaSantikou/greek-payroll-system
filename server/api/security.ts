...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const accessData = await gdprService.processAccessRequest(
        employeeId,
        userId
      );
      res.json(accessData);
...
router.post(
  '/api/gdpr/rectification/:employeeId',
  isAuthenticated,
  async (req, res) => {
    try {
      const userId = (req.user as any)?.claims?.sub;
      ...
      await gdprService.processRectificationRequest(
        employeeId,
        corrections,
        userId
      );
...
router.get(
  '/api/gdpr/portability/:employeeId',
  isAuthenticated,
  async (req, res) => {
    try {
      const userId = (req.user as any)?.claims?.sub;
      ...
      const portableData = await gdprService.processPortabilityRequest(
        employeeId,
        userId
      );
...
router.post('/api/approvals/initiate', isAuthenticated, async (req, res) => {
  try {
    ...
    const approval = await rbacService.initiateApproval(
      userId,
      operation,
      resourceId,
      requestData
...
router.post(
  '/api/approvals/:approvalId/process',
  isAuthenticated,
  async (req, res) => {
    try {
      const userId = (req.user as any)?.claims?.sub;
      ...
      const result = await rbacService.processApproval(
        approvalId,
        userId,
        decision,
        comments
...
router.get('/api/permissions/check', isAuthenticated, async (req, res) => {
  try {
    ...
    if (!resource || !action) {
      return res
        .status(400)
        .json({ error: 'resource and action parameters required' });
    }
...
router.get('/api/audit/search', isAuthenticated, async (req, res) => {
  try {
    ...
    if (!startDate || !endDate) {
      return res
        .status(400)
        .json({ error: 'startDate and endDate parameters required' });
    }
...
router.get(
  '/api/compliance/data-residency',
  isAuthenticated,
  async (req, res) => {
    try {
      const compliance =
        await dataResidencyService.validateDataResidencyCompliance();
      res.json(compliance);
    } catch (error) {
      console.error('Error checking data residency compliance:', error);
      res
        .status(500)
        .json({ error: 'Failed to check data residency compliance' });
    }
  }
);

router.get(
  '/api/backup/retention-schedule',
  isAuthenticated,
  async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: 'Failed to fetch backup retention schedule' });
    }
  }
);
