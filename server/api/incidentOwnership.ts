...
router.post('/assign/:incidentId', isAuthenticated, async (req, res) => {
  try {
    ...
    const assignment =
      await ownershipService.autoAssignIncident(incidentContext);
    ...
    res.json({
      ...
      assignment,
      message:
        assignment.assignmentType === 'fallback_assigned'
...
router.post('/escalate/:incidentId', isAuthenticated, async (req, res) => {
  try {
    ...
    await ownershipService.escalateIncident(
      incidentId,
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      await ownershipService.updateResponderAvailability(
        responderId,
        isAvailable,
        reason
      );
 ...
 */
router.delete(
  '/assignments/:assignmentId/escalation',
  isAuthenticated,
  async (req, res) => {
    try {
