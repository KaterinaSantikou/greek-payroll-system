...
export function registerCsrdRoutes(app: Router) {
  ...
  app.get('/api/csrd/dashboard/:propertyId', async (req, res) => {
    try {
      ...
      const [latestWorkforce] = await db
        ...
        .where(
          eq(s1WorkforceCharacteristics.reportingPeriodId, currentPeriod.id)
        )
      ...
      const dashboardData = {
        ...
        keyMetrics: {
          workforce: latestWorkforce
            ? {
                ...
                genderPayGap: latestPayMetrics
                  ? parseFloat(latestPayMetrics.genderPayGapPercentage)
                  : null,
          ...
          payEquity: latestPayMetrics
            ? {
                genderPayGap: parseFloat(
                  latestPayMetrics.genderPayGapPercentage
                ),
        ...
        compliance: {
          ...
          materialityStatus: currentPeriod.s1WorkforceMaterial
            ? 'Material'
            : 'Not Material',
  ...
  app.post(
    '/api/csrd/calculate/workforce/:reportingPeriodId',
    async (req, res) => {
      try {
        ...
        const characteristics =
          await csrdService.calculateWorkforceCharacteristics(
            reportingPeriodId,
        ...
        res
          .status(500)
          .json({ error: 'Failed to calculate workforce characteristics' });
      }
    }
  );
  ...
  app.post(
    '/api/csrd/calculate/turnover/:reportingPeriodId',
    async (req, res) => {
      try {
        const { reportingPeriodId } = req.params;
  ...
  app.post(
    '/api/csrd/calculate/pay-metrics/:reportingPeriodId',
    async (req, res) => {
      try {
        const { reportingPeriodId } = req.params;
  ...
  app.post(
    '/api/csrd/calculate/work-life-balance/:reportingPeriodId',
    async (req, res) => {
      try {
        const { reportingPeriodId } = req.params;
        const { periodStart, periodEnd } = req.body;
        ...
        res
          .status(500)
          .json({ error: 'Failed to calculate work-life balance metrics' });
      }
    }
  );
  ...
  app.post(
    '/api/csrd/calculate/health-safety/:reportingPeriodId',
    async (req, res) => {
      try {
        const { reportingPeriodId } = req.params;
        const { periodStart, periodEnd } = req.body;
        ...
        res
          .status(500)
          .json({ error: 'Failed to calculate health & safety metrics' });
      }
    }
  );
  ...
  app.post(
    '/api/csrd/calculate/enhanced-s1/:reportingPeriodId',
    async (req, res) => {
      try {
        const { reportingPeriodId } = req.params;
        ...
        res
          .status(500)
          .json({ error: 'Failed to calculate enhanced S1 metrics' });
      }
    }
  );
  ...
  app.post('/api/csrd/switch-version', async (req, res) => {
    try {
      ...
      res.json({
        success: true,
        message: `Switched to ESRS version: ${version}`,
      });
    } catch (error) {
  ...
  app.post('/api/csrd/materiality-assessment', async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: 'Failed to create materiality assessment' });
    }
  });

  app.get(
    '/api/csrd/materiality-assessment/:reportingPeriodId',
    async (req, res) => {
      try {
        const { reportingPeriodId } = req.params;
  ...
  app.get('/api/csrd/export/:reportingPeriodId', async (req, res) => {
    try {
      const { reportingPeriodId } = req.params;
      const { exportType = 's1_only', applyMateriality = 'true' } = req.query;
  ...
  app.patch('/api/csrd/materiality/:reportingPeriodId', async (req, res) => {
    try {
      ...
      const [updatedPeriod] = await db
        ...
        .set({
          ...data,
          materialityAssessmentDate:
            data.materialityAssessmentDate ||
      ...
      res
        .status(500)
        .json({ error: 'Failed to update materiality assessment' });
    }
  });
  ...
  app.get('/api/csrd/metrics', async (req, res) => {
    try {
      ...
      const s1Engine = new (
        await import('../s1ComplianceEngine')
      ).S1ComplianceEngine();
      ...
      const metricsData = {
        genderPayGap: {
          ...
          methodology: {
            hourlyDerivation:
              genderPayGapData.metadata.hourlyDerivationBreakdown.methodology,
            nonEmployeesExcluded:
              genderPayGapData.metadata.nonEmployeesExcluded,
            sampleSize: genderPayGapData.metadata.sampleSize,
            methodologyDisclosure: genderPayGapData.methodologyDisclosure,
          },
          status:
            genderPayGapData.metadata.sampleSize > 10 ? 'ready' : 'incomplete',
        },
        topToMedianRatio: {
          ...
          formula:
            'Ratio = Highest paid total compensation ÷ Median employee total compensation',
          inclusions: [
        ...
        healthSafetyCoverage: {
          ...
          lastCalculated: new Date().toISOString(),
          formula:
            'Coverage = Employees covered by H&S management system ÷ Total employees × 100',
          inclusions: [
        ...
        incidentsRate: {
          ...
          lastCalculated: new Date().toISOString(),
          formula:
            'Rate = (Work-related injuries × 200,000) ÷ Total hours worked',
          inclusions: [
        ...
        workLifeUsage: {
          ...
          lastCalculated: new Date().toISOString(),
          formula:
            'Usage Rate = (Employees who took leave ÷ Eligible employees) × 100',
          inclusions: [
          ...
          methodology: {
            ...
            sampleSize: Object.values(
              workLifeData.greekSpecificCategories
            ).reduce((sum, cat) => sum + cat.eligible, 0),
          },
          status: 'ready',
        },
      };
  ...
  app.get('/api/csrd/evidence-packs/:reportingPeriodId', async (req, res) => {
    try {
      ...
      const evidencePacks =
        await evidencePackService.listEvidencePacks(reportingPeriodId);
  ...
  app.post('/api/csrd/xbrl/taxonomy/load', async (req, res) => {
    try {
      ...
      const taxonomy =
        await xbrlTaggingService.loadESRSSet1Taxonomy(taxonomyVersion);
  ...
  app.post('/api/csrd/xbrl/generate/:reportingPeriodId', async (req, res) => {
    try {
      ...
      if (!metricsData || typeof metricsData !== 'object') {
        return res
          .status(400)
          .json({ error: 'metricsData object is required' });
      }
  ...
  app.post('/api/csrd/xbrl/report/:reportingPeriodId', async (req, res) => {
    try {
      ...
      const reportSections =
        await xbrlTaggingService.generateHumanReadableReport(
          reportingPeriodId,
          metricsData,
          language as 'en' | 'el'
        );
      ...
      res
        .status(500)
        .json({ error: 'Failed to generate human-readable report' });
    }
  });
  ...
  app.get('/api/csrd/xbrl/instances/:reportingPeriodId', async (req, res) => {
    try {
      ...
      const xbrlInstances =
        await xbrlTaggingService.getXBRLInstances(reportingPeriodId);
  ...
  // Get Report Sections
  app.get(
    '/api/csrd/xbrl/report-sections/:reportingPeriodId',
    async (req, res) => {
      try {
        const { reportingPeriodId } = req.params;
  ...
  app.post('/api/csrd/xbrl/taxonomy/update', async (req, res) => {
    try {
      ...
      const updatedTaxonomy =
        await xbrlTaggingService.updateTaxonomy(taxonomyVersion);
  ...
  app.get(
    '/api/csrd/data-lineage/:reportingPeriodId/:metricCode',
    async (req, res) => {
      try {
  ...
  app.post(
    '/api/csrd/generate-complete-package/:reportingPeriodId',
    async (req, res) => {
      try {
        const { reportingPeriodId } = req.params;
        const {
        ...
        if (!metricsIncluded || !Array.isArray(metricsIncluded)) {
          return res
            .status(400)
            .json({ error: 'metricsIncluded array is required' });
        }
        ...
        res
          .status(500)
          .json({ error: 'Failed to generate complete audit package' });
      }
    }
  );
}
