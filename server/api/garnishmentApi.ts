...
export function registerGarnishmentRoutes(app: Express) {
   ...
   * Calculate garnishments for a pay period (preview/test calculation)
   */
  app.post('/api/garnishments/calculate', isAuthenticated, async (req, res) => {
    try {
      // Validate input data
      const calculationInputs = garnishmentCalculationInputsSchema.parse(req.body);
      
      const calculationResult = await GarnishmentService.calculateGarnishments(calculationInputs);
      ...
      console.error('Error calculating garnishments:', error);
      res.status(500).json({
        success: false,
