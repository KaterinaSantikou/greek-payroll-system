...
const idempotencyMiddleware = (req: any, res: any, next: any) => {
  ...
  if (!idempotencyKey && req.method === 'POST') {
    return res
      .status(400)
      .json({
        error: 'Idempotency-Key header required for payroll operations',
      });
  }
...
router.get('/api/payroll/runs/:id', isAuthenticated, addUserContext, requirePayrollAccess, async (req, res) => {
  try {
    ...
    const garnishmentLines = lines.filter(line =>
      line.lineType?.startsWith('GARN_')
    );
    const totalGarnishments = garnishmentLines.reduce(
      (sum, line) => sum + Math.abs(parseFloat(line.amount || '0')),
      0
    );

    const totals = {
      totalGrossPay: lines.reduce(
        (sum, line) => sum + parseFloat(line.grossAmount || '0'),
        0
      ),
      totalNetPay: lines.reduce(
        (sum, line) => sum + parseFloat(line.netAmount || '0'),
        0
      ),
      ...
      totalEfka: lines.reduce(
        (sum, line) => sum + parseFloat(line.efkaAmount || '0'),
        0
      ),
      totalGarnishments,
  ...
  async (req, res) => {
    try {
      ...
      if (!runData.dryRun && runData.runType === 'regular') {
        const hasExisting = await payrollRepository.hasExistingFinalizedRun(
          runData.period
        );
        if (hasExisting) {
      ...
      if (error instanceof z.ZodError) {
        return res
          .status(400)
          .json({ error: 'Invalid payroll run data', details: error.errors });
      }
  ...
  async (req, res) => {
    try {
      ...
      if (!finalizedRun) {
        return res
          .status(500)
          .json({ error: 'Failed to finalize payroll run' });
      }
      ...
      if (error instanceof z.ZodError) {
        return res
          .status(400)
          .json({ error: 'Invalid finalization data', details: error.errors });
      }
...
// Helper functions
async function initiatePayrollCalculation(
  run: any,
  runData: any
): Promise<void> {
  try {
    ...
    setTimeout(async () => {
      try {
        ...
        if (employeeIds.length > 0) {
          const employees =
            await payrollRepository.getEmployeePayrollInfo(employeeIds);
          const timesheets = await payrollRepository.getTimesheetData(
            employeeIds,
            run.period
          );
          ...
          const employeeData = employees.map(emp => {
            const timesheet = timesheets.find(
              ts => ts.employeeId === emp.employeeId
            );
            return {
              ...emp,
          ...
          const result = payrollService.processPayrollScope(
            scope,
            employeeData,
            {
          ...
          if (result.success) {
            await payrollRepository.updatePayrollRunStatus(
              run.id,
              'calculated'
            );
          } else {
...
function generateAuditHash(data: any): string {
  return Buffer.from(JSON.stringify(data) + Date.now())
    .toString('base64')
    .slice(0, 32);
}
  ...
  return [
    ...
    ...(run.finalizedAt
      ? [
          {
            action: 'finalized',
            timestamp: run.finalizedAt,
            user: run.finalizedBy,
          },
        ]
      : []),
  ];
...
router.get('/api/payroll/scopes', isAuthenticated, async (req, res) => {
  try {
    ...
    // Delegate to infrastructure layer
    const result = await payrollRepository.getPayrollScopesByPeriod(
      period as string
    );
    ...
    try {
      ...
      if (scope.status !== 'draft') {
        scopeLines = await db
          ...
          .orderBy(
            asc(payrollScopeLines.employeeId),
            asc(payrollScopeLines.code)
          );
      }
...
router.put(
  '/api/payroll/scopes/:scopeId/status',
  isAuthenticated,
  async (req, res) => {
    try {
      const { scopeId } = req.params;
      ...
      if (
        !['draft', 'computing', 'computed', 'reviewed', 'finalized'].includes(
          status
        )
      ) {
...
router.get(
  '/api/payroll/period-ledgers/:period',
  isAuthenticated,
  async (req, res) => {
    try {
...
router.post(
  '/api/payroll/period-ledgers/:period/consolidate',
  isAuthenticated,
  async (req, res) => {
    try {
      const { period } = req.params;
