...
export function registerGRCRoutes(app: Express) {
  ...
  app.get('/api/grc/soa', isAuthenticated, async (req: any, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: 'Failed to fetch Statement of Applicability' });
    }
  });
  ...
  app.put('/api/grc/soa/:controlId', isAuthenticated, async (req: any, res) => {
    try {
      ...
      const updatedSoa = await GRCService.updateSoA(
        tenantId,
        controlId,
        req.body,
        userId
  ...
  app.post(
    ...
    async (req: any, res) => {
      try {
        ...
        const evidence = await GRCService.uploadEvidence(
          tenantId,
          controlId,
          req.body,
          userId
  ...
  app.get(
    ...
    async (req: any, res) => {
      try {
        ...
        const evidence = await GRCService.getControlEvidence(
          tenantId,
          controlId
        );
        res.json(evidence);
  ...
  app.get('/api/grc/checks/results', isAuthenticated, async (req: any, res) => {
    try {
      ...
      const results = await ChecksEngine.getCheckResults(
        tenantId,
        controlId as string
      );
      res.json(results);
  ...
  app.post(
    ...
    async (req: any, res) => {
      try {
        ...
        const result = await FindingsService.importPenTestFindings(
          tenantId,
          req.body,
          userId
        );
  ...
  // Triage finding
  app.put(
    '/api/grc/findings/:findingId/triage',
    isAuthenticated,
    async (req: any, res) => {
      try {
        const tenantId = req.headers['x-tenant-id'] || 'default';
        ...
        const finding = await FindingsService.triageFinding(
          tenantId,
          findingId,
          req.body,
          userId
  ...
  // Add remediation
  app.post(
    '/api/grc/findings/:findingId/remediation',
    isAuthenticated,
    async (req: any, res) => {
      try {
        const tenantId = req.headers['x-tenant-id'] || 'default';
        ...
        const remediation = await FindingsService.addRemediation(
          tenantId,
          findingId,
          req.body,
          userId
  ...
  // Request retest
  app.post(
    '/api/grc/findings/:findingId/retest',
    isAuthenticated,
    async (req: any, res) => {
      try {
        const tenantId = req.headers['x-tenant-id'] || 'default';
        ...
        const finding = await FindingsService.requestRetest(
          tenantId,
          findingId,
          userId
        );
  ...
  app.get(
    ...
    async (req: any, res) => {
      try {
        ...
        const details = await FindingsService.getFindingDetails(
          tenantId,
          findingId
        );
        res.json(details);
  ...
  app.get('/api/grc/integrations', isAuthenticated, async (req: any, res) => {
    try {
      const tenantId = req.headers['x-tenant-id'] || 'default';
      const integrations =
        await IntegrationsHub.getTenantIntegrations(tenantId);
  ...
  app.post(
    '/api/grc/integrations/:configId/test',
    isAuthenticated,
    async (req: any, res) => {
      try {
  ...
  // Sync integration
  app.post(
    '/api/grc/integrations/:configId/sync',
    isAuthenticated,
    async (req: any, res) => {
      try {
        const { configId } = req.params;
  ...
  app.post(
    ...
    async (req: any, res) => {
      try {
        ...
        res
          .status(500)
          .json({ error: 'Failed to initialize controls framework' });
      }
    }
  );
