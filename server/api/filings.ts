...
const idempotencyMiddleware = (req: any, res: any, next: any) => {
  ...
  if (!idempotencyKey && req.method === 'POST') {
    return res
      .status(400)
      .json({ error: 'Idempotency-Key header required for filing operations' });
  }
  ...
  async (req, res) => {
    try {
      ...
      if (error instanceof z.ZodError) {
        return res
          .status(400)
          .json({ error: 'Invalid APD filing data', details: error.errors });
      }
  ...
  async (req, res) => {
    try {
      ...
      if (error instanceof z.ZodError) {
        return res
          .status(400)
          .json({ error: 'Invalid ΦΜΥ filing data', details: error.errors });
      }
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const webhookEvent =
        created.status === 'error' ? 'ergani.error' : 'filing.submitted';
      ...
      if (error instanceof z.ZodError) {
        return res
          .status(400)
          .json({ error: 'Invalid ERGANI event data', details: error.errors });
      }
