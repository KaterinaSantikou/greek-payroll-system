...
import {
  validateIbanEnhanced,
  getValidationMetrics,
} from '@shared/ibanValidationEnhanced';
...
const ValidateIbanSchema = z.object({
  ...
  nameThreshold: z.number().min(0).max(1).optional().default(0.8),
});
...
export function registerIbanValidationRoutes(app: Express) {
  ...
  router.get('/employee/:employeeId', isAuthenticated, async (req, res) => {
    try {
      ...
      const ibanDetails =
        await IbanValidationService.getEmployeeIban(employeeId);
  ...
  router.get(
    ...
    async (req, res) => {
      try {
        ...
        const history = await IbanValidationService.getValidationHistory(
          employeeId,
          limit
        );
    ...
    try {
      ...
      const kpis = {
        ...
        qualityMetrics: {
          ...
          mostCommonRejectReason:
            Object.entries(metrics.rejectReasons).sort(
              ([, a], [, b]) => b - a
            )[0]?.[0] || 'N/A',
        },
        slaCompliance: {
          ...
          currentSlaScore:
            metrics.p95TimeMs < 150 && metrics.failRate < 0.05 ? 100 : 85,
        },
      };
        ...
        data: {
          summary: {
            ...
            definitionOfDone: {
              ...
              p95Under150ms: results.performanceMetrics.allUnder150ms
                ? '✓ Met'
                : '✗ Failed',
