...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    if (allowedMimes.includes(file.mimetype)) {
      ...
      cb(new Error('Unsupported file type. Please upload PDF, image, or document files.'));
    }
  }
});
...
export function registerDocumentAIRoutes(app: Express) {
    ...
    try {
      ...
      if (documentType && !validTypes.includes(documentType)) {
        return res.status(400).json({ 
          error: 'Invalid document type. Must be one of: contract, id_card, passport, other' 
        });
      }
      ...
      if (result.documentType === 'id_card' || result.documentType === 'contract') {
        validationResult = documentAI.validateGreekData(result.structuredData as any);
      }
   ...
   */
  app.post('/api/documents/process-batch', isAuthenticated, upload.array('documents', 10), async (req, res) => {
    try {
      ...
      res.json({
        ...
        data: {
          ...
          summary: {
            ...
            contracts: results.filter(r => r.documentType === 'contract').length,
    ...
    try {
      ...
      res.json({
        ...
        data: {
          ...
          suggestions: {
            contract: text.toLowerCase().includes('σύμβαση') || text.toLowerCase().includes('εργασία'),
            id_card: text.toLowerCase().includes('ταυτότητα') || text.toLowerCase().includes('δελτίο'),
            passport: text.toLowerCase().includes('διαβατήριο') || text.toLowerCase().includes('passport')
          }
        }
      });
   ...
   */
  app.post('/api/documents/validate-greek-data', isAuthenticated, async (req, res) => {
    try {
      const { data } = req.body;
   ...
   */
  app.post('/api/documents/create-signature-request', isAuthenticated, async (req, res) => {
    try {
      const {
        documentId,
        documentName,
      ...
      if (!documentId || !documentName || !documentUrl || !signers || !Array.isArray(signers)) {
        return res.status(400).json({ 
          error: 'Required fields: documentId, documentName, documentUrl, signers' 
        });
      }

      // Validate signers format
      for (const signer of signers) {
        if (!signer.name || !signer.email || !signer.role || typeof signer.sequence !== 'number') {
          return res.status(400).json({ 
   ...
   */
  app.post('/api/documents/send-signature-request/:requestId', isAuthenticated, async (req, res) => {
    try {
   ...
   */
  app.get('/api/documents/signature-request/:requestId', isAuthenticated, async (req, res) => {
    try {
      const { requestId } = req.params;
      
      const signatureRequest = eSignatureService.getSignatureRequest(requestId);
      ...
      res.json({
        ...
        data: {
          ...
          statistics: {
            ...
            signed: signatureRequest.signers.filter(s => s.status === 'signed').length,
            pending: signatureRequest.signers.filter(s => s.status === 'pending').length,
            declined: signatureRequest.signers.filter(s => s.status === 'declined').length,
          }
        }
      });
   ...
   */
  app.get('/api/documents/signature-requests', isAuthenticated, async (req, res) => {
    try {
   ...
   */
  app.post('/api/documents/send-reminder/:requestId', isAuthenticated, async (req, res) => {
    try {
      const { requestId } = req.params;
