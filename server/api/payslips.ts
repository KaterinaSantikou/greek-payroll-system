...
export async function getPayslip(req: Request, res: Response) {
  try {
    ...
    const payslip = await generatePayslip(employee, period, includeDeltas === 'true');
...
export async function getPayslipHistory(req: Request, res: Response) {
  try {
    ...
    const history = await getPayslipHistoryData(employee, parseInt(limit as string), year as string);
...
export async function getBulkPayslips(req: Request, res: Response) {
  try {
    ...
    if (!['manager', 'hr', 'payroll'].includes(userRole)) {
      return res.status(403).json({ error: 'Insufficient permissions for bulk access' });
    }

    const payslips = await getBulkPayslipData(period, department as string, status as string);
    
    // Apply role filtering to each payslip
    const filteredPayslips = payslips.map(p => applyPayslipRoleFiltering(p, userRole));
...
// Helper functions
async function generatePayslip(employeeId: string, period: string, includeDeltas: boolean): Promise<PayslipData> {
  ...
  const mockPayslip: PayslipData = {
    ...
    summary: {
      totalEarnings: 2890.00,
      totalDeductions: 1013.94,
      netPay: 1876.06,
      taxableIncome: 2670.00,
      socialInsuranceBasis: 2670.00,
    ...
    deltas: includeDeltas ? {
      ...
      changes: {
        ...
        'OT': {
          previousAmount: 225.00,
          currentAmount: 270.00,
          delta: 45.00,
          deltaPercentage: 20.00,
...
async function getPayslipHistoryData(employeeId: string, limit: number, year?: string): Promise<PayslipData[]> {
  // Mock history generation
  const periods = ['2025-01', '2024-12', '2024-11', '2024-10'].slice(0, limit);
  
  return Promise.all(periods.map(period => generatePayslip(employeeId, period, true)));
}

async function getBulkPayslipData(period: string, department?: string, status?: string): Promise<PayslipData[]> {
...
function generateBulkSummary(payslips: Partial<PayslipData>[]) {
  return {
    ...
    totalGrossPay: payslips.reduce((sum, p) => sum + (p.summary?.totalEarnings || 0), 0),
    totalNetPay: payslips.reduce((sum, p) => sum + (p.summary?.netPay || 0), 0),
    totalTaxes: payslips.reduce((sum, p) => sum + (p.summary?.ytdTotals?.taxes || 0), 0),
    totalEmployerCosts: payslips.reduce((sum, p) => sum + (p.summary?.employerCosts || 0), 0)
  };
}
...
async function canAccessPayslip(userId: string, role: string, employeeId: string): Promise<boolean> {
  switch (role) {
    case 'employee':
...
async function applyPayslipRoleFiltering(payslip: PayslipData, role: string): Promise<Partial<PayslipData>> {
  switch (role) {
    case 'employee':
      ...
      return {
        ...
        metadata: {
          ...
          bankAccount: payslip.metadata.bankAccount?.replace(/(.{4})(.*)(.{4})/, '$1****$3'), // Mask bank account
