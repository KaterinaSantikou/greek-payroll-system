...
const idempotencyMiddleware = (req: any, res: any, next: any) => {
  ...
  if (!idempotencyKey && ['POST', 'PUT', 'PATCH'].includes(req.method)) {
    return res
      .status(400)
      .json({ error: 'Idempotency-Key header required for write operations' });
  }
  ...
  async (req, res) => {
    try {
      ...
      if (existingEmployee.length > 0) {
        return res
          .status(409)
          .json({ error: 'Employee with this AFM already exists' });
      }
      ...
      if (error instanceof z.ZodError) {
        return res
          .status(400)
          .json({ error: 'Invalid employee data', details: error.errors });
      }
  ...
  async (req, res) => {
    try {
      ...
      if (error instanceof z.ZodError) {
        return res
          .status(400)
          .json({ error: 'Invalid contract data', details: error.errors });
      }
