...
const UpdateRulePackStatusSchema = z.object({
  id: z.string(),
  status: z.enum(['draft', 'review', 'approved', 'published', 'superseded', 'retired']),
...
export function registerChangeLogLegalWatchRoutes(app: Express) {
  ...
  app.get('/api/changelog/rule-packs/:id', isAuthenticated, async (req, res) => {
    try {
      const { id } = req.params;
  ...
  app.post('/api/changelog/rule-packs', isAuthenticated, async (req, res) => {
    try {
      ...
      const rulePackVersion = changeLogLegalWatchService.createRulePackVersion(validatedData);
  ...
  app.put('/api/changelog/rule-packs/status', isAuthenticated, async (req, res) => {
    try {
      const { id, status, legalApproval } = UpdateRulePackStatusSchema.parse(req.body);
  ...
  app.get('/api/changelog/compare/:sourceId/:targetId', isAuthenticated, async (req, res) => {
    try {
      const { sourceId, targetId } = req.params;
      const comparison = changeLogLegalWatchService.compareRulePackVersions(sourceId, targetId);
  ...
  app.get('/api/changelog/tenants/:tenantId/deployments', isAuthenticated, async (req, res) => {
    try {
      const { tenantId } = req.params;
      const deployments = changeLogLegalWatchService.getTenantDeployments(tenantId);
      res.json({
        success: true,
        data: deployments
      });
  ...
  app.post('/api/changelog/deploy', isAuthenticated, async (req, res) => {
    try {
      const { tenantId, rulePackVersionId, deployedBy, notes } = DeployToTenantSchema.parse(req.body);
  ...
  app.put('/api/changelog/legal-watches', isAuthenticated, async (req, res) => {
    try {
      ...
      const updatedWatch = changeLogLegalWatchService.updateLegalWatch(id, updates);
  ...
  app.put('/api/changelog/legal-alerts', isAuthenticated, async (req, res) => {
    try {
      ...
      const updatedAlert = changeLogLegalWatchService.updateLegalAlert(id, processedUpdates);
  ...
  app.post('/api/changelog/simulate-legal-scan', isAuthenticated, async (req, res) => {
    try {
      ...
      const mockAlert = {
        ...
        summary: 'Proposed changes to Greek labor law regarding overtime calculations and digital work cards',
        fullText: 'The Greek Parliament is considering amendments to existing labor laws that would affect overtime calculations and mandate digital work card compliance for all employees in the hospitality sector.',
  ...
  app.post('/api/changelog/batch-deploy', isAuthenticated, async (req, res) => {
    try {
      const { tenantIds, rulePackVersionId, deployedBy, notes } = z.object({
      ...
      const deployments = tenantIds.map(tenantId => 
        changeLogLegalWatchService.deployToTenant(tenantId, rulePackVersionId, deployedBy, notes)
      );
      ...
      res.json({
        ...
        data: {
          ...
          summary: {
            ...
            successful: deployments.filter(d => d.deploymentStatus !== 'failed').length,
