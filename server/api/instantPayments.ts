...
const processInstantPayrollSchema = z.object({
  ...
  urgencyLevel: z
    .enum(['URGENT', 'CORRECTION', 'OFF_CYCLE', 'REGULAR'])
    .optional(),
});
...
export function registerInstantPaymentRoutes(app: Express): void {
   ...
   */
  app.get(
    '/api/instant-payments/capabilities',
    isAuthenticated,
    async (req: Request, res: Response) => {
      try {
   ...
   */
  app.post(
    '/api/instant-payments/pain002',
    async (req: Request, res: Response) => {
      try {
        const validatedData = processPain002Schema.parse(req.body);

        console.log(
          `📊 Processing pain.002 status report for message: ${validatedData.messageId}`
        );

        const result = await service.processPain002StatusReport(
          validatedData.statusXml
        );
   ...
   */
  app.post(
    '/api/instant-payments/camt054',
    async (req: Request, res: Response) => {
      try {
        const validatedData = processCamt054Schema.parse(req.body);

        console.log(
          `💰 Processing camt.054 notification for reconciliation: ${validatedData.reconciliationId}`
        );

        const result = await service.processCamt054Notification(
          validatedData.camtXml
        );
   ...
   */
  app.get(
    '/api/instant-payments/compliance-status',
    isAuthenticated,
    async (req: Request, res: Response) => {
      try {
        const now = new Date();
        ...
        const complianceStatus = {
          ...
          phase1Status: {
            ...
            daysRemaining: Math.max(
              0,
              Math.ceil(
                (phase1Deadline.getTime() - now.getTime()) /
                  (1000 * 60 * 60 * 24)
              )
            ),
          },
          phase2Status: {
            ...
            daysRemaining: Math.max(
              0,
              Math.ceil(
                (phase2Deadline.getTime() - now.getTime()) /
                  (1000 * 60 * 60 * 24)
              )
            ),
          },
  ...
  function generateCapabilityRecommendations(capabilities: any): string[] {
    ...
    if (!capabilities.instantSupported) {
      recommendations.push(
        'Consider upgrading to banks that support SEPA Instant transfers'
      );
    }

    if (capabilities.supportedBanks.length < 3) {
      recommendations.push(
        'Diversify banking relationships to increase instant payment coverage'
      );
    }
    ...
    recommendations.push(
      'Use instant payments for urgent corrections and off-cycle payroll'
    );
    recommendations.push(
      'Monitor pain.002 status reports for real-time payment tracking'
    );
  ...
  function generateComplianceRecommendations(
    ...
    if (now < phase1) {
      recommendations.push(
        'Urgently implement SCT Inst connectivity before January 9, 2025'
      );
      recommendations.push(
        'Test instant payment processing with major Greek banks'
      );
    } else if (now < phase2) {
      recommendations.push(
        'Ensure all banking partners support SCT Inst by October 9, 2025'
      );
      recommendations.push(
        'Optimize instant payment workflows for all payroll scenarios'
      );
    } else {
      recommendations.push(
        'Maintain compliance monitoring for regulatory updates'
      );
      recommendations.push(
        'Leverage instant payments for competitive advantage'
      );
    }

    recommendations.push(
      'Implement pain.002/camt.054 real-time reconciliation'
    );
