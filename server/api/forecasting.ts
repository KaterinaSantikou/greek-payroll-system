...
export function registerForecastingRoutes(app: Express): void {
  ...
  app.post(
    ...
    async (req: any, res) => {
      try {
        ...
        const forecast = await forecastingService.generateStaffingForecast(
          propertyId,
          forecastDays
        );
        ...
        res.json({
          ...
          data: {
            ...
            metadata: {
              ...
              forecastConfidence:
                forecast.forecast.reduce((sum, f) => sum + f.confidence, 0) /
  ...
  app.post(
    ...
    async (req: any, res) => {
      try {
        ...
        const forecast = await forecastingService.generateOvertimeForecast(
          propertyId,
          forecastWeeks
        );
        ...
        const riskSummary = {
          ...
          highRiskOutlets: forecast.outletForecasts.filter(
            o => o.riskLevel === 'high' || o.riskLevel === 'critical'
          ).length,
          totalPredictedCost: forecast.weeklyTrends.reduce(
            (sum, week) => sum + week.costImpact,
            0
          ),
          avgWeeklyCost:
            forecast.weeklyTrends.length > 0
              ? forecast.weeklyTrends.reduce(
                  (sum, week) => sum + week.costImpact,
                  0
                ) / forecast.weeklyTrends.length
              : 0,
        };
  ...
  app.post(
    '/api/forecasting/correlation-insights',
    isAuthenticated,
    async (req: any, res) => {
      try {
        const userId = req.user?.claims?.sub;
        ...
        const insights =
          await forecastingService.getOccupancyCorrelationInsights(propertyId);

        // Calculate optimization potential
        const optimizationPotential = {
          efficiencyGain:
            Math.max(0, 1.0 - insights.staffingEfficiency.currentEfficiency) *
            100,
          costSavingsEstimate:
            insights.staffingEfficiency.currentEfficiency < 0.8
              ? Math.round(
                  (0.8 - insights.staffingEfficiency.currentEfficiency) * 50000
                )
              : 0, // €50k baseline
          implementationPriority:
            insights.staffingEfficiency.currentEfficiency < 0.7
              ? 'high'
              : insights.staffingEfficiency.currentEfficiency < 0.85
                ? 'medium'
                : 'low',
        };
  ...
  app.get(
    '/api/forecasting/dashboard/:propertyId',
    isAuthenticated,
    async (req: any, res) => {
      try {
        const userId = req.user?.claims?.sub;
        ...
        const [staffingForecast, overtimeForecast, correlationInsights] =
          await Promise.all([
        ...
        const kpis = {
          nextWeekStaffingCost: staffingForecast.forecast
            .slice(0, 7)
            .reduce((sum, f) => sum + f.recommendedStaffingCost, 0),
          nextWeekOvertimeRisk: overtimeForecast.outletForecasts.filter(
            o => o.riskLevel === 'high' || o.riskLevel === 'critical'
          ).length,
          occupancyTrendDirection:
            staffingForecast.metrics.trends.occupancyTrend,
          efficiencyScore: Math.round(
            correlationInsights.staffingEfficiency.currentEfficiency * 100
          ),
          predictedRevenue: staffingForecast.forecast
            .slice(0, 7)
        ...
        if (staffingForecast.metrics.trends.overtimeTrend === 'increasing') {
          alerts.push({
            ...
            message:
              'Overtime hours showing increasing trend - consider staffing adjustments',
            priority: 'high',
          });
        }
        ...
        const quickWins = [
          ...correlationInsights.recommendations.staffingAdjustments.slice(
            0,
            2
          ),
          ...correlationInsights.recommendations.processImprovements.slice(
            0,
            1
          ),
        ].map(rec => ({
        ...
        res.json({
          ...
          data: {
            ...
            staffingForecast: {
              ...
              confidence:
                staffingForecast.forecast
                  .slice(0, 7)
                  .reduce((sum, f) => sum + f.confidence, 0) / 7,
            },
            overtimeForecast: {
              currentWeekTotal:
                overtimeForecast.currentWeekSummary.totalOvertimeHours,
              nextWeekPredicted:
                overtimeForecast.weeklyTrends[0]?.totalPredicted || 0,
              riskOutlets: overtimeForecast.outletForecasts.filter(
                o => o.riskLevel !== 'low'
              ).length,
            },
            correlationInsights: {
              efficiency:
                correlationInsights.staffingEfficiency.currentEfficiency,
              optimalRange:
                correlationInsights.staffingEfficiency.optimalOccupancyRange,
              costPerGuest:
                correlationInsights.staffingEfficiency.costPerGuestAtOptimal,
            },
            metadata: {
              generatedAt: new Date().toISOString(),
              generatedBy: userId,
  ...
  app.get(
    '/api/forecasting/historical/:propertyId',
    isAuthenticated,
    async (req: any, res) => {
      try {
        const userId = req.user?.claims?.sub;
        ...
        const forecast = await forecastingService.generateStaffingForecast(
          propertyId,
          1
        ); // Just to get historical data

        // Analyze historical performance
        const performance = {
          occupancyPatterns: forecast.metrics.seasonalPatterns.slice(
            0,
            monthsNum
          ),
          ...
          bestPerformingPeriods: forecast.historical
            .sort(
              (a, b) =>
                b.occupancy / (b.staffingCost / 1000) -
                a.occupancy / (a.staffingCost / 1000)
            )
            .slice(0, 5),
        ...
        if (forecast.metrics.correlation.occupancyToStaffing < 0.6) {
          performance.improvementOpportunities.push({
            ...
            impact: 'High',
            description:
              'Weak correlation between occupancy and staffing suggests scheduling optimization opportunity',
          });
        }
        ...
        if (forecast.metrics.trends.overtimeTrend === 'increasing') {
          performance.improvementOpportunities.push({
            ...
            impact: 'High',
            description:
              'Rising overtime trend indicates need for proactive workforce planning',
          });
        }
        ...
        res.json({
          ...
          data: {
            ...
            insights: {
              ...
              correlationScore:
                forecast.metrics.correlation.occupancyToStaffing,
              seasonalVariability:
                Math.max(
                  ...forecast.metrics.seasonalPatterns.map(s => s.avgOccupancy)
                ) -
                Math.min(
                  ...forecast.metrics.seasonalPatterns.map(s => s.avgOccupancy)
                ),
              efficiencyBenchmark:
                performance.bestPerformingPeriods[0]?.occupancy /
                  (performance.bestPerformingPeriods[0]?.staffingCost / 1000) ||
                0,
            },
            metadata: {
              generatedAt: new Date().toISOString(),
