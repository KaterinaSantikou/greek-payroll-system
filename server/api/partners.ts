...
export function registerPartnerRoutes(app: Express) {
  ...
  app.get('/api/partners/firms', isAuthenticated, async (req: any, res) => {
    try {
      ...
      const memberFirms = await db
        ...
        .innerJoin(
          partnerMembers,
          eq(partnerFirms.id, partnerMembers.partnerFirmId)
        )
        .where(
          and(
  ...
  app.get('/api/partners/clients', isAuthenticated, async (req: any, res) => {
    try {
      ...
      const accessibleClients = await db
        ...
        .innerJoin(
          partnerFirms,
          eq(clientAccessGrants.partnerFirmId, partnerFirms.id)
        )
        .innerJoin(
          partnerMembers,
          eq(partnerFirms.id, partnerMembers.partnerFirmId)
      ...
      const enhancedClients = clients.map(client => ({
        ...client,
        clientName:
          client.clientTenantId === 'princess-sa'
            ? 'Princess SA'
            : client.clientTenantId === 'demo-hotel'
              ? 'Demo Hotel Group'
              : `Client ${client.clientTenantId.slice(0, 8)}`,
        afm:
          client.clientTenantId === 'princess-sa' ? '123456789' : '987654321',
        makerCheckerMode: client.grantedScopes.includes('filings:submit')
          ? 'partner_checker'
          : 'client_checker',
  ...
  app.get('/v1/partner/obo-token', isAuthenticated, async (req: any, res) => {
    try {
      ...
      if (!tenant_id) {
        return res
          .status(400)
          .json({ error: 'tenant_id parameter is required' });
      }
      ...
      res.status(500).json({
        error:
          error instanceof Error
            ? error.message
            : 'Failed to create context token',
      });
    }
  });
  ...
  app.post(
    ...
    async (req: any, res) => {
      ...
      try {
        ...
        const consentActive = await ConsentService.isConsentActive(
          asTenantId,
          partnerFirmId
        );
        if (!consentActive) {
    ...
    async (req: any, res) => {
      try {
        ...
        if (!tenantId || !partnerFirmId) {
          return res
            .status(400)
            .json({ error: 'tenantId and partnerFirmId are required' });
        }
  ...
  // Revoke an OBO token
  app.delete(
    '/api/partners/obo-tokens/:tokenId',
    isAuthenticated,
    async (req: any, res) => {
      try {
        const userId = req.user?.claims?.sub;
    ...
    async (req: any, res) => {
      try {
        ...
        const [accessGrant] = await db
          ...
          .innerJoin(
            partnerMembers,
            eq(clientAccessGrants.partnerFirmId, partnerMembers.partnerFirmId)
          )
          .where(
            and(
              eq(partnerMembers.userId, userId),
  ...
  app.post(
    '/api/partners/invitations/send',
    isAuthenticated,
    async (req: any, res) => {
      try {
        const userId = req.user?.claims?.sub;
        const validatedData = insertClientAccessInvitationSchema.parse(
          req.body
        );
        ...
        if (!partnerMember.length) {
          return res
            .status(403)
            .json({ error: 'Not authorized for this partner firm' });
        }
        ...
        if (!partnerMember[0].canManageClients) {
          return res
            .status(403)
            .json({ error: 'Not authorized to send client invitations' });
        }
  ...
  app.get(
    '/api/partners/permission-scopes',
    isAuthenticated,
    async (req: any, res) => {
      try {
        res.json({
          ...
          makerCheckerModes: [
            {
              value: 'client_checker',
              label: 'Client approves partner actions',
              description: 'Partner prepares; client approves/submits.',
            },
            {
              value: 'partner_checker',
              label: 'Partner internal approval',
              description:
                'Partner staff prepares; partner reviewer approves/submits.',
            },
            {
              value: 'dual',
    ...
    async (req: any, res) => {
      try {
        ...
        if (!partnerMember[0].canManageClients) {
          return res
            .status(403)
            .json({ error: 'Not authorized to revoke client access' });
        }
