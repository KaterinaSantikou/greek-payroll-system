  ...
  try {
    ...
    if (!methodology || !participants || !severity || !title || !facilitator) {
      return res.status(400).json({
        success: false,
        error:
          'methodology, participants, severity, title, and facilitator are required',
      });
    }
...
router.get('/analyses', isAuthenticated, async (req, res) => {
  try {
    ...
    if (methodology) {
      filters.methodology = Array.isArray(methodology)
        ? methodology
        : [methodology];
    }
 ...
 */
router.post(
  '/analyses/:analysisId/action-items',
  isAuthenticated,
  async (req, res) => {
    try {
      const { analysisId } = req.params;
      ...
      averageDuration:
        analyses
          ...
          .reduce((acc, analysis) => {
            ...
            return acc + duration;
          }, 0) / analyses.filter(a => a.completedAt && a.startedAt).length ||
        0,
      completionRate:
