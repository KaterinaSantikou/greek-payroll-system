...
const CalculateAllocationSchema = z.object({
  periodId: z.string(),
  employeeShifts: z.array(z.object({
    employeeId: z.string(),
    roleId: z.string(),
...
const UpdateTipRoleSchema = z.object({
  ...
  updates: z.object({
    ...
    category: z.enum(['service', 'kitchen', 'support', 'management']).optional(),
...
const UpdatePOSOutletSchema = z.object({
  ...
  updates: z.object({
    name: z.string().optional(),
    type: z.enum(['restaurant', 'bar', 'room_service', 'spa', 'retail']).optional(),
...
export function registerHotelTipPoolingRoutes(app: Express) {
  ...
  app.put('/api/tip-pooling/outlets', isAuthenticated, async (req, res) => {
    try {
      ...
      const updatedOutlet = hotelTipPoolingService.updatePOSOutlet(outletId, updates);
  ...
  app.post('/api/tip-pooling/import-revenue', isAuthenticated, async (req, res) => {
    try {
  ...
  app.post('/api/tip-pooling/calculate/:periodId', isAuthenticated, async (req, res) => {
    try {
  ...
  app.post('/api/tip-pooling/allocations', isAuthenticated, async (req, res) => {
    try {
      const { periodId, employeeShifts } = CalculateAllocationSchema.parse(req.body);
      const allocations = hotelTipPoolingService.calculateAllocations(periodId, employeeShifts);
  ...
  app.get('/api/tip-pooling/payroll-summary/:periodId', isAuthenticated, async (req, res) => {
    try {
      const periodId = req.params.periodId;
  ...
  app.get('/api/tip-pooling/analytics', isAuthenticated, async (req, res) => {
    try {
      ...
      const analytics = hotelTipPoolingService.getRevenueAnalytics(startDate, endDate);
  ...
  app.post('/api/tip-pooling/simulate-data', isAuthenticated, async (req, res) => {
    try {
      const outlets = hotelTipPoolingService.getPOSOutlets();
      const simulatedData = [];
