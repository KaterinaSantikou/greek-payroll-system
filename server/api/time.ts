...
const idempotencyMiddleware = (req: any, res: any, next: any) => {
  ...
  if (!idempotencyKey && req.method === 'POST') {
    return res
      .status(400)
      .json({ error: 'Idempotency-Key header required for punch operations' });
  }
  ...
  async (req, res) => {
    try {
      ...
      if (error instanceof z.ZodError) {
        return res
          .status(400)
          .json({ error: 'Invalid punch data', details: error.errors });
      }
...
router.get('/api/timesheets', isAuthenticated, async (req, res) => {
  try {
    ...
    // Calculate totals
    const totals = {
      totalRegularHours: result.reduce(
        (sum, t) => sum + (t.regularHours || 0),
        0
      ),
      totalOvertimeHours: result.reduce(
        (sum, t) => sum + (t.overtimeHours || 0),
        0
      ),
      totalNightHours: result.reduce((sum, t) => sum + (t.nightHours || 0), 0),
      totalSundayHours: result.reduce(
        (sum, t) => sum + (t.sundayHours || 0),
        0
      ),
      totalHolidayHours: result.reduce(
        (sum, t) => sum + (t.holidayHours || 0),
        0
      ),
    };
...
router.post('/api/timesheets/lock', isAuthenticated, async (req, res) => {
  try {
    ...
    const result = await db
      ...
      .where(
        and(sql`id = ANY(${timesheetIds})`, eq(timesheets.status, 'draft'))
      )
      .returning();
