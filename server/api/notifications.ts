    ...
    async (req: any, res) => {
      try {
        ...
        if (!notification[0] || notification[0].status === 'acted_upon') {
          return res
            .status(400)
            .json({ message: 'Notification already processed' });
        }
        ...
        await processApprovalAction(notification[0], {
          action,
          comments,
          userId,
        });
...
async function processSlackApproval(
  actionData: any,
  slackUserId: string
): Promise<void> {
...
async function processTeamsApproval(
  data: any,
  teamsUserId: string
): Promise<void> {
