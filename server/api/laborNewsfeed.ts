    ...
    try {
      ...
      if (item.citations) {
        for (const citation of item.citations) {
          await db.insert(laborNewsfeedCitations).values({
      ...
      console.log(
        `✅ Processed news item: ${item.headline.substring(0, 50)}...`
      );
    } catch (error) {
...
Περίληψη:`;
  ...
  private async storeItemForReview(item: any, reason: string): Promise<void> {
    await db.insert(laborNewsfeedItems).values({
      externalId: item.id,
      category: item.category,
      headline: item.headline,
      summary:
        item.summary || 'Δεν είναι διαθέσιμη περίληψη - χρειάζεται έλεγχος',
      source: item.source,
      sourceUrl: item.url,
      publishedDate: item.date,
      isActive: false,
    ...
    return [
      {
        ...
        content:
          'Με την νέα υπουργική απόφαση, από την 1η Απριλίου 2025, ο κατώτατος μισθός για υπαλλήλους ορίζεται σε 880 ευρώ το μήνα, ενώ για εργάτες στα 39,30 ευρώ την ημέρα. Η αύξηση αφορά τόσο την πλήρη όσο και τη μερική απασχόληση.',
        ...
        citations: [
          {
            id: 'turn0search13',
            type: 'search',
            description: 'Ministry official announcement',
            url: 'https://ypergasias.gov.gr/',
          },
          {
            id: 'turn0search5',
            type: 'news',
            description: 'Press release confirmation',
            url: null,
          },
        ],
      },
      {
        ...
        content:
          'Η ψηφιακή κάρτα εργασίας επεκτείνεται σε χονδρικό εμπόριο, ενέργεια, χρηματοοικονομικούς τομείς και υποστήριξη τουρισμού. Πιλοτική εφαρμογή από 26/6/2025 και υποχρεωτική από 3/11/2025.',
        ...
        citations: [
          {
            id: 'turn0search11',
            type: 'search',
            description: 'Legal analysis KREMALIS',
            url: 'https://kremalis.com/',
          },
          {
            id: 'turn0search19',
            type: 'news',
  ...
  private async getConfig() {
    const existing = await db.select().from(laborNewsfeedConfig).limit(1);
  ...
  private async updateLastRefresh(): Promise<void> {
    ...
    const nextRefresh = new Date(
      now.getTime() + config.refreshIntervalMinutes * 60 * 1000
    );
...
export function registerLaborNewsfeedRoutes(app: Express): void {
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        res.json({
          ...
          last_updated:
            config.lastRefresh?.toISOString() || new Date().toISOString(),
          update_policy: {
            refresh_interval_minutes: config.refreshIntervalMinutes,
            sources_note:
              'Official ministry pages + reputable media/legal insights',
          },
  ...
  app.post(
    '/api/labor-newsfeed/refresh',
    isAuthenticated,
    async (req: Request, res: Response) => {
      try {
        await service.fetchAndParseNews();
        res.json({
          message: 'Newsfeed refresh completed',
  ...
  app.post(
    '/api/labor-newsfeed/auto-refresh',
    async (req: Request, res: Response) => {
      try {
