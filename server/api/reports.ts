...
router.post('/api/reports/apd/generate', isAuthenticated, async (req, res) => {
  try {
    ...
    if (!period) {
      return res
        .status(400)
        .json({ error: 'Period is required (format: YYYY-MM)' });
    }
    ...
    res.json({
      ...
      totalContributions:
        apdFile.totalEmployeeContributions + apdFile.totalEmployerContributions,
...
router.post(
  '/api/reports/ergani/termination',
  isAuthenticated,
  async (req, res) => {
    try {
...
router.post(
  '/api/reports/ergani/submit-batch',
  isAuthenticated,
  async (req, res) => {
    try {
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const payslip = await payslipGenerator.generatePayslip(
        employeeData,
        payrollData,
        period
      );
...
router.get(
  '/api/reports/payslip/:payslipId/html',
  isAuthenticated,
  async (req, res) => {
    try {
...
router.post('/api/reports/gl/export', isAuthenticated, async (req, res) => {
  try {
    ...
    const journalExport = await glExportService.generateJournalExport(
      payrollData,
      period,
      properties
    );
...
router.post(
  '/api/reports/audit-pack/generate',
  isAuthenticated,
  async (req, res) => {
    try {
      const {
      ...
      if (!employeeIds || !startDate || !endDate) {
        return res
          .status(400)
          .json({
            error: 'Employee IDs, start date, and end date are required',
          });
      }
...
router.post(
  '/api/reports/annual-certificate/generate',
  isAuthenticated,
  async (req, res) => {
    try {
      const {
        employeeId,
        year,
        certificationType = 'EMPLOYEE_COPY',
      } = req.body;

      if (!employeeId || !year) {
        return res
          .status(400)
          .json({ error: 'Employee ID and year are required' });
      }

      const certificate =
        await annualCertificateGenerator.generateAnnualCertificate(
          employeeId,
          year,
          certificationType
        );
...
router.post(
  '/api/reports/annual-certificate/aade-batch',
  isAuthenticated,
  async (req, res) => {
    try {
      ...
      if (!employeeIds || !year) {
        return res
          .status(400)
          .json({ error: 'Employee IDs and year are required' });
      }

      const submissionPackage =
        await annualCertificateGenerator.generateAADESubmissionPackage(
          employeeIds,
          year
        );
  ...
  try {
    const status = {
      ...
      systems: {
        apdGeneration: {
          status: 'operational',
          lastGenerated: '2025-01-19T12:00:00Z',
        },
        fmyGeneration: {
          status: 'operational',
          lastGenerated: '2025-01-19T12:00:00Z',
        ...
        glExport: {
          status: 'operational',
          lastExported: '2025-01-19T11:00:00Z',
        },
        auditPacks: {
          status: 'operational',
          lastGenerated: '2025-01-18T16:00:00Z',
        },
      },
      monthlyStatistics: {
