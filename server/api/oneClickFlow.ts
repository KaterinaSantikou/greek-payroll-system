...
const freezeRequestSchema = z.object({
  ...
  emergencyPassword: z
    .string()
    .min(8, 'Emergency password must be at least 8 characters'),
...
export async function freezeRunAndGenerateKit(req: Request, res: Response) {
  try {
    ...
    const { runId, reason, emergencyPassword, includeSepaXml } =
      validation.data;
...
export async function downloadOfflineKit(req: Request, res: Response) {
  try {
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename="${kitFilename}"`
    );
    ...
    const placeholderContent = Buffer.from(
      `Offline Kit ${freezeId} - Generated for emergency payroll processing`
    );
...
export async function performPreChecks(req: Request, res: Response) {
  try {
    ...
    if (Math.random() > 0.7) {
      preChecks.warnings.push(
        'Some payment lines have missing bank references'
      );
    }
