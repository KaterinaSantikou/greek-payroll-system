...
export function registerSelfServiceRoutes(app: Express) {
  ...
  app.get(
    ...
    async (req: any, res) => {
      try {
        ...
        const currentPaycheck = await db
          .select({
  ...
  app.get(
    '/api/self-service/paycheck-timeline/:year',
    isAuthenticated,
    async (req: any, res) => {
      try {
        const userId = req.user.claims.sub;
        ...
        const enhancedPaychecks = payhecks.map(paycheck => ({
          ...
          payslipData: paycheck.payslipData || {
            earnings: {
              ...
              allowances: paycheck.grossPay * 0.1,
            },
            deductions: {
  ...
  app.get(
    '/api/self-service/work-card-logs/:month',
    isAuthenticated,
    async (req: any, res) => {
      try {
        const userId = req.user.claims.sub;
  ...
  app.get(
    '/api/self-service/current-pay-period',
    isAuthenticated,
    async (req: any, res) => {
      try {
        const userId = req.user.claims.sub;
  ...
  app.get(
    '/api/self-service/correction-requests',
    isAuthenticated,
    async (req: any, res) => {
      try {
        const userId = req.user.claims.sub;
        ...
        res
          .status(500)
          .json({ message: 'Failed to fetch correction requests' });
      }
    }
  );
  ...
  app.post(
    '/api/self-service/time-correction',
    isAuthenticated,
    async (req: any, res) => {
      try {
        const userId = req.user.claims.sub;
        ...
        res
          .status(500)
          .json({ message: 'Failed to submit correction request' });
      }
    }
  );
  ...
  app.get(
    '/api/self-service/export-work-history',
    isAuthenticated,
    async (req: any, res) => {
      try {
        const userId = req.user.claims.sub;
        ...
        if (format === 'csv') {
          // Generate CSV
          const csvHeader =
            'Date,Clock In,Clock Out,Total Hours,Overtime,Location,ERGANI Status\n';
          const csvData = workHistory
            .map(
              log =>
          ...
          res.setHeader(
            'Content-Disposition',
            `attachment; filename="work-history-${startDate}-${endDate}.csv"`
          );
