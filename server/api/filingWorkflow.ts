...
export function registerFilingWorkflowRoutes(app: Express) {
  ...
  app.post('/api/partners/filings/apd/prepare', isAuthenticated, async (req: any, res) => {
    try {
      ...
      if (!period) {
        return res.status(400).json({ error: 'Period is required (e.g., "12/2024")' });
      }
      ...
      res.json({
        ...
        period,
        validationResults,
        canProceed: validationResults.isValid || Object.keys(validationOverrides || {}).length > 0,
        nextStep: validationResults.isValid ? 'send_for_approval' : 'fix_validation_errors',
      });
    } catch (error) {
  ...
  app.post('/api/partners/filings/apd/:filingId/send-for-approval', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user?.claims?.sub;
      const tenantId = (req.session as any).currentTenant;
      const partnerFirmId = (req.session as any).currentPartnerFirm;
      ...
      if (!draftFiling || draftFiling.status !== 'draft') {
        return res.status(404).json({ error: 'Draft filing not found or already processed' });
      }
      ...
      const approvalRequest = {
        ...
        status: 'pending',
        priority: new Date() < new Date(Date.now() + 7 * 24 * 60 * 60 * 1000) ? 'high' : 'normal',
        tenantId,
        partnerFirmId,
  ...
  app.get('/api/partners/approvals', isAuthenticated, async (req: any, res) => {
    try {
      ...
      const approvals = Object.values((req.session as any).approvals || {})
        .filter((approval: any) => 
  ...
  app.post('/api/partners/approvals/:approvalId/decision', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user?.claims?.sub;
      ...
      await AuditService.logEvent({
        ...
        eventData: {
          ...
          reviewTime: new Date().getTime() - new Date(approval.requestedAt).getTime(),
        },
      });
      ...
      res.json({
        ...
        nextStep: decision === 'approve' ? 'submit_to_aade' : 'revise_and_resubmit',
      });
    } catch (error) {
  ...
  app.post('/api/partners/filings/apd/:filingId/submit', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user?.claims?.sub;
      const tenantId = (req.session as any).currentTenant;
      const partnerFirmId = (req.session as any).currentPartnerFirm;
      ...
      if (!filing || filing.status !== 'ready') {
        return res.status(400).json({ error: 'Filing not ready for submission or not found' });
      }
      ...
      res.json({
        ...
        auditTrail: {
          ...
          approved: { by: filing.reviewedBy || 'auto', at: filing.reviewedAt },
  ...
  app.post('/api/partners/audit-pack/generate', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user?.claims?.sub;
      const tenantId = (req.session as any).currentTenant;
      const partnerFirmId = (req.session as any).currentPartnerFirm;
              ...
              setTimeout(() => {
                ...
                AuditService.logEvent({
                  ...
                  eventData: {
                    ...
                    processingTime: new Date().getTime() - new Date(jobRef.requestedAt).getTime(),
  ...
  app.get('/api/partners/audit-pack/:packId/status', isAuthenticated, async (req: any, res) => {
    try {
  ...
  // Download audit pack
  app.get('/api/partners/audit-pack/:packId/download', isAuthenticated, async (req: any, res) => {
    try {
      const { packId } = req.params;
      const job = (req.session as any).jobs?.[packId];
      
      if (!job || job.status !== 'completed') {
        return res.status(404).json({ error: 'Audit pack not ready or not found' });
      }
      ...
      res.setHeader('Content-Disposition', `attachment; filename="audit_pack_${job.tenantId}_${job.requestedAt.slice(0, 10)}.zip"`);
  ...
  app.get('/api/partners/filings', isAuthenticated, async (req: any, res) => {
    try {
      ...
      const drafts = Object.values((req.session as any).drafts || {})
        .filter((filing: any) => filing.tenantId === tenantId);
...
async function validateAPDSubmission(params: {
  ...
  const lowWageEmployees = params.employeeData.filter(emp => emp.grossSalary < minWage);
  if (lowWageEmployees.length > 0) {
    errors.push(`${lowWageEmployees.length} employees below minimum wage (€${minWage})`);
  }
  ...
  const overCapEmployees = params.employeeData.filter(emp => emp.contributions > maxContribution);
  if (overCapEmployees.length > 0) {
    warnings.push(`${overCapEmployees.length} employees exceed contribution cap`);
  }
  ...
  return {
    ...
    summary: {
      ...
      totalGross: params.employeeData.reduce((sum, emp) => sum + (emp.grossSalary || 0), 0),
      totalContributions: params.employeeData.reduce((sum, emp) => sum + (emp.contributions || 0), 0),
    },
  };
}

function parseDateRange(dateRange: string | { startDate: string; endDate: string }): { startDate: string; endDate: string } {
