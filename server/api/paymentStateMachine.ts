...
export function paymentStateMachineRoutes(app: Express) {
  ...
  app.get('/v1/payment-state-machine/definitions', async (req, res) => {
    try {
      res.json({
        ...
        state_machine_rules: {
          batch_flow:
            'prepared → submitted → accepted → partially_settled → settled → reconciled (or failed)',
          line_happy_path: 'prepared → submitted → accepted → settled',
          line_rejection_path: 'prepared/submitted → rejected',
          line_superseding:
            'Any non-final line can become superseded by re-issue',
          split_batch_rule:
            'A batch can be accepted while some lines are rejected',
          reissue_rule:
            'Re-issue creates new batch (usually SCT_INST) and sets old lines to superseded',
        },
      ...
      res.status(500).json({
        ...
        detail:
          error instanceof Error
            ? error.message
            : 'Failed to retrieve state machine definitions',
      });
    }
  });
   ...
   */
  app.post(
    '/v1/payment-state-machine/batch/:batch_id/transition',
    async (req, res) => {
      try {
        ...
        const result = await PaymentStateMachine.transitionBatchStatus(
          batch_id,
          new_status,
          reason
        );
        ...
        res.json({
          ...
          allowed_next_transitions:
            PaymentStateMachine.getAllowedBatchTransitions(new_status as any),
        });
      } catch (error) {
        ...
        res.status(500).json({
          ...
          detail:
            error instanceof Error
              ? error.message
              : 'Failed to transition batch status',
        });
      }
    }
  );
   ...
   */
  app.post(
    '/v1/payment-state-machine/batch/:batch_id/auto-update',
    async (req, res) => {
      try {
        const { batch_id } = req.params;

        const result =
          await PaymentStateMachine.autoUpdateBatchStatus(batch_id);

        res.json({
          batch_id,
          auto_update_result: result,
          state_machine: result.updated
            ? 'BATCH_AUTO_UPDATED'
            : 'NO_UPDATE_NEEDED',
        ...
        res.status(500).json({
          ...
          detail:
            error instanceof Error
              ? error.message
              : 'Failed to auto-update batch status',
        });
      }
    }
  );
   ...
   */
  app.get(
    '/v1/payment-state-machine/batch/:batch_id/status-analysis',
    async (req, res) => {
      try {
        const { batch_id } = req.params;

        const analysis =
          await PaymentStateMachine.calculateBatchStatusFromLines(batch_id);

        res.json({
          batch_id,
          status_analysis: analysis,
          split_batch_detected:
            analysis.analysis.hasRejected &&
            analysis.lineStatusSummary.accepted > 0,
        ...
        res.status(500).json({
          ...
          detail:
            error instanceof Error
              ? error.message
              : 'Failed to analyze batch status',
        });
      }
    }
  );
   ...
   */
  app.post(
    '/v1/payment-state-machine/line/:line_id/transition',
    async (req, res) => {
      try {
        ...
        const result = await PaymentStateMachine.transitionLineStatus(
          line_id,
          new_status,
          reason_code,
          reason
        ...
        res.json({
          ...
          allowed_next_transitions:
            PaymentStateMachine.getAllowedLineTransitions(new_status as any),
        });
      } catch (error) {
        ...
        res.status(500).json({
          ...
          detail:
            error instanceof Error
              ? error.message
              : 'Failed to transition line status',
        });
      }
    }
  );
    ...
    try {
      ...
      if (!original_batch_id || !line_ids_to_supersede || !new_batch_id) {
        return res.status(400).json({
          ...
          detail:
            'original_batch_id, line_ids_to_supersede, and new_batch_id are required',
      ...
      res.status(500).json({
        ...
        detail:
          error instanceof Error
            ? error.message
            : 'Failed to process re-issue superseding',
      });
    }
  });
   ...
   */
  app.get(
    '/v1/payment-state-machine/batch/:batch_id/validate-consistency',
    async (req, res) => {
      try {
        const { batch_id } = req.params;

        const validation =
          await PaymentStateMachine.validateBatchConsistency(batch_id);

        res.json({
          batch_id,
          consistency_validation: validation,
          state_machine: validation.consistent
            ? 'BATCH_CONSISTENT'
            : 'BATCH_INCONSISTENT',
          ...
          split_batch_analysis: {
            is_split_batch:
              validation.statusAnalysis.analysis.hasRejected &&
              validation.statusAnalysis.lineStatusSummary.accepted > 0,
            accepted_lines:
              validation.statusAnalysis.lineStatusSummary.accepted,
            rejected_lines:
              validation.statusAnalysis.lineStatusSummary.rejected,
            superseded_lines:
              validation.statusAnalysis.lineStatusSummary.superseded,
        ...
        res.status(500).json({
          ...
          detail:
            error instanceof Error
              ? error.message
              : 'Failed to validate batch consistency',
        });
      }
    }
  );
   ...
   */
  app.get(
    '/v1/payment-state-machine/batch/:batch_id/split-display',
    async (req, res) => {
      try {
        const { batch_id } = req.params;
        ...
        res.json({
          ...
          cockpit_display: {
            batch_status_badge: {
              ...
              text: isSplitBatch
                ? `${analysis.recommendedStatus.toUpperCase()} (SPLIT)`
                : analysis.recommendedStatus.toUpperCase(),
            },
            line_breakdown: {
              ...
              progress_percentage:
                analysis.analysis.activeLines > 0
                  ? Math.round(
                      (analysis.lineStatusSummary.settled /
                        analysis.analysis.activeLines) *
                        100
                    )
                  : 0,
            },
            split_batch_indicators: isSplitBatch
              ? {
                  ...
                  success_rate: Math.round(
                    (analysis.lineStatusSummary.accepted /
                      analysis.analysis.activeLines) *
                      100
                  ),
                }
        ...
        res.status(500).json({
          ...
          detail:
            error instanceof Error
              ? error.message
              : 'Failed to generate split-batch display data',
        });
      }
    }
  );
    ...
    try {
      ...
      res.status(500).json({
        ...
        detail:
          error instanceof Error
            ? error.message
            : 'Failed to check state machine health',
      });
    }
  });
}
