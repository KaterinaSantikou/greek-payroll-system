...
export function registerSeveranceRoutes(app: Express) {
  ...
  app.get(
    '/api/severance/pending-approvals',
    isAuthenticated,
    async (req: any, res) => {
      try {
        // Mock data for now - in production this would come from database
    ...
    async (req: any, res) => {
      try {
        ...
        const calculationResult =
          await SeveranceFinalPayService.calculateSeveranceFinalPay(
            validatedData
          );
  ...
  app.post(
    '/api/severance/calculate-severance',
    isAuthenticated,
    async (req: any, res) => {
      try {
        ...
        const calculationOutputs =
          await SeveranceFinalPayService.calculateSeveranceFinalPay(inputs);
  ...
  app.post(
    '/api/severance/process-termination',
    isAuthenticated,
    async (req: any, res) => {
      try {
        const schema = z.object({
        ...
        if (inputs.requireApproval && previewCalculation.grossTotal > 10000) {
          // €10k threshold
          ...
          res.json({
            ...
            approvalId,
            message:
              'High-value severance requires legal and payroll admin approval',
            previewCalculation,
          });
        } else {
          // Process termination directly
          const result =
            await SeveranceFinalPayService.processTermination(inputs);
  ...
  app.get(
    '/api/severance/terminations/:employeeId',
    isAuthenticated,
    async (req: any, res) => {
      try {
        ...
        const terminations =
          await SeveranceFinalPayService.getEmployeeTerminations(employeeId);
  ...
  app.get(
    '/api/severance/calculation/:calculationId',
    isAuthenticated,
    async (req: any, res) => {
      try {
  ...
  app.post(
    '/api/severance/generate-ergani-payload/:terminationId',
    isAuthenticated,
    async (req: any, res) => {
      try {
        ...
        const terminations =
          await SeveranceFinalPayService.getEmployeeTerminations(
            employeeData.amka
          );
        ...
        const erganiPayload =
          await ErganiTerminationService.generateErganiTerminationPayload(
            terminationRecord,
            employeeData
          );
  ...
  app.post(
    '/api/severance/generate-termination-letter/:terminationId',
    isAuthenticated,
    async (req: any, res) => {
      try {
        const terminationId = req.params.terminationId;
        ...
        const { employeeData, includeSeveranceDetails } = schema.parse(
          req.body
        );
        ...
        if (includeSeveranceDetails && terminationRecord.severanceEligible) {
          severanceCalculation =
            await SeveranceFinalPayService.getSeveranceCalculation(
        ...
        switch (terminationRecord.terminationType) {
          ...
          default:
            throw new Error(
              `Unsupported termination type: ${terminationRecord.terminationType}`
            );
        }
  ...
  app.get(
    '/api/severance/validation/termination-rules',
    isAuthenticated,
    async (req: any, res) => {
      try {
        const rules = {
  ...
  app.post(
    ...
    async (req: any, res) => {
      try {
        ...
        const { getErganiEventCode } = await import(
          '../services/SeveranceHelpers'
        );
...
<ERGANI_TERMINATION>
  ...
  app.post(
    '/api/v1/docs/termination-letter',
    isAuthenticated,
    async (req: any, res) => {
      try {
        const schema = z.object({
        ...
        const data = schema.parse(req.body);
        const { generateTerminationLetter } = await import(
          '../services/SeveranceHelpers'
        );
...
<body>
  ...
  app.post(
    '/api/severance/test-golden',
    isAuthenticated,
    async (req: any, res) => {
      try {
        const { runGoldenTestSuite } = await import(
          '../services/SeveranceHelpers'
        );
