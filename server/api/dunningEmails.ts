...
import {
  DunningEmailService,
  type DunningVariables,
} from '../services/DunningEmailService';
...
router.post('/initialize/:invoiceId', isAuthenticated, async (req, res) => {
  try {
    ...
    const stages = await dunningService.initializeDunningSequence(
      invoiceId,
      variables
    );
...
router.post('/test', isAuthenticated, async (req, res) => {
  try {
    ...
    const testVariables: DunningVariables = {
      ...
      grace_suspend_date: new Date(
        Date.now() + 7 * 24 * 60 * 60 * 1000
      ).toISOString(),
      support_email: 'support@example.com',
      support_phone: isGreek ? '+30 210 123 4567' : '+1 (555) 123-4567',
      supplier_name: isGreek
        ? 'PayrollSync Ελλάδας ΑΕ'
        : 'PayrollSync Greece Ltd',
      supplier_vat: '123456789',
      supplier_tax_office: isGreek ? 'ΔΟΥ Αθηνών' : 'Athens Tax Office',
      supplier_address: isGreek
        ? 'Βασ. Σοφίας 123, 10676 Αθήνα'
        : 'Vas. Sofias 123, 10676 Athens',
