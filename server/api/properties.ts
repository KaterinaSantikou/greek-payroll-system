...
export function registerPropertiesRoutes(app: Express) {
  ...
  app.get("/api/properties", isAuthenticated, async (req: any, res) => {
    try {
      ...
      const propertiesWithStats = await db
        ...
        .from(properties)
        .leftJoin(employees, eq(properties.propertyId, employees.defaultPropertyId))
        .groupBy(
  ...
  app.get("/api/properties/stats/:propertyId", isAuthenticated, async (req: any, res) => {
    try {
      ...
      const result = {
        ...propertyStats,
        monthlyLabourCost: Math.round(propertyStats.activeEmployees * 2500 + Math.random() * 10000)
      };
  ...
  app.post("/api/user/property-preference", isAuthenticated, async (req: any, res) => {
    try {
      ...
      res.status(500).json({ message: "Failed to update property preference" });
    }
  });

  // Get consolidated group view data
  app.get("/api/properties/group-stats", isAuthenticated, async (req: any, res) => {
    try {
      ...
      res.json({
        ...
        totalMonthlyLabourCost: propertyBreakdown.reduce((sum, prop) => 
          sum + (prop.employeeCount * 2500 + Math.random() * 10000), 0
        )
      });
    } catch (error) {
