...
import { z } from "zod";
import { complianceCopilot, type ComplianceAnomaly } from "../ai/complianceCopilot";
...
export function registerAIRoutes(app: Express): void {
  ...
  app.post('/api/ai/compliance/analyze', isAuthenticated, async (req: any, res) => {
    try {
      ...
      const { propertyIds, startDate, endDate, severity, types } = validation.data;
      ...
      if (types && types.length > 0) {
        filteredAnomalies = filteredAnomalies.filter(a => types.includes(a.type));
      }
      ...
      res.json({
        ...
        data: {
          ...
          summary: {
            ...
            bySeverity: filteredAnomalies.reduce((acc, a) => {
              acc[a.severity] = (acc[a.severity] || 0) + 1;
              return acc;
            }, {} as Record<string, number>),
            byType: filteredAnomalies.reduce((acc, a) => {
              acc[a.type] = (acc[a.type] || 0) + 1;
              return acc;
            }, {} as Record<string, number>),
            autoFixable: filteredAnomalies.filter(a => a.autoFixable).length,
            totalFinancialRisk: filteredAnomalies.reduce((sum, a) => sum + (a.financialImpact || 0), 0)
          }
        }
      });
  ...
  app.get('/api/ai/compliance/insights', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user?.claims?.sub;
      ...
      const anomalies = await complianceCopilot.analyzeCompliance(propertyIds, startDate, endDate);
      ...
      for (let week = 0; week < Math.ceil(daysNum / 7); week++) {
        ...
        const weekAnomalies = anomalies.filter(a => 
          a.detectedAt >= weekStart && a.detectedAt < weekEnd
        );
        ...
        weeklyTrends.push({
          ...
          criticalCount: weekAnomalies.filter(a => a.severity === 'critical').length,
          mostCommonType: getMostCommonType(weekAnomalies),
          financialImpact: weekAnomalies.reduce((sum, a) => sum + (a.financialImpact || 0), 0)
        });
      }
      ...
      res.json({
        ...
        data: {
          ...
          overview: {
            ...
            autoFixablePercentage: Math.round((anomalies.filter(a => a.autoFixable).length / anomalies.length) * 100) || 0,
            totalFinancialRisk: anomalies.reduce((sum, a) => sum + (a.financialImpact || 0), 0),
            averageConfidence: Math.round(anomalies.reduce((sum, a) => sum + a.confidence, 0) / anomalies.length) || 0
          },
          trends: {
            weekly: weeklyTrends,
          ...
          riskAreas,
          recommendations: await generateSmartRecommendations(anomalies, propertyIds)
        }
      });
  ...
  app.post('/api/ai/compliance/auto-fix', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user?.claims?.sub;
      ...
      // Execute auto-fix
      const result = await complianceCopilot.autoFixAnomaly(anomalyId, userId);

      await auditLog({
        userId,
        action: result.success ? 'ai_auto_fix_success' : 'ai_auto_fix_failure',
        resourceType: 'compliance',
        resourceId: anomalyId,
        details: { 
  ...
  app.post('/api/ai/compliance/report', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user?.claims?.sub;
      ...
      const { propertyId, includeRecommendations, includeActionPlan, format } = validation.data;
      ...
      const report = await complianceCopilot.generateComplianceReport(anomalies, propertyId);
  ...
  app.get('/api/ai/compliance/config', isAuthenticated, async (req: any, res) => {
    try {
      res.json({
        success: true,
        data: {
...
async function generateSmartRecommendations(
  ...
  const wageViolations = anomalies.filter(a => a.type === 'under_min_wage').length;
  const overtimeIssues = anomalies.filter(a => a.type === 'excessive_overtime').length;
  const erganiDelays = anomalies.filter(a => a.type === 'ergani_delay').length;

  if (criticalCount > 0) {
    recommendations.push(`🚨 ${criticalCount} critical compliance issues require immediate attention to avoid penalties`);
  }

  if (autoFixableCount > 5) {
    recommendations.push(`⚡ ${autoFixableCount} issues can be automatically resolved - consider enabling auto-fix`);
  }

  if (wageViolations > 0) {
    recommendations.push(`💰 Wage calculation system needs review - ${wageViolations} minimum wage violations detected`);
  }

  if (overtimeIssues > 3) {
    recommendations.push(`⏰ High overtime violations suggest need for better workforce planning and scheduling`);
  }

  if (erganiDelays > 0) {
    recommendations.push(`📋 ${erganiDelays} ERGANI delays detected - implement automated submission workflows`);
  }

  // Add property-specific recommendations if analyzing specific properties
  if (propertyIds && propertyIds.length === 1) {
    recommendations.push(`🏨 Property-specific analysis complete - consider comparing with other properties for benchmarking`);
  }
