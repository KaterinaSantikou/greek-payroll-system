...
import { z } from 'zod';
import {
  legalDocumentTracker,
  getLegalReferencesForPayroll,
} from './legal-document-tracker.js';
...
class GreekLawConfigLoader {
  ...
  async loadConfig(version?: string): Promise<GreekLawConfig> {
    ...
    try {
      ...
      console.warn(
        `Failed to load config version ${configVersion}, falling back to defaults:`,
        error
      );
    }
   ...
   */
  private async loadFromExternalSource(
    version: string
  ): Promise<GreekLawConfig | null> {
  ...
  private loadFromEnvironmentVariables(version: string): GreekLawConfig | null {
    try {
      ...
      if (
        !process.env.GREEK_MINIMUM_WAGE_MONTHLY ||
        !process.env.GREEK_EFKA_EMPLOYEE_MAIN
      ) {
        return null;
      }
      ...
      const config: GreekLawConfig = {
        version,
        effectiveDate:
          process.env.GREEK_LAW_EFFECTIVE_DATE || new Date().toISOString(),
        legalReferences: process.env.GREEK_LAW_REFERENCES?.split(',') || [],

        taxBrackets: this.parseEnvTaxBrackets('GREEK_TAX_BRACKETS'),
        solidarityTaxBrackets: this.parseEnvTaxBrackets(
          'GREEK_SOLIDARITY_TAX_BRACKETS'
        ),

        efkaRates: {
          employee: {
            main: parseFloat(process.env.GREEK_EFKA_EMPLOYEE_MAIN || '0.1067'),
            auxiliary: parseFloat(
              process.env.GREEK_EFKA_EMPLOYEE_AUXILIARY || '0.0333'
            ),
            unemployment: parseFloat(
              process.env.GREEK_EFKA_EMPLOYEE_UNEMPLOYMENT || '0.0213'
            ),
          },
          employer: {
            main: parseFloat(process.env.GREEK_EFKA_EMPLOYER_MAIN || '0.1542'),
            auxiliary: parseFloat(
              process.env.GREEK_EFKA_EMPLOYER_AUXILIARY || '0.0333'
            ),
            unemployment: parseFloat(
              process.env.GREEK_EFKA_EMPLOYER_UNEMPLOYMENT || '0.0503'
            ),
            sickness: parseFloat(
              process.env.GREEK_EFKA_EMPLOYER_SICKNESS || '0.0287'
            ),
            workAccident: parseFloat(
              process.env.GREEK_EFKA_EMPLOYER_WORK_ACCIDENT || '0.0067'
            ),
          },
        },
        ...
        minimumWage: {
          ...
          effectiveDate:
            process.env.GREEK_MINIMUM_WAGE_EFFECTIVE_DATE || '2024-04-01',
          legalReference: process.env.GREEK_MINIMUM_WAGE_LEGAL_REF,
        },

        workingTimeLimits: {
          standardMonthlyHours: parseFloat(
            process.env.GREEK_STANDARD_MONTHLY_HOURS || '173.33'
          ),
          maxDailyHours: parseFloat(process.env.GREEK_MAX_DAILY_HOURS || '8'),
          maxWeeklyHours: parseFloat(
            process.env.GREEK_MAX_WEEKLY_HOURS || '40'
          ),
          maxOvertimeDaily: parseFloat(
            process.env.GREEK_MAX_OVERTIME_DAILY || '2'
          ),
          maxOvertimeWeekly: parseFloat(
            process.env.GREEK_MAX_OVERTIME_WEEKLY || '5'
          ),
          maxAnnualOvertime: parseFloat(
            process.env.GREEK_MAX_ANNUAL_OVERTIME || '150'
          ),
        },

        taxFreeLimits: {
          mealVouchers: parseFloat(
            process.env.GREEK_TAX_FREE_MEAL_VOUCHERS || '11'
          ),
          transportAllowance: parseFloat(
            process.env.GREEK_TAX_FREE_TRANSPORT || '150'
          ),
          educationAllowance: parseFloat(
            process.env.GREEK_TAX_FREE_EDUCATION || '200'
          ),
          effectiveDate:
            process.env.GREEK_TAX_FREE_EFFECTIVE_DATE || '2024-01-01',
        },

        tipsRules: {
          flatTaxRate: parseFloat(process.env.GREEK_TIPS_TAX_RATE || '0.15'),
          minimumDeclaredPercentage: parseFloat(
            process.env.GREEK_TIPS_MIN_DECLARED || '0.08'
          ),
        },
      ...
      console.warn(
        'Failed to parse environment variables for Greek law config:',
        error
      );
      return null;
    }
...
export async function getGreekLawConfig(
  version?: string
): Promise<GreekLawConfig> {
